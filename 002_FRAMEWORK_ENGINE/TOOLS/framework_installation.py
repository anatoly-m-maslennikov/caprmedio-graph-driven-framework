...
The canonical source is ``002_FRAMEWORK_ENGINE/TOOLS`` in the target
...
SOURCE_DIRECTORY = Path("002_FRAMEWORK_ENGINE/TOOLS")
...
def install_release(
    ...
    *,
    ...
    staging = Path(tempfile.mkdtemp(prefix=f".staging-{uuid.uuid4().hex}-", dir=install_root))
    try:
        for row in rows:
