...
- **Intended use:** Support `CARMADIO-REQUIREMENT-GOV-022`,
  `CARMADIO-TEST-CASE-GOV-022`, and `CARMADIO-TASK-GOV-041`; not certify hosted delivery,
...
- **Method/setup:** Resolve the current repository-owned lifecycle rules; audit
  ...
  schemas, templates, and `CARMADIO-E155`; run canonical verification; then run the
...
- **Reopen when:** Change/intake/Decision/proof/run/checkpoint provenance shape,
  the YAML subset, `CARMADIO-E155`, the canonical verification commands, or bounded
...
| Failure contract | Missing or malformed Change/intake provenance and missing Decision/proof fields produce stable `CARMADIO-E155`; Markdown `none` is accepted only as an explicit human-only disposition | Pass |
...
| Commit provenance | The evaluated commit body contains `Implements: CARMADIO-DECISION-GOV-001` and the contributing Codex session | Pass |
...
`CARMADIO-PROBLEM-TOOL-001`, not a failure of the candidate or adopter checks.
