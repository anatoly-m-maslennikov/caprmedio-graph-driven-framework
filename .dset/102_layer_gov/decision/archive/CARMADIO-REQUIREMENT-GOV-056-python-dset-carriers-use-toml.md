...
artifact_id: CARMADIO-REQUIREMENT-GOV-056
...
relations:
  - type: child_of
    targets:
      - CARMADIO-REQUIREMENT-GOV-040
  - type: override_of
    targets:
      - CARMADIO-DECISION-GOV-015
  - type: replacement_of
    targets:
      - CARMADIO-CONSTRAINT-GOV-001
---

# Requirement — Python-owned DSET carriers use TOML

All CARMADIO-owned structured artifacts, settings, generated structured state,
...
The Python-owned DSET framework uses TOML for every CARMADIO-owned structured carrier and schema; YAML remains only where an external host contract mandates YAML, and JSON remains only in genuinely JavaScript or TypeScript-owned areas.
