...
**Rule ID:** `CAPRMADIO-RULE-ARCHITECTURE`
...
at `.caprmadio/caprmadio_settings.toml`; `.caprmadio/artifact_catalog.toml` separately owns
...
sources inside the target project's `.caprmadio`. An active source atom is authority; the
...
text. Atomic sources: `CAPRMADIO-REQUIREMENT-GOV-102` and
...
otherwise valid rule. `CAPRMADIO-RULE-ARTIFACT-MAINTENANCE` governs transactional
discharge and risk-proportionate proof with explicit reopen conditions.
Atomic source: `CAPRMADIO-IMPL-GOV-005`.
...
continue into governed work. `CAPRMADIO-RULE-LIFECYCLE` governs later local routing,
not the pre-root bootstrap command.

1. Walk upward from the working location until exactly one project containing the unique settings carrier `caprmadio_settings.toml` inside `.caprmadio` is found. Nested competing project roots fail closed.
2. Read the selected `repository_governance` profile.
3. Search only the selected `.caprmadio` for settings, governing documents, atoms,
...
paths. A resolver may derive a transient location after selecting `.caprmadio`, but
must not expose that location as identity or use files outside `.caprmadio` as
...
catalogs, tools, and skill instructions live in `.caprmadio/`. Durable running
records live in `.caprmadio_journal/` as append-only NDJSON. Disposable caches,
locks, scratch state, and process-local output live in `.caprmadio_runtime/`.
No reader treats the journal or runtime root as a governance fallback.
Atomic source: `CAPRMADIO-REQUIREMENT-GOV-111`.
...
Atomic sources: `CAPRMADIO-REQUIREMENT-META-041`,
`CAPRMADIO-REQUIREMENT-META-054`, and `CAPRMADIO-REQUIREMENT-GOV-108`.
...
truth. It never links individual atoms or `.caprmadio_runtime` descendants. It is
...
Atomic source: `CAPRMADIO-REQUIREMENT-GOV-104`.
...
edge.
Atomic sources: `CAPRMADIO-IMPL-GOV-004` and
`CAPRMADIO-DECISION-GOV-035`.
