...
at `.carmadio/carmadio_settings.toml`; `.carmadio/artifact_catalog.toml` separately owns
...
sources inside the target project's `.carmadio`. An active source atom is authority; the
...
1. Walk upward from the working location until exactly one project containing the unique settings carrier `carmadio_settings.toml` inside `.carmadio` is found. Nested competing project roots fail closed.
2. Read the selected `repository_governance` profile.
3. Search only the selected `.carmadio` for settings, governing documents, atoms,
...
paths. A resolver may derive a transient location after selecting `.carmadio`, but
must not expose that location as identity or use files outside `.carmadio` as
...
catalogs, tools, and skill instructions live in `.carmadio/`. Durable running
records live in `.carmadio_journal/` as append-only NDJSON. Disposable caches,
locks, scratch state, and process-local output live in `.carmadio_runtime/`.
...
`.carmadio/000_carmadio_methodology/` as ordinary project-local files. Skills resolve
only the installed `.carmadio` copy during governed work; they do not follow
...
truth. It never links individual atoms or `.carmadio_runtime` descendants. It is
