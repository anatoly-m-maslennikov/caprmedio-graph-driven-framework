...
META_ROOT = Path(".caprmadio/100_LAYER_1_META")
TARGET = META_ROOT / "04_requirement/CAPRMADIO-META-CATL-001-requirement--active-atoms-by-subject-scope.md"
ROLE_DIRECTORIES = {
    "01_concern",
    "02_analysis",
    "03_plan",
...
def load_atoms(root: Path) -> list[Atom]:
    ...
    for path in sorted(meta_root.rglob("CAPRMADIO-*.md")):
        if path == root / TARGET:
            continue
...
def publish(root: Path, rendered: str, frontier: str, session_id: str, updated_at: str) -> None:
    ...
    common = {
        ...
        "subjects": ["CAPRMADIO-META-CATL-001", frontier],
    }
