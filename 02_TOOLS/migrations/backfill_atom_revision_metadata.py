#!/usr/bin/env python3
"""Backfill version and updated_at on every CAPRMADIO Markdown Atom.

Usage:
    python3 02_TOOLS/migrations/backfill_atom_revision_metadata.py
    python3 02_TOOLS/migrations/backfill_atom_revision_metadata.py --apply
...
def is_atom(path: Path, delimiter: str, frontmatter: str) -> bool:
    ...
    return path.name.startswith("CAPRMADIO-") and role_directory(path) is not None
...
def candidate_paths(root: Path) -> list[Path]:
    ...
    paths = [
        path
        for path in control_root.rglob("CAPRMADIO-*.md")
        if "000_caprmadio_framework" not in path.parts
    ]
    return sorted(paths)
