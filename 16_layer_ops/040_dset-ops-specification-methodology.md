...
### CARMADIO-DECISION-OPS-007 — Flat Version lifecycle roles
...
### CARMADIO-DECISION-OPS-013 — Operate without backward implementation authority
...
## CARMADIO-REQUIREMENT-OPS-001 — Production supportability is explicit and risk-scaled
...
**Scenario CARMADIO-SCENARIO-OPS-001:** A production-bound local tool uses bounded structured local diagnostic records and run/build identity appropriate to its risk without being forced to deploy a tracing backend; a non-production one-shot tool records a justified not-applicable disposition.

**Scenario CARMADIO-SCENARIO-OPS-002:** A distributed, stateful, retryable, or high-risk service propagates correlation and deploy/change identity across relevant effect boundaries, provides safe bounded diagnostics and incident runbooks, and adds tracing, audited access, rollback, or stronger redaction when its topology and profile require them.

## CARMADIO-REQUIREMENT-OPS-002 — Repository delivery is supportable
...
**Scenario CARMADIO-SCENARIO-OPS-003:** An operator can diagnose a blocked or incorrect `dev → main` delivery, contain it by preserving the draft/PR evidence, and recover without bypassing protected `main` or rewriting history.

## CARMADIO-REQUIREMENT-OPS-003 — DSET 0.3 release is framework-first
...
**Scenario CARMADIO-SCENARIO-OPS-004:** A TypeScript-only gate is not applied to Python sources, but its versioned adopter fixture passes before the gate is presented to Your Harness as released DSET behavior.

## CARMADIO-REQUIREMENT-OPS-004 — Every protected release PR has one deterministic transition

After policy activation, every configured integration-to-protected release-branch PR must contain exactly one committed release declaration in one release-owning Change; any other participating Changes contain only a reference to that owner. Omission, multiple declarations, and dangling/cyclic references fail preparation and cannot classify the PR as non-release. The declaration records protected base ref/SHA/version, candidate SHA, target, readiness owner, and exactly one class. `CARMADIO-RULE-RELEASE` owns the complete bootstrap/pre-1.0/RC/final/post-1.0 table, highest-impact aggregation, ambiguity stop, integer arithmetic, and immutable published-RC progression.

**Scenario CARMADIO-SCENARIO-OPS-005:** `0.2.4` becomes `0.3.0` for a normal PR or `0.2.5` for a small PR; `0.9.0` becomes `0.10.0`, never `1.0.0`.

## CARMADIO-REQUIREMENT-OPS-005 — Release authority is configured and idempotent
...
**Scenario CARMADIO-SCENARIO-OPS-006:** A PR with mismatched product/package targets fails before merge; a passing merge is tagged at its exact merge commit without modifying `main` files afterward.

## CARMADIO-REQUIREMENT-OPS-006 — RC and final releases are fully working gates
...
**Scenario CARMADIO-SCENARIO-OPS-007:** A green candidate with one required pilot unfinished remains `0.y.z`; a fully working `1.0.0-rc.2` may promote to `1.0.0` only after final evidence passes and without adding features.

## CARMADIO-REQUIREMENT-OPS-007 — Product/package identity is coordinated
...
**Scenario CARMADIO-SCENARIO-OPS-008:** Product/package `0.3.0` may validly use schema `1.0` and governance profile `core-v1@0.2`; neither compatibility version implies DSET 1.0 readiness.

## CARMADIO-REQUIREMENT-OPS-013 — Release lifecycle artifacts are flat and typed
...
**Scenario CARMADIO-SCENARIO-OPS-014:** A `0.4.0` candidate references its `0.4`
