...
| Inject fakes for the network/clock/disk; every fixed bug ships a pinning test — plan per `02_Test and Evaluation Case Patterns — Proof Artifact Conventions.md`, enforced per `05_Layered Build Standard — DDD, TDD, Small Functions, Typed Gates.md` | **Test doubles / Humble Object** (Meszaros, *xUnit Test Patterns*); **regression-test-driven** |
...
- [ ] **Spec first** — write the entities + their state machine before code; let tests pin each rule (spec: `01_Spec Authoring Patterns — Service Spec Conventions.md`; test case: `02_Test and Evaluation Case Patterns — Proof Artifact Conventions.md`).
...
- [ ] **A regression test for every fixed bug** (fails on old code, passes on the fix), in the same change — per `02_Test and Evaluation Case Patterns — Proof Artifact Conventions.md`.
