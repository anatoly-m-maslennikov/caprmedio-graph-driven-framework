...
class RepositoryLayoutTest(unittest.TestCase):
    ...
    def test_legacy_slim_layout_remains_readable(self) -> None:
        with temporary_directory() as raw:
            ...
            (dset / "project").mkdir()
            (dset / "versions").mkdir()
            (dset / "caprmadio_settings.toml").write_text(
                'schema_version = "1.3"\n\n[structure]\nlayout = "slim-v1"\n',
                encoding="utf-8",
            )
            ...
            self.assertEqual(layout.settings_path, dset / "caprmadio_settings.toml")
            ...
            (root / "caprmadio_settings.toml").write_text("", encoding="utf-8")
    ...
    def test_separated_layout_repeats_content_roles_without_project_container(
        self,
    ) -> None:
        with temporary_directory() as raw:
            root = Path(raw).resolve()
            ...
            (caprmadio / "03_REQUIREMENT").mkdir()
            (caprmadio / "06_DELIVERY").mkdir()
            (caprmadio / "caprmadio_settings.toml").write_text(
                'schema_version = "1.8"\n\n'
            ...
            self.assertEqual(layout.project_root, caprmadio)
            self.assertEqual(layout.versions_root, caprmadio / "06_DELIVERY")
            self.assertEqual(
                CONTENT_ROLE_DIRECTORIES,
                (
                    "01_CONCERN",
            ...
            self.assertFalse((caprmadio / "100_project").exists())
            self.assertFalse((caprmadio / "150_versions").exists())
            self.assertEqual(FLAT_APPLIED_LAYERS, frozenset({"implementation"}))
            self.assertTrue(
                (caprmadio / APPLIED_LAYER_DIRECTORIES["implementation"]).is_dir()
            )
    ...
    def test_legacy_layout_preserves_central_paths(self) -> None:
        with temporary_directory() as raw:
            ...
            self.assertEqual(layout.settings_path, root / "caprmadio_settings.toml")
    ...
    def test_layered_layout_discovers_owned_and_distributed_paths(self) -> None:
        with temporary_directory() as raw:
            ...
            (change / "change.yaml").write_text(
                dump({"id": "CAPRMADIO-CHANGE-TOOL-001", "slug": "portable-cli"}),
                encoding="utf-8",
            )
            ...
            self.assertEqual(layout.find_change("CAPRMADIO-CHANGE-TOOL-001"), change)
