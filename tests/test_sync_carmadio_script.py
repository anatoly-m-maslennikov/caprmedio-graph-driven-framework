"""Verify the explicit root-to-installed CARMADIO synchronization script."""
...
from scripts import sync_carmadio


class SyncCarmadioScriptTests(unittest.TestCase):
    ...
    def test_preview_reports_drift_without_writing(self) -> None:
        ...
        with (
            patch.object(sync_carmadio, "methodology_drift", return_value=drift),
            patch.object(sync_carmadio, "sync_methodology") as sync,
        ):
            self.assertEqual(sync_carmadio.synchronize(Path(".")), drift)
    ...
    def test_apply_uses_canonical_engine_and_requires_zero_residual_drift(
        ...
        with (
            ...
            self.assertEqual(
                sync_carmadio.synchronize(Path("."), apply=True),
                initial,
            )
    ...
    def test_apply_fails_when_drift_remains(self) -> None:
        ...
        with (
            ...
            patch.object(sync_carmadio, "sync_methodology"),
    ...
    def test_apply_resynchronizes_a_changed_bootstrap_bundle(self) -> None:
        with (
            ...
            drift = sync_carmadio.synchronize(Path("."), apply=True)
        self.assertEqual(
            drift,
    ...
    def test_main_prints_stable_preview(self) -> None:
        ...
        output = io.StringIO()
        with (
            patch.object(sync_carmadio, "synchronize", return_value=drift),
            contextlib.redirect_stdout(output),
        ):
            self.assertEqual(sync_carmadio.main(["."]), 0)
