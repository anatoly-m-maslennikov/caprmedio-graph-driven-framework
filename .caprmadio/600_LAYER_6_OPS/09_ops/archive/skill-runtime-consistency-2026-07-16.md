...
- **Intended use:** Support `CAPRMADIO-DECISION-SKILL-001` and
  `CAPRMADIO-DECISION-GOV-005` for the bounded deterministic claims above.
- **LLM session IDs:**
...
release-transition library cores are implemented. `CAPRMADIO-TASK-TOOL-010` remains
open for initialization, the portable CLI/host bridge, synchronized version
writes, and post-merge publication. `CAPRMADIO-TASK-SKILL-020` remains open for real
...
independent eval remain open under `CAPRMADIO-TASK-GOV-045` and
`CAPRMADIO-TASK-TOOL-046`.
