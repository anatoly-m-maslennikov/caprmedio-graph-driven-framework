...
- Active Change deltas connect to current layer-owned accepted IDs and retain
  only genuinely unaccepted additions. `CAPRMADIO-PROBLEM-GOV-001` is resolved.
...
| Generated temporary adopter | Partial pass | Candidate materializes and validates one bounded schema 1.1 compatibility adopter. A second generated current-schema 1.2 adopter remains `CAPRMADIO-PROBLEM-TOOL-002`. |
...
- `CAPRMADIO-PROBLEM-TOOL-001`: published-validator assurance remains a declared
  degraded schema-transition boundary.
- `CAPRMADIO-PROBLEM-TOOL-002`: no generated current-schema adopter.
- `CAPRMADIO-PROBLEM-SKILL-001`: `dset`, `dset-release`, run writer, checkpoint, and
  resume runtime are incomplete.
- `CAPRMADIO-PROBLEM-OPS-001`: no exact-current-head GitHub proof.
