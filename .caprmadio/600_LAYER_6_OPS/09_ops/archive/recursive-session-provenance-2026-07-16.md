...
- **Intended use:** Support `CAPRMADIO-REQUIREMENT-GOV-022`,
  `CAPRMADIO-TEST-CASE-GOV-022`, and `CAPRMADIO-TASK-GOV-041`; not certify hosted delivery,
...
- **Method/setup:** Resolve the current repository-owned lifecycle rules; audit
  ...
  schemas, templates, and `CAPRMADIO-E155`; run canonical verification; then run the
...
- **Reopen when:** Change/intake/Decision/proof/run/checkpoint provenance shape,
  the YAML subset, `CAPRMADIO-E155`, the canonical verification commands, or bounded
...
| Failure contract | Missing or malformed Change/intake provenance and missing Decision/proof fields produce stable `CAPRMADIO-E155`; Markdown `none` is accepted only as an explicit human-only disposition | Pass |
...
| Commit provenance | The evaluated commit body contains `Implements: CAPRMADIO-DECISION-GOV-001` and the contributing Codex session | Pass |
...
`CAPRMADIO-PROBLEM-TOOL-001`, not a failure of the candidate or adopter checks.
