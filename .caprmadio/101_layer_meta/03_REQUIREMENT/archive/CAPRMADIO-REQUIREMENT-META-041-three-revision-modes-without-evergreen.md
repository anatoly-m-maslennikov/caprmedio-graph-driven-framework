...
artifact_id: CAPRMADIO-REQUIREMENT-META-041
...
relations:
  - type: replacement_of
    targets:
      - CAPRMADIO-REQUIREMENT-META-040
  - type: relates_to
    targets:
...
CAPRMADIO has exactly three Revision modes:
...
`evergreen` is not part of the CAPRMADIO artifact model. Current specifications,
...
append-only record sequences, or mutable maintained artifacts. CAPRMADIO does not
use Evergreen as a mode, policy, artifact class, or lifecycle term.

## Rationale

The three values expose the only permitted change patterns needed by CAPRMADIO:
