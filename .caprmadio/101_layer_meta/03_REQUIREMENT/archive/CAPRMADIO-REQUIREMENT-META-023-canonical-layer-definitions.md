...
artifact_id: CAPRMADIO-REQUIREMENT-META-023
...
CAPRMADIO uses the ordered flow `META → GOV → TOOL → SKILL → IMPL → OPS`.
...
| TOOL | Executable CAPRMADIO capabilities, validation, resolution, diagnostics, generation, and repository mechanics |
...
CAPRMADIO uses the ordered layers META, GOV, TOOL, SKILL, IMPL, and OPS, with one canonical non-overlapping responsibility assigned to each layer.
