...
artifact_id: CAPRMADIO-REQUIREMENT-GOV-092
...
`.caprmadio/caprmadio_settings.toml` owns one documented `governance_surfaces` table.
...
caprmadio_settings.toml owns an explicit boolean activation state for every registered optional governance surface, with every surface inactive by default and deactivation preserving carriers and history.
