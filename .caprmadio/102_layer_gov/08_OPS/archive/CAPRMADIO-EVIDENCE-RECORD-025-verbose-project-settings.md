...
artifact_id: CAPRMADIO-EVIDENCE-RECORD-025
...
`CAPRMADIO-REQUIREMENT-GOV-037` and deterministic Test `CAPRMADIO-TEST-CASE-GOV-038`.

This record supports the canonical `caprmadio_settings.toml` cut only. It does not
...
`caprmadio_settings.toml`. The runtime reads the legacy `dset.toml` name only as a
