...
### CAPRMADIO-DECISION-SKILL-001 — Repository-local self-hosting
...
### CAPRMADIO-DECISION-SKILL-003 — Public wrapper surface
...
## CAPRMADIO-REQUIREMENT-SKILL-001 — Workflow skills remain focused
...
**Scenario CAPRMADIO-SCENARIO-SKILL-001:** A diagnosis request cannot silently authorize a fix, and prototype evidence cannot enter production without an accepted Decision/design and normal implementation proof.

## CAPRMADIO-REQUIREMENT-SKILL-002 — DSET 0.3 skills are thin wrappers
...
**Scenario CAPRMADIO-SCENARIO-SKILL-002:** Static inspection finds a workflow ID and resolver handoff in a skill but no copied checklist, concrete threshold, embedded fallback procedure, or substantive normative rule.

## CAPRMADIO-REQUIREMENT-SKILL-003 — DSET 0.3 wrappers follow local changes
...
only below that repository's `.caprmadio` control root. Product source and other
...
`.caprmadio`, but that location is neither authority nor persisted reference data.

**Scenario CAPRMADIO-SCENARIO-SKILL-003:** Two adopters use byte-identical wrappers
...
**Scenario CAPRMADIO-SCENARIO-SKILL-005:** Byte-identical `dset` wrappers route two repositories differently because their project-owned rules and current artifacts differ, while both report the resolved local identities before acting.

## CAPRMADIO-REQUIREMENT-SKILL-006 — Skill runs record bounded local evidence

The runtime adapter must follow `CAPRMADIO-RULE-SKILL-RUNS` and the versioned skill-run schema. It atomically creates one immutable record per invocation, finalizes terminal status without editing another run, preserves interrupted temporary records as diagnosable evidence, enforces finite age/count/byte retention, and uses only allowlisted bounded/redacted fields. Before initialization, in read-only state, or when persistence fails, it emits the same record to the caller with `persistence: unavailable`; a risk/profile rule may require durable logging and stop a consequential action. Run records remain advisory and cannot replace accepted specs, active changes, Git/hosted state, or promoted proof.

**Scenario CAPRMADIO-SCENARIO-SKILL-006:** A local log suggests verification is current, but Git shows later code changes; `dset` treats Git/change state as authoritative and recommends refreshed proof.

## CAPRMADIO-REQUIREMENT-SKILL-007 — Delegation inherits the main session by default
...
**Scenario CAPRMADIO-SCENARIO-SKILL-009:** An operator invokes `dset`, which chains clarification and planning before the host compacts context. The next invocation reloads the same checkpoint, detects a newer Git change, invalidates the stale next-action hint, and recommends fresh verification without asking the operator to reconstruct the session or invoke a sixth skill.

## CAPRMADIO-REQUIREMENT-SKILL-015 — Every operator input becomes atomic intake

Every operator input to a CAPRMADIO-governed project must pass through `decisions`
...
**Scenario CAPRMADIO-SCENARIO-SKILL-013:** An operator supplies one requirement and
...
## CAPRMADIO-REQUIREMENT-SKILL-013 — Implementation preparation is project-selectable

A public DSET skill accepts the desired outcome. `.caprmadio/caprmadio_settings.toml`
...
**Scenario CAPRMADIO-SCENARIO-SKILL-012:** An operator using the default lazy mode
...
`CAPRMADIO-REQUIREMENT-SKILL-012` is replaced and archived by
`CAPRMADIO-REQUIREMENT-SKILL-013`; both it and its earlier predecessor remain
immutable history.

## CAPRMADIO-REQUIREMENT-SKILL-011 — Skills use one deterministic shared runtime
...
identities exclusively within the selected `.caprmadio`, and start or resume bounded
...
**Scenario CAPRMADIO-SCENARIO-SKILL-011:** The same installed `ca-implement`
...
## CAPRMADIO-REQUIREMENT-SKILL-016 — Configure governance surfaces wrapper

Atomic source: `CAPRMADIO-REQUIREMENT-SKILL-016`.
