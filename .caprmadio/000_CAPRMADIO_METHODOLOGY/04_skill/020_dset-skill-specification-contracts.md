...
All governed target skills are thin wrappers over the repository governance registry. The registered `CAPRMADIO-RULE-LIFECYCLE`, `CAPRMADIO-RULE-SKILL-RUNS`, `CAPRMADIO-RULE-RELEASE`, `CAPRMADIO-RULE-DELEGATION-BUDGET`, and `CAPRMADIO-RULE-WORK-ITEMS` documents own substantive behavior. Versioned bounded local run records under `.caprmadio_runtime/runs/` are operational evidence only and are excluded from committed project truth.
...
are Problem subtypes; Test Case and Evaluation Case are QA subtypes. Type follows meaning,
...
### CAPRMADIO-CONTRACT-SKILL-001 — Released skills are host-native
...
### CAPRMADIO-CONTRACT-SKILL-002 — Skills resolve target-project governance
