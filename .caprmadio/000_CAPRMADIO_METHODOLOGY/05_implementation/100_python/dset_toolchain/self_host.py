...
def _released_contract(root: Path) -> dict[str, Any]:
    ...
    if not version_path.is_file():
        raise DsetCommandError(
            "CAPRMADIO-E140", version_path, "self-host version contract is missing"
        )
...
def _customize_adopter(root: Path, adopter: Path, resolved: dict[str, Any]) -> Path:
    ...
    rule = next(
        item for item in resolved["rules"] if item["id"] == "CAPRMADIO-RULE-DOMAIN-SPEC"
    )
    ...
    if "CAPRMADIO-E139" not in {item.code for item in validate_governance(adopter)}:
        raise DsetCommandError(
            "CAPRMADIO-E141", path, "local customization was not detected"
        )
    ...
    with tarfile.open(fileobj=io.BytesIO(result.stdout), mode="r:") as archive:
        for member in archive.getmembers():
            ...
            try:
                ...
                raise DsetCommandError(
                    "CAPRMADIO-E140", destination, "released archive escapes destination"
                ) from error
