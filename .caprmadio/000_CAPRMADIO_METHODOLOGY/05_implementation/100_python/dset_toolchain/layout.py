...
CURRENT_DSET_ROOT: Final[str] = ".caprmadio"
...
# PROJECT_DELIVERY_ROOT owns project-wide version lifecycle carriers.
PROJECT_DELIVERY_ROOT: Final[str] = "06_DELIVERY"
...
class RepositoryLayout:
    ...
    layer-owned roots below ``dset/``. Schema 1.3 uses the hidden ``.caprmadio/``
    control root. Schema 1.4 separates the portable framework below ``.caprmadio``
    ...
    inside ``.caprmadio`` while leaving the governed product outside it.
    """
    ...
    def settings_path(self) -> Path:
        ...
        if self.slim:
            return self.caprmadio_root / "caprmadio_settings.toml"
        return self.root / "caprmadio_settings.toml"
    ...
    def project_root(self) -> Path:
        """Handle root using the declared repository contract."""
        if self.separated:
            return self.caprmadio_root
        return (
            self.root / "00_project" if self.recursive else self.caprmadio_root / "project"
        )
    ...
    def versions_root(self) -> Path:
        """Handle root using the declared repository contract."""
        if self.separated:
            return self.caprmadio_root / PROJECT_DELIVERY_ROOT
        return (
            self.root / "10_versions" if self.recursive else self.caprmadio_root / "versions"
        )
    ...
    def scopes_root(self) -> Path:
        ...
        if self.recursive:
            return self.root
        return self.caprmadio_root if self.slim else self.caprmadio_root / "scopes"
    ...
    def framework_layer_root(self, layer: str) -> Path:
        ...
        if self.recursive:
            return self.caprmadio_root / RECURSIVE_LAYER_DIRECTORIES[normalized]
    ...
    def framework_project_root(self) -> Path:
        ...
        if self.separated:
            return self.caprmadio_root / METHODOLOGY_ROOT / "00_project"
        if self.recursive:
            return self.caprmadio_root / "00_project"
    ...
    def manifest_path(self) -> Path:
        ...
        return self.structured_file(self.caprmadio_root, "dset.toml")
    ...
    def artifact_type_registry_path(self) -> Path:
        ...
        if self.schema_version == SEPARATED_SCHEMA_VERSION:
            return self.caprmadio_root / "artifact_catalog.toml"
        if self.recursive or self.separated:
            return self.settings_path
        if self.slim:
    ...
    def traceability_path(self) -> Path:
        """Handle path using the declared repository contract."""
        if self.recursive or self.separated:
            return self.root / ".caprmadio_runtime/generated/traceability.toml"
        if self.slim:
        ...
        return self.structured_file(
            self.caprmadio_root, self._preferred_name("traceability")
        )
    ...
    def migrations_root(self) -> Path:
        """Handle root using the declared repository contract."""
        if self.separated:
            return self.root / ".caprmadio_runtime" / "migrations"
        if self.recursive:
    ...
    def history_root(self) -> Path:
        """Handle root using the declared repository contract."""
        if self.separated:
            return self.layer_root("ops") / "08_OPS" / "history"
        if self.recursive:
    ...
    def supportability_root(self) -> Path:
        """Handle root using the declared repository contract."""
        if self.separated:
            return self.layer_root("ops") / "08_OPS" / "supportability"
        base = (
    ...
    def schema_roots(self) -> tuple[Path, ...]:
        ...
        return (self.caprmadio_root / "schemas",)
    ...
    def package_roots(self) -> tuple[Path, ...]:
        ...
        return (self.caprmadio_root / "specs" / "packages",)
    ...
    def resolve_dset_path(self, relative: str | Path) -> Path:
        ...
        return self.caprmadio_root / _canonical_relative(relative)

    def _owned_file(self, layer: str, name: str) -> Path:
        base = self.layer_root(layer) if self.layered else self.caprmadio_root
...
def discover_layout(root: Path) -> RepositoryLayout:
    ...
    slim = current_root / "caprmadio_settings.toml"
    ...
    if is_slim:
        if structure_layout == SEPARATED_METHODOLOGY_LAYOUT:
            applied_directories = tuple(APPLIED_LAYER_DIRECTORIES.values())
            missing_layers = [
                path
                for path in (
        ...
        if legacy_root.exists():
            raise ValueError(
                "layout conflict: schema 1.3 .caprmadio root and legacy dset root "
...
def has_manifest(root: Path) -> bool:
    ...
    root = root.resolve()
    return (
        (root / CURRENT_DSET_ROOT / "caprmadio_settings.toml").is_file()
...
def _registered_snapshot_after_cutover(snapshot: Path) -> bool:
    ...
    for root in (resolved.parent, *resolved.parents):
        ...
        if registry is None:
            continue
        cutover = (
            (root / ".caprmadio/caprmadio_settings.toml").is_file()
            or (root / "dset/caprmadio_settings.toml").is_file()
