...
def assess_artifact_candidate(
    ...
    for field in fields:
        if not _has_explicit_value(candidate, field):
            diagnostics.append(
                {
                    "code": "CAPRMADIO-ARTIFACT-MISSING",
                    "field": field,
    ...
    if promotion["status"] == "invalid":
        diagnostics.append(
            {
                "code": "CAPRMADIO-ARTIFACT-PROMOTION",
        ...
        diagnostics.append(
            {
                "code": "CAPRMADIO-ARTIFACT-PROMOTION-AUTHORITY",
        ...
        diagnostics.append(
            {
                "code": "CAPRMADIO-ARTIFACT-PROMOTE-FIRST",
                "field": "scope_path",
...
def _value_diagnostics(
    ...
    if isinstance(authority, str) and not AUTHORITY_PATTERN.fullmatch(authority):
        diagnostics.append(
            {
                "code": "CAPRMADIO-ARTIFACT-AUTHORITY",
                "field": "authority",
                "message": (
    ...
    if _is_string_sequence(sessions):
        ...
        if len(normalized) != len(set(normalized)) or any(
            ...
            diagnostics.append(
                {
                    "code": "CAPRMADIO-ARTIFACT-SESSION",
    ...
    if isinstance(priority, str) and priority not in {*priorities, "unknown"}:
        diagnostics.append(
            {
                "code": "CAPRMADIO-ARTIFACT-PRIORITY",
                "field": "priority",
    ...
    if isinstance(acceptance, str) and acceptance not in {"proposed", "accepted"}:
        diagnostics.append(
            {
                "code": "CAPRMADIO-ARTIFACT-ACCEPTANCE",
                "field": "acceptance",
    ...
    if isinstance(conflict_state, str) and conflict_state not in {
        ...
        diagnostics.append(
            {
                "code": "CAPRMADIO-ARTIFACT-CONFLICT",
...
def _unknown_diagnostics(
    ...
    for index, item in enumerate(raw_unknowns):
        ...
        diagnostics.append(
            {
                "code": "CAPRMADIO-ARTIFACT-AMBIGUOUS",
                "field": field,
...
def _repository_scope_model(
    ...
    try:
        ...
        return None, [
            {
                "code": "CAPRMADIO-ARTIFACT-REPOSITORY",
                "field": "scope_path",
