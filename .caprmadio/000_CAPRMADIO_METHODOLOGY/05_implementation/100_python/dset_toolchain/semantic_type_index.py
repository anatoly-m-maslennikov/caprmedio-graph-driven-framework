...
def _collect_legacy_decision(
    ...
    if not (
        ...
        or (
            ...
            and path.name.startswith("CAPRMADIO-DECISION-")
        )
    ):
        return
...
def _collect_intake_item(
    ...
    if classification is None:
        diagnostics.append(
            Diagnostic(
                "CAPRMADIO-E166", path, f"intake semantic type is not recognized: {raw_type}"
            )
        )
        return
    _register(
...
def _register(
    ...
    *,
    ...
    if not _valid_classification(classification):
        diagnostics.append(
            Diagnostic("CAPRMADIO-E166", path, f"invalid flat classification: {identifier}")
        )
        return
...
def _ignored(root: Path, path: Path) -> bool:
    ...
    return (
        relative.parts[:1] == (".caprmadio_runtime",)
...
def _safe_load(path: Path, diagnostics: list[Diagnostic]) -> Any:
    ...
    try:
        ...
        diagnostics.append(
            Diagnostic("CAPRMADIO-E166", path, f"cannot classify carrier: {error}")
        )
        return None
