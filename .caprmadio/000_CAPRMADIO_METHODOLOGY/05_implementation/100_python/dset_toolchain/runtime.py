...
def start_run(
    ...
    *,
    ...
    next_reason_code: str = "CAPRMADIO-RUNTIME-START",
    ...
    _require(
        mode_id is None or public_entrypoint == "caprmadio" or mode_id == expected_mode,
    ...
    checkpoint_relative = (
        f".caprmadio_runtime/sessions/{selected_session_id}.json"
        if storage is not None
        else None
    )
    started_at = _utc_now()
...
def resume_checkpoint(
    ...
    *,
    ...
    This function is read-only and never creates ``.caprmadio``.
    """
...
def _storage_for_root(root: Path | None) -> _Storage | None:
    ...
    if not _is_caprmadio_root(root):
        return None
    assert root is not None
    resolved = root.resolve()
    runs = resolved / ".caprmadio_runtime" / "runs"
    sessions = resolved / ".caprmadio_runtime" / "sessions"
    try:
...
def _is_caprmadio_root(root: Path | None) -> bool:
    ...
    if data["schema_version"] in {"1.1", "1.2"}:
        ...
        if previous_version == "1.1":
            data["closure"] = initial_closure(
                str(data.get("public_entrypoint", "caprmadio")),
