...
def validate_governance(
    ...
    if not registry_path.is_file():
        return [_diag("CAPRMADIO-E130", registry_path, "governance registry is missing")]
    try:
        data = project_section(root, "governance_registry")
    except (OSError, ValueError, StructuredDataError) as error:
        return [_diag("CAPRMADIO-E131", registry_path, str(error))]
    if not isinstance(data, dict):
        return [_diag("CAPRMADIO-E131", registry_path, "registry must be a mapping")]
...
def validate_governance_registry(
    ...
    if not isinstance(profile, dict):
        return [_diag("CAPRMADIO-E137", registry_path, "profile must be a mapping")]
    ...
    if not isinstance(profile_version, str) or not profile_version:
        diagnostics.append(
            _diag("CAPRMADIO-E137", registry_path, "profile version is missing")
        )
    profile_custom = profile.get("customization")
    if profile_custom not in CUSTOMIZATION:
        diagnostics.append(
            _diag("CAPRMADIO-E139", registry_path, "invalid profile customization status")
        )

    rules = data.get("rules")
    if not isinstance(rules, list) or not rules:
        return diagnostics + [
            _diag("CAPRMADIO-E131", registry_path, "rules must be a non-empty list")
        ]
    rule_items = [item for item in rules if isinstance(item, dict)]
    if len(rule_items) != len(rules):
        diagnostics.append(
            _diag("CAPRMADIO-E131", registry_path, "every rule must be a mapping")
        )
    ...
    for rule in rule_items:
        ...
        if not isinstance(rule_id, str) or not RULE_PATTERN.fullmatch(rule_id):
            diagnostics.append(
                _diag("CAPRMADIO-E131", registry_path, f"invalid rule ID: {rule_id}")
            )
            continue
        if rule_id in by_id:
            diagnostics.append(
                _diag("CAPRMADIO-E132", registry_path, f"duplicate rule owner: {rule_id}")
            )
            continue
        by_id[rule_id] = rule
        ...
        if layer not in RULE_LAYERS:
            diagnostics.append(
                _diag("CAPRMADIO-E131", registry_path, f"invalid rule layer: {rule_id}")
            )
        owner = rule.get("owner")
        if not isinstance(owner, str) or not owner.strip():
            diagnostics.append(
                _diag("CAPRMADIO-E131", registry_path, f"missing rule owner: {rule_id}")
            )
        ...
        if not local.is_file():
            diagnostics.append(
                _diag("CAPRMADIO-E133", local, f"governing document is missing: {rule_id}")
            )
            continue
        ...
        if not isinstance(source_sha, str) or not SHA256_PATTERN.fullmatch(source_sha):
            diagnostics.append(
                _diag("CAPRMADIO-E139", registry_path, f"invalid source digest: {rule_id}")
            )
            continue
        customization = rule.get("customization")
        if customization not in CUSTOMIZATION:
            diagnostics.append(
                _diag("CAPRMADIO-E139", registry_path, f"invalid customization: {rule_id}")
            )
            continue
    ...
    if profile_custom == "unmodified" and actual_custom:
        diagnostics.append(
            _diag(
                "CAPRMADIO-E139", registry_path, "custom rules require custom profile status"
            )
        )
    ...
    if not workflow_items:
        diagnostics.append(
            _diag("CAPRMADIO-E136", registry_path, "workflows must be a non-empty list")
        )
    workflow_ids: set[str] = set()
    for workflow in workflow_items:
        if not isinstance(workflow, dict):
            diagnostics.append(
                _diag("CAPRMADIO-E136", registry_path, "every workflow must be a mapping")
            )
            continue
        ...
        if (
            ...
            diagnostics.append(
                _diag("CAPRMADIO-E136", registry_path, f"invalid workflow: {workflow_id}")
            )
            continue
        ...
        if not isinstance(selected, list) or not selected:
            diagnostics.append(
                _diag(
                    "CAPRMADIO-E136", registry_path, f"workflow has no rules: {workflow_id}"
                )
            )
            continue
        seen: set[str] = set()
...
def resolve_workflow(
    ...
    if workflow is None:
        return None, [_diag("CAPRMADIO-E136", path, f"unknown workflow: {workflow_id}")]
...
def materialize_governance(
    ...
    *,
    ...
    if not manifest.is_file():
        raise DsetCommandError("CAPRMADIO-E001", manifest, "project manifest is missing")
    ...
    if profile.get("id") != profile_id:
        raise DsetCommandError(
            "CAPRMADIO-E140", profile_path, "governance profile identity mismatch"
        )
    ...
    def source_template(relative: Path) -> Path:
        ...
        try:
            ...
            raise DsetCommandError(
                "CAPRMADIO-E140", source_layout.template_roots[0] / relative, str(error)
            ) from error
    ...
    for item in rules:
        ...
        if layer not in RULE_LAYERS:
            raise DsetCommandError(
                "CAPRMADIO-E140", profile_path, f"invalid rule layer: {rule_id}/{layer}"
            )
    ...
    try:
        ...
        for item in rules:
            ...
            if item["id"] == "CAPRMADIO-RULE-RELEASE" and release_not_applicable:
            ...
            if release_not_applicable:
                dependencies = [
                    ...
                    if dependency != "CAPRMADIO-RULE-RELEASE"
                ]
        ...
        for workflow in cast(list[dict[str, Any]], profile["workflows"]):
            ...
            if release_not_applicable:
                rendered["rules"] = [
                    ...
                    if rule != "CAPRMADIO-RULE-RELEASE"
                ]
...
def diff_governance(root: Path, source_root: Path) -> str:
    ...
    for rule in cast(list[dict[str, Any]], data.get("rules", [])):
        ...
        template = _unique_named_file(source_root / ".caprmadio", source.get("template"))
        local = _rule_carrier(root, rule)
...
def _validate_dependencies(
    ...
    for rule_id, rule in by_id.items():
        ...
        if not isinstance(dependencies, list):
            diagnostics.append(
                _diag("CAPRMADIO-E135", path, f"dependencies must be a list: {rule_id}")
            )
            continue
        for dependency in dependencies:
    ...
    def visit(rule_id: str) -> None:
        ...
        if rule_id in visiting:
            diagnostics.append(
                _diag("CAPRMADIO-E135", path, f"rule dependency cycle includes: {rule_id}")
            )
            return
...
def _validate_wrappers(
    ...
    for wrapper in wrappers:
        if not isinstance(wrapper, dict):
            diagnostics.append(
                _diag("CAPRMADIO-E138", registry_path, "every wrapper must be a mapping")
            )
            continue
        ...
        if not isinstance(workflow, str) or workflow not in workflow_ids:
            diagnostics.append(
                _diag(
                    "CAPRMADIO-E138", registry_path, f"unknown wrapper workflow: {workflow}"
                )
            )
            continue
        if workflow in seen:
            diagnostics.append(
                _diag("CAPRMADIO-E138", registry_path, f"duplicate wrapper: {workflow}")
            )
        seen.add(workflow)
        ...
        if path is None or not path.is_file():
            diagnostics.append(
                _diag("CAPRMADIO-E138", registry_path, f"wrapper is missing: {workflow}")
            )
        elif not isinstance(expected, str) or _sha256(path) != expected:
            diagnostics.append(
                _diag("CAPRMADIO-E138", path, f"wrapper identity mismatch: {workflow}")
            )
    return diagnostics
...
def _unique_named_file(control_root: Path, raw: Any) -> Path | None:
    ...
    if not isinstance(raw, str):
        return None
    try:
        root = control_root.parent if control_root.name == ".caprmadio" else control_root
...
def _rule_carrier(root: Path, rule: dict[str, Any]) -> Path | None:
    ...
    if document is not None:
        return _unique_named_file(root / ".caprmadio", document)
