...
def validate_legacy_authority_ledger(root: Path) -> list[Diagnostic]:
    ...
    if not path.is_file():
        try:
            ...
            return [Diagnostic("CAPRMADIO-E167", path, str(error))]
        if expected["records"]:
            if not (root / ".git").exists():
                return []
            return [Diagnostic("CAPRMADIO-E167", path, "legacy authority ledger is missing")]
        return []
    try:
        actual = load(path)
    except (OSError, UnicodeError, StructuredDataError) as error:
        return [Diagnostic("CAPRMADIO-E167", path, f"invalid legacy ledger: {error}")]
