...
def validate_local_python_profile(root: Path) -> list[Diagnostic]:
    ...
    try:
        ...
        return [Diagnostic("CAPRMADIO-E180", profile_path, str(error))]
...
def _find_profile(root: Path) -> Path:
    ...
    source = [path for path in matches if ".caprmadio" not in path.parts]
    if len(source) == 1:
        return source[0]
    if len(matches) == 1:
        return matches[0]
...
def _validate_file(path: Path, limits: PythonProfileLimits) -> list[Diagnostic]:
    ...
    try:
        ...
        return [
            _diagnostic("CAPRMADIO-E180", path, f"Python carrier cannot be read: {error}")
        ]
...
def _module_diagnostics(
    ...
    if ast.get_docstring(tree, clean=False) is None:
        diagnostics.append(
            _diagnostic("CAPRMADIO-E181", path, "module docstring is missing")
        )
    ...
    if "\t" in source:
        diagnostics.append(
            _diagnostic("CAPRMADIO-E180", path, "tab indentation is prohibited")
        )
    return diagnostics
...
def _definition_diagnostics(
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ClassDef) and ast.get_docstring(node) is None:
            diagnostics.append(
                _at("CAPRMADIO-E184", path, node, "class docstring is missing")
            )
...
def _function_contract_diagnostics(
    ...
    if _requires_docstring(path, node, measured) and ast.get_docstring(node) is None:
        diagnostics.append(
            _at("CAPRMADIO-E184", path, node, "function docstring is missing")
        )
    ...
    if missing or node.returns is None:
        ...
        diagnostics.append(
            _at("CAPRMADIO-E185", path, node, f"annotations missing: {details}")
        )
    return diagnostics
    ...
    for node in tree.body:
        ...
        if names:
            ...
            if not previous.startswith("#"):
                diagnostics.append(
                    _at("CAPRMADIO-E186", path, node, "constant lacks documentation")
                )
