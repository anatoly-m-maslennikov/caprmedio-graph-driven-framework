...
def health_path(root: Path) -> Path:
    ...
    generated = (
        ...
        if layout.layered
        else layout.caprmadio_root / "generated"
    )
...
def _relative_ignored(relative: Path) -> bool:
    ...
    return any(
        ...
        or (part.startswith(".") and part not in {".github", ".caprmadio"})
...
def _in_project_scope(root: Path, relative: Path) -> bool:
    ...
    if len(relative.parts) == 1:
        return True
    layout = discover_layout(root)
    if relative.is_relative_to(layout.caprmadio_root.relative_to(root)):
        return True
    manifest = load(layout.manifest_path)
