#!/usr/bin/env python3
"""Rename public DSET skill IDs to CAPRMADIO's ``caprmadio``/``ca-*`` scheme.
...
PRIMARY_CONTEXT_REPLACEMENTS = (
    (r"(?<![A-Za-z0-9-])\$dset(?![A-Za-z0-9-])", "$caprmadio"),
    (r"(?<![A-Za-z0-9-])--skill dset(?![A-Za-z0-9-])", "--skill caprmadio"),
    (r"(?m)^name: dset$", "name: caprmadio"),
    (r"skills/dset(?=/|\b)", "skills/caprmadio"),
    (r"\[skills\.dset\]", "[skills.caprmadio]"),
    (r'"dset": "lifecycle-orchestration"', '"caprmadio": "lifecycle-orchestration"'),
    (r'"dset": None', '"caprmadio": None'),
    (r'public_entrypoint == "dset"', 'public_entrypoint == "caprmadio"'),
    (r'public_entrypoint="dset"', 'public_entrypoint="caprmadio"'),
    (r'skill_id="dset"', 'skill_id="caprmadio"'),
    (r'entrypoint != "dset"', 'entrypoint != "caprmadio"'),
    (
        r'data\.get\("public_entrypoint", "dset"\)',
        'data.get("public_entrypoint", "caprmadio")',
    ),
    (r'"lifecycle-orchestration": "dset"', '"lifecycle-orchestration": "caprmadio"'),
    (r'skill = "dset"', 'skill = "caprmadio"'),
    (r'name == "dset"', 'name == "caprmadio"'),
    (r'_skill_text\("dset"\)', '_skill_text("caprmadio")'),
    (r'/ "dset" / "SKILL\.md"', '/ "caprmadio" / "SKILL.md"'),
    (r'\["dset", "dset-', '["caprmadio", "dset-'),
    (r'enum = \["dset",', 'enum = ["caprmadio",'),
)
...
def is_current_surface(relative: Path) -> bool:
    ...
    if relative == SELF_PATH or relative.parts[:1] == ("90_legacy",):
        return False
    if relative.parts[:1] == (".caprmadio",):
        return relative == Path(".caprmadio/caprmadio_settings.toml")
...
def migrated_content(
    ...
    if relative.parts[:1] == ("skills",):
        migrated = migrated.replace("DSET", "CAPRMADIO")
        if relative == Path("skills/README.md"):
            migrated = re.sub(r"(?<![A-Za-z0-9-])`dset`", "`caprmadio`", migrated)
            migrated = migrated.replace("(dset/SKILL.md)", "(caprmadio/SKILL.md)")
        if relative == Path("skills/host-distribution.md"):
            migrated = migrated.replace(
                "exact 19-skill catalog: `dset`,",
                "exact 19-skill catalog: `caprmadio`,",
            )
...
def render_summary(plan: MigrationPlan) -> str:
    ...
    lines = [
        "CAPRMADIO public-skill migration plan",
        f"root: {plan.root}",
