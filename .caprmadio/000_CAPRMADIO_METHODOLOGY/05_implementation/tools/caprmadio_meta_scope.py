...
Usage:
    python 15_layer_implementation/tools/caprmadio_meta_scope.py migrate
    python 15_layer_implementation/tools/caprmadio_meta_scope.py migrate \
        --apply --expect-plan-digest SHA256
    python 15_layer_implementation/tools/caprmadio_meta_scope.py project
    python 15_layer_implementation/tools/caprmadio_meta_scope.py project --write
    python 15_layer_implementation/tools/caprmadio_meta_scope.py project --check
...
``.caprmadio_runtime/caprmadio-meta-scope`` folder, verify complete bytes before
...
META_RELATIVE = Path(".caprmadio/101_layer_meta")
CATALOG_RELATIVE = META_RELATIVE / (
    "CAPRMADIO-META-CATL-001-requirement--active-atoms-by-subject-scope.md"
)
RUNTIME_RELATIVE = Path(".caprmadio_runtime/caprmadio-meta-scope")
...
SCOPE_OVERRIDES = {
    "CAPRMADIO-ANALYSIS-META-001": "lifecycle-traceability",
    "CAPRMADIO-CONTRACT-META-001": "framework-boundary",
    "CAPRMADIO-EVIDENCE-RECORD-034": "framework-boundary",
    "CAPRMADIO-REQUIREMENT-META-004": "assurance",
    "CAPRMADIO-REQUIREMENT-META-011": "framework-boundary",
    "CAPRMADIO-REQUIREMENT-META-053": "lifecycle-traceability",
    "CAPRMADIO-REQUIREMENT-META-075": "lifecycle-traceability",
    "CAPRMADIO-REQUIREMENT-META-077": "lifecycle-traceability",
    "CAPRMADIO-REQUIREMENT-META-090": "authority",
    "CAPRMADIO-REQUIREMENT-META-092": "assurance",
    "CAPRMADIO-REQUIREMENT-META-113": "lifecycle-traceability",
    "CAPRMADIO-REQUIREMENT-META-114": "principles",
    "CAPRMADIO-REQUIREMENT-META-115": "principles",
    "CAPRMADIO-REQUIREMENT-META-116": "principles",
    "CAPRMADIO-REQUIREMENT-META-117": "principles",
    "CAPRMADIO-REQUIREMENT-META-118": "principles",
}
CATALOG_ID = "CAPRMADIO-META-CATL-001"
GENERATOR_ID = "caprmadio-meta-scope"
GENERATOR_VERSION = 2
...
def _catalog_bytes(atoms: tuple[Atom, ...]) -> bytes:
    ...
    metadata = {
        ...
        "relations": [
            {
                "type": "child_of",
                "targets": ["CAPRMADIO-REQUIREMENT-META-127"],
            }
        ],
    }
...
def main() -> int:
    ...
    try:
        ...
        if args.debug:
            raise
        print(f"caprmadio-meta-scope: {error}", file=sys.stderr)
        return 1
