...
class TraceabilityTests(unittest.TestCase):
    ...
    def test_real_history_maps_archives_and_active_change(self) -> None:
        ...
        legacy = [
            item for item in relations if item["source"] == "CAPRMADIO-REQUIREMENT-GOV-034"
        ]
        self.assertEqual(legacy[0]["type"], "child_of")
        self.assertEqual(legacy[0]["target"], "CAPRMADIO-REQUIREMENT-GOV-033")
        ...
        self.assertTrue(
            any(
                item["target"] == "CAPRMADIO-REQUIREMENT-GOV-034"
        ...
        decision_relations = [
            ...
            if item["source"] == "CAPRMADIO-DECISION-GOV-013"
        ]
        self.assertIn(("resolution_of", "CAPRMADIO-QUESTION-GOV-004"), decision_relations)
        self.assertIn(
            ("replacement_of", "CAPRMADIO-REQUIREMENT-GOV-034"),
            decision_relations,
        )
        projection_ranges = [
            item["range"]
            for item in relations
            if item["source"] == "CAPRMADIO-SPECIFICATION-001"
        ...
        self.assertEqual(
            ...
            {
                "CAPRMADIO-ATOMIC-RECORD-113",
                "CAPRMADIO-ATOMIC-RECORD-114",
                "CAPRMADIO-ATOMIC-RECORD-115",
                "CAPRMADIO-ATOMIC-RECORD-078",
                "CAPRMADIO-ATOMIC-RECORD-079",
                "CAPRMADIO-ATOMIC-RECORD-080",
            },
        )
        self.assertEqual(
            {
        ...
        self.assertEqual(atoms["CAPRMADIO-DECISION-GOV-010"]["type"], "decision")
        self.assertEqual(atoms["CAPRMADIO-DECISION-GOV-010"]["priority"], "high")
        self.assertEqual(atoms["CAPRMADIO-DECISION-GOV-010"]["current_status"], "accepted")
        self.assertFalse(atoms["CAPRMADIO-DECISION-GOV-010"]["archived"])
        typed = atoms["CAPRMADIO-DECISION-GOV-013"]["relations"]
        self.assertTrue(any(item["type"] == "resolution_of" for item in typed))
        classifications = {
            item["id"]: item for item in trace["semantic_classifications"]
        }
        self.assertEqual(
            classifications["CAPRMADIO-OPPORTUNITY-GOV-001"]["type"], "question"
        )
        self.assertEqual(
            classifications["CAPRMADIO-OPPORTUNITY-GOV-001"]["subtype"],
            "opportunity",
        )
        self.assertTrue(
            classifications["CAPRMADIO-OPPORTUNITY-GOV-001"]["historical_carrier"]
        )
