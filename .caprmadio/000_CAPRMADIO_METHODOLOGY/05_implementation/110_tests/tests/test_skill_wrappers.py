...
class SkillWrapperTests(unittest.TestCase):
    ...
    def test_governed_wrappers_fail_closed_after_shared_context(self) -> None:
        ...
        for name, workflow in PUBLIC_SKILL_WORKFLOWS.items():
            if name in PRE_RESOLUTION_SKILLS or name == "caprmadio":
                continue
            with self.subTest(workflow=workflow):
                self.assertIn("CAPRMADIO-RULE-LIFECYCLE", workflows[workflow])
    ...
    def test_primary_and_direct_stop_boundaries_are_distinct(self) -> None:
        primary = self._skill_text("caprmadio")
    ...
    def test_implementation_uses_governed_entry_criteria_closure(self) -> None:
        lifecycle = (
            ROOT / ".caprmadio/04_layer_skill/procedure-lifecycle-orchestration.md"
        ...
        skill_runs = (ROOT / ".caprmadio/04_layer_skill/procedure-skill-runs.md").read_text(
            encoding="utf-8"
        )
        skill_runs_template = (
            ROOT / ".caprmadio/04_layer_skill/templates/governance/core-v1/skill-runs.md"
        ).read_text(encoding="utf-8")
