...
class PriorityConflictTests(unittest.TestCase):
    ...
    def test_source_projection_relation_is_derived_before_priority(self) -> None:
        ...
        projection = self._write_fact(
            root,
            "CAPRMADIO-SPECIFICATION-099",
            artifact_type="specification",
            priority="high",
            scope={"kind": "project", "id": "dset-temporary-adopter"},
            relations=[{"type": "projection_of", "target": "CAPRMADIO-DECISION-001"}],
        )
        candidate = self._atomic_candidate(root)
        candidate["right"] = {
            "id": "CAPRMADIO-SPECIFICATION-099",
            "source": projection,
        }
        ...
        self.assertEqual(result["selected_id"], "CAPRMADIO-DECISION-001")
    ...
    def test_first_class_conflict_is_assessed_emitted_and_sealed(self) -> None:
        ...
        output = root / "dset/changes/CAPRMADIO-ATOMIC-RECORD-003-conflict.md"
    ...
    def test_replacement_is_derived_from_relation_and_archive(self) -> None:
        ...
        self.assertEqual(replacement["selected_id"], "CAPRMADIO-CONTRACT-002")
        self.assertEqual(
    ...
    def _atomic_candidate(cls, root: Path) -> dict[str, Any]:
        """Handle candidate using the declared repository contract."""
        return {
            "left": {"id": "CAPRMADIO-DECISION-001"},
            "right": {"id": "CAPRMADIO-CONTRACT-002"},
    ...
    def _emission_candidate(cls, root: Path) -> dict[str, Any]:
        ...
        candidate["conflict_atom"] = {
            "artifact_id": "CAPRMADIO-ATOMIC-RECORD-003",
            "semantic_id": "CAPRMADIO-CONFLICT-003",
            ...
            "material_links": ["CAPRMADIO-DECISION-001", "CAPRMADIO-CONTRACT-002"],
            "promotion": {"parent_scope": None},
            "acceptance": "proposed",
            "priority": "high",
            "lineage": ["CAPRMADIO-DECISION-001", "CAPRMADIO-CONTRACT-002"],
    ...
    def _seed_parties(root: Path, *, replacement: bool = False) -> None:
        ...
        for carrier, semantic, subtype, priority in (
            ("001", "CAPRMADIO-DECISION-001", None, "medium"),
            ("002", "CAPRMADIO-CONTRACT-002", "contract", "high"),
        ):
            path = root / f"dset/changes/CAPRMADIO-ATOMIC-RECORD-{carrier}-party.md"
            subtype_line = f"subtype: {subtype}\n" if subtype else ""
            relation = (
                "relations:\n  - type: replacement_of\n    target: CAPRMADIO-DECISION-001\n"
                if replacement and semantic == "CAPRMADIO-CONTRACT-002"
                else ""
            )
            path.write_text(
                "---\n"
                "artifact_type: atomic_record\n"
                f"artifact_id: CAPRMADIO-ATOMIC-RECORD-{carrier}\n"
                "type: decision\n"
                f"{subtype_line}"
