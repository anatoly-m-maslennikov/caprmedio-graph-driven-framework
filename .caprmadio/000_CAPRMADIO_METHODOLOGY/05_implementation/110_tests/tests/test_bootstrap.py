...
class BootstrapTests(unittest.TestCase):
    ...
    def test_stage_places_implementation_hub_at_flat_layer_root(self) -> None:
        with (
            ...
            implementation = stage / ".caprmadio/105_layer_implementation"
    ...
    def test_execute_initializes_and_validates_nonempty_repository(self) -> None:
        with temporary_directory() as raw:
            ...
            self.assertTrue((target / "skills" / "caprmadio" / "SKILL.md").is_file())
            ...
            self.assertTrue((target / ".caprmadio/caprmadio_settings.toml").is_file())
            self.assertTrue(
                (target / ".caprmadio/03_REQUIREMENT/APP-PROJECT-HUB.md").is_file()
            )
            self.assertTrue(
                (target / ".caprmadio/06_DELIVERY/APP-VERSIONS-HUB.md").is_file()
            )
            self.assertTrue(
                (
                    target
                    / ".caprmadio/105_layer_implementation/APP-IMPL-HUB.md"
                ).is_file()
            )
            self.assertFalse(
                (
                    target
                    / ".caprmadio/105_layer_implementation/03_REQUIREMENT"
                ).exists()
            )
            self.assertEqual(
                (target / ".caprmadio_runtime/.gitignore").read_text(encoding="utf-8"),
                "*\n!.gitignore\n",
            )
            self.assertFalse((target / ".caprmadio/runtime").exists())
            self.assertFalse((target / "caprmadio_settings.toml").exists())
    ...
    def test_execute_detects_hosted_automation_supportability(self) -> None:
        with temporary_directory() as raw:
            ...
            runbook = (
                target
                / ".caprmadio/106_layer_ops/08_OPS/supportability"
                / "APP-OPS-SUPPORTABILITY.md"
    ...
    def test_existing_destination_stops_without_partial_write(self) -> None:
        with temporary_directory() as raw:
            target = (Path(raw) / "existing").resolve()
            manifest = target / ".caprmadio" / "caprmadio_settings.toml"
