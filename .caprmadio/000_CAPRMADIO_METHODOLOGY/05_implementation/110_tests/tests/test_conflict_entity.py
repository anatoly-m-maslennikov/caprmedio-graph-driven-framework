...
GOV_PACKAGE = ROOT / ".caprmadio" / "02_layer_gov"
...
class ConflictEntityTests(unittest.TestCase):
    ...
    def test_accepted_truth_defines_four_types_and_flat_subtypes(self) -> None:
        ...
        self.assertIn("CAPRMADIO-INVARIANT-GOV-017", domain)
        self.assertIn("CAPRMADIO-REQUIREMENT-GOV-027", spec)
    ...
    def test_conflict_proof_ids_are_registered(self) -> None:
        ...
        self.assertIn("CAPRMADIO-REQUIREMENT-GOV-027", package["requirements"])
        self.assertIn("CAPRMADIO-TEST-CASE-GOV-027", package["tests"])
        self.assertIn("CAPRMADIO-EVALUATION-CASE-GOV-017", package["evals"])

    def test_absorbing_decision_preserves_immutable_predecessor(self) -> None:
        predecessor = DECISIONS / (
            "CAPRMADIO-DECISION-GOV-004-problems-questions-and-conflicts-are-distinct.md"
        )
        successor = DECISIONS / (
            "CAPRMADIO-DECISION-GOV-005-semantic-artifact-types-and-open-conflicts.md"
        )
        self.assertTrue(predecessor.is_file())
        text = successor.read_text(encoding="utf-8")
        self.assertIn("**Absorbs:** `CAPRMADIO-DECISION-GOV-004` in full", text)
        self.assertIn("workflow never defines artifact type", text)

        fpf_predecessor = DECISIONS / (
            "CAPRMADIO-DECISION-GOV-007-flat-canonical-type-and-subtype-model.md"
        )
        fpf_successor = DECISIONS / (
            "CAPRMADIO-DECISION-GOV-008-fpf-aligned-boundaries-for-the-flat-type-model.md"
        )
        self.assertTrue(fpf_predecessor.is_file())
        fpf_text = fpf_successor.read_text(encoding="utf-8")
        self.assertIn("**Absorbs:** `CAPRMADIO-DECISION-GOV-007` in full", fpf_text)
        self.assertIn("One atom, one primary governed claim", fpf_text)

    def test_live_and_template_work_item_rules_match(self) -> None:
        live = ROOT / ".caprmadio" / "02_layer_gov" / "specification-work-items.md"
        template = (
            ROOT
            / ".caprmadio"
            / "02_layer_gov"
