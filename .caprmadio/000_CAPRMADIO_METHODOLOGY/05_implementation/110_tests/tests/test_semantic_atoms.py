...
class SemanticAtomTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.atom_path = (
            self.root / "dset/changes/CAPRMADIO-ATOMIC-RECORD-001-output-contract.md"
        )
    ...
    def test_legacy_ledger_keeps_historical_yaml_identity_after_toml_cutover(
        ...
        with temporary_directory() as raw:
            ...
            historical.write_text(
                "contracts:\n  - CAPRMADIO-CONTRACT-GOV-001\n",
                encoding="utf-8",
            )
            (package_root / "package.toml").write_text(
                "contracts = []\n", encoding="utf-8"
            )
            selector = "contracts:CAPRMADIO-CONTRACT-GOV-001"
            digest = hashlib.sha256(f"{selector}\n".encode()).hexdigest()
            (governance / "legacy-authority.yaml").write_text(
                'schema_version: "1.0"\n'
                "records:\n"
                "  -\n"
                "    semantic_id: CAPRMADIO-CONTRACT-GOV-001\n"
                "    type: decision\n"
                "    subtype: contract\n"
                "    fragments:\n"
                "      -\n"
            ...
            self.assertIn("CAPRMADIO-CONTRACT-GOV-001", legacy_authority_ids(root))
            rows = {
                str(item["id"]): item
                for item in build_semantic_classification_index(root)
            }
            self.assertEqual(rows["CAPRMADIO-CONTRACT-GOV-001"]["subtype"], "contract")
            self.assertTrue(rows["CAPRMADIO-CONTRACT-GOV-001"]["historical_carrier"])
    ...
    def test_four_type_atom_is_sealed_and_later_mutation_fails(self) -> None:
        ...
        self.assertEqual(atoms["CAPRMADIO-DECISION-001"].semantic_type, "decision")
        self.assertEqual(atoms["CAPRMADIO-DECISION-001"].subtype, "contract")
        ...
        messages = [item.message for item in validate_semantic_atoms(self.root)]
        self.assertIn(
            "sealed atom sha256 changed: CAPRMADIO-DECISION-001",
            messages,
        )
    ...
    def test_archive_moves_atom_byte_for_byte_and_updates_lookup(self) -> None:
        ...
        destination = archive_atom(self.root, "CAPRMADIO-DECISION-001")
    ...
    def test_archive_rejects_active_structural_dependants(self) -> None:
        self._write_atom()
        seal_atom(self.root, self.atom_path)
        second = self.root / "dset/changes/CAPRMADIO-ATOMIC-RECORD-002-format.md"
        second.write_text(
            self._atom_text(
                carrier="CAPRMADIO-ATOMIC-RECORD-002",
                semantic="CAPRMADIO-CONTRACT-002",
            ).replace(
                "llm_session_ids:\n",
                "relations:\n"
                "  - type: child_of\n"
        ...
        with self.assertRaisesRegex(ValueError, "active child reliance"):
            archive_atom(self.root, "CAPRMADIO-DECISION-001")

    def _write_atom(self) -> str:
        """Write atom using the declared repository contract."""
        text = self._atom_text(
            carrier="CAPRMADIO-ATOMIC-RECORD-001",
            semantic="CAPRMADIO-DECISION-001",
        )
        self.atom_path.write_text(text, encoding="utf-8")
        return text
