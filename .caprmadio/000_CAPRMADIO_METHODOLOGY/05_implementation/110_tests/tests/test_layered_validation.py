...
class LayeredValidationTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self._write_fragment("meta", "CAPRMADIO-REQUIREMENT-001", "CAPRMADIO-TEST-CASE-001")
        self._write_fragment("tool", "CAPRMADIO-REQUIREMENT-TOOL-001", "CAPRMADIO-TEST-CASE-TOOL-001")
    ...
    def test_change_ids_may_be_owned_by_native_atomic_records(self) -> None:
        ...
        native_ids = {
            "requirements": "CAPRMADIO-REQUIREMENT-TOOL-100",
            "tests": "CAPRMADIO-TEST-CASE-TOOL-100",
            "evals": "CAPRMADIO-EVALUATION-CASE-TOOL-100",
        }
        for sequence, (group, identifier) in enumerate(native_ids.items(), start=100):
            values = manifest[group]
            assert isinstance(values, list)
            values.append(identifier)
            carrier = change / f"CAPRMADIO-ATOMIC-RECORD-{sequence}.md"
    ...
    def test_work_area_registry_requires_unique_safe_existing_directories(self) -> None:
        ...
        for work_areas in invalid_cases:
            with self.subTest(work_areas=work_areas):
                ...
                self.assertIn(
                    "CAPRMADIO-E143", {item.code for item in validate_repository(self.root)}
                )
    ...
    def test_intake_derives_layers_and_requires_stable_owner_change(self) -> None:
        ...
        valid: dict[str, Any] = {
            ...
            "owner_change": "CAPRMADIO-CHANGE-GOV-001",
            "decision": "pending",
            "llm_session_ids": [],
            "external_refs": [],
        }
        ...
        invalid_cases = (
            ...
            {**baseline, "items": [{**valid, "id": "CAPRMADIO-PROBLEM-GOV-001"}]},
            ...
            {
                **baseline,
                "items": [{**valid, "decision": "CAPRMADIO-DECISION-TOOL-001"}],
            },
        )
    ...
    def test_current_intake_uses_flat_question_and_problem_subtypes(self) -> None:
        ...
        self.assertIn("CAPRMADIO-E142", codes)
        self.assertIn("CAPRMADIO-E166", codes)
    ...
    def test_atomic_artifacts_require_explicit_session_provenance(self) -> None:
        ...
        self.assertIn(
            "CAPRMADIO-E155", {item.code for item in validate_repository(self.root)}
        )
        ...
        decision = change / "decision-CAPRMADIO-DECISION-GOV-099.md"
        ...
        proof.write_text(
            ...
            'artifact_id = "CAPRMADIO-EVIDENCE-RECORD-099"\n'
    ...
    def test_fragment_ownership_and_cross_fragment_uniqueness(self) -> None:
        ...
        data = load(path)
        assert isinstance(data, dict)
        data["requirements"] = ["CAPRMADIO-REQUIREMENT-001"]
        path.write_text(dump(data, path), encoding="utf-8")
        (path.parent / "spec.md").write_text(
            "# CAPRMADIO-REQUIREMENT-001\n", encoding="utf-8"
        )

        codes = {item.code for item in validate_repository(self.root)}

        self.assertIn("CAPRMADIO-E146", codes)
        self.assertIn("CAPRMADIO-E147", codes)

    def test_missing_and_misidentified_fragments_are_distinct(self) -> None:
        self._fragment_path("tool").unlink()
        self.assertIn(
            "CAPRMADIO-E144", {item.code for item in validate_repository(self.root)}
        )
        self._write_fragment("tool", "CAPRMADIO-REQUIREMENT-TOOL-001", "CAPRMADIO-TEST-CASE-TOOL-001")
        path = self._fragment_path("tool")
        data = load(path)
        ...
        path.write_text(dump(data, path), encoding="utf-8")
        self.assertIn(
            "CAPRMADIO-E145", {item.code for item in validate_repository(self.root)}
        )
    ...
    def test_change_layer_and_duplicate_change_ids_are_rejected(self) -> None:
        ...
        self._write_change(
            ...
            "layered-change",
            id_layer="OPS",
            stable_id="CAPRMADIO-CHANGE-TOOL-099",
        )
        self.assertIn(
            "CAPRMADIO-E149", {item.code for item in validate_repository(self.root)}
        )

    def test_schema_1_2_shapes_are_explicit(self) -> None:
        project = json.loads(
            (ROOT / ".caprmadio/01_layer_meta/schemas/project.schema.toml").read_text(
                encoding="utf-8"
            )
        )
        fragment = json.loads(
            (
                ROOT / ".caprmadio/01_layer_meta/schemas/package-fragment.schema.toml"
        ...
        change = json.loads(
            (ROOT / ".caprmadio/02_layer_gov/schemas/change.schema.toml").read_text(
                encoding="utf-8"
            )
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            ),
            {
                ...
                ".caprmadio/05_layer_ops/procedure-release.md",
            },
        )
    ...
    def test_distributed_schema_ids_match_layer_owned_paths(self) -> None:
        ...
        for path in sorted((ROOT / ".caprmadio").glob("*/schemas/*.json")):
    ...
    def test_schema_1_2_work_area_contract_has_two_target_modes(self) -> None:
        ...
        traceability = json.loads(
            (ROOT / ".caprmadio/03_layer_tool/schemas/traceability.schema.toml").read_text(
                encoding="utf-8"
            )
        )
    ...
    def test_native_atomic_record_can_own_a_change_decision(self) -> None:
        ...
        manifest = load(manifest_path)
        assert isinstance(manifest, dict)
        manifest["decisions"] = ["CAPRMADIO-DECISION-TOOL-099"]
        manifest_path.write_text(dump(manifest), encoding="utf-8")
        atom = change / "CAPRMADIO-ATOMIC-RECORD-099-native-decision.md"
        atom.write_text(
            "---\n"
            "artifact_type: atomic_record\n"
            "artifact_id: CAPRMADIO-ATOMIC-RECORD-099\n"
            "type: decision\n"
            "semantic_id: CAPRMADIO-DECISION-TOOL-099\n"
            "status: accepted\n"
            "priority: medium\n"
            "llm_session_ids: []\n"
            "---\n\n"
        ...
        self.assertNotIn("CAPRMADIO-E106", {item.code for item in diagnostics})
    ...
    def test_workspace_and_dependency_currentness_are_enforced(self) -> None:
        ...
        data["dependencies"] = [
            {
                "change_id": "CAPRMADIO-CHANGE-GOV-001",
                "pull_request": {
                ...
                "consumes": ["CAPRMADIO-CONTRACT-GOV-001"],
    ...
    def test_scaffold_allocates_stable_change_ids_separately_from_slugs(self) -> None:
        ...
        self.assertEqual(first_data["id"], "CAPRMADIO-CHANGE-TOOL-001")
        self.assertEqual(first_data["slug"], "portable-cli")
        self.assertEqual(second_data["id"], "CAPRMADIO-CHANGE-TOOL-002")
        ...
        (self.root / "caprmadio_settings.toml").write_text(
            "\n".join(
                (
                    'schema_version = "1.2"',
                    "[changes]",
    ...
    def test_workspace_branch_is_unique_only_while_changes_are_active(self) -> None:
        first = self._write_change(
            "tool", "first-change", stable_id="CAPRMADIO-CHANGE-TOOL-101"
        )
        second = self._write_change(
            "tool", "second-change", stable_id="CAPRMADIO-CHANGE-TOOL-102"
        )
        ...
        self.assertNotIn(
            "CAPRMADIO-E154", {item.code for item in validate_repository(self.root)}
        )
        first_manifest = self._change_manifest(first)
    ...
    def _write_change(
        ...
        *,
        ...
        requirement = f"CAPRMADIO-REQUIREMENT-{id_layer}-099"
        test = f"CAPRMADIO-TEST-CASE-{id_layer}-099"
        data = {
            "schema_version": "1.2",
            "id": stable_id or f"CAPRMADIO-CHANGE-{id_layer}-099",
            "slug": change_slug,
            "profile": "small",
            "status": "proposed",
            "primary_layer": layer,
            ...
            "release": {"owner_change": stable_id or f"CAPRMADIO-CHANGE-{id_layer}-099"},
            "intake": [],
            "requirements": [requirement],
            "tests": [test],
            "evals": [],
