...
class RuntimeTests(unittest.TestCase):
    ...
    def test_run_and_checkpoint_lifecycle_is_atomic_and_schema_shaped(self) -> None:
        ...
        checkpoint_path = (
            self.root / ".caprmadio_runtime" / "sessions" / f"{invocation.session_id}.json"
        )
        ...
        updated = update_checkpoint(
            ...
            pending=[
                {
                    "kind": "proof",
                    "id": "CAPRMADIO-TEST-CASE-SKILL-009",
                    "path": "proofs/session.json",
                }
            ],
            touched_paths=["tests/test_runtime.py"],
            next_mode="complete",
            next_reason_code="CAPRMADIO-RUNTIME-COMPLETE",
        )
        ...
        terminals = [
            path
            for path in (self.root / ".caprmadio_runtime" / "runs").glob("*.json")
        ...
        run_schema = json.loads(
            (ROOT / ".caprmadio/04_layer_skill/schemas/skill-run.schema.toml").read_text(
                encoding="utf-8"
            )
        )
    ...
    def test_resume_requires_one_compatible_active_checkpoint(self) -> None:
        scope = {
            "project": "dset",
            "change": "CAPRMADIO-CHANGE-SKILL-001",
    ...
    def test_absent_or_non_caprmadio_root_never_creates_project_state(self) -> None:
        ...
        invocation = start_run(
            ...
            next_reason_code="CAPRMADIO-RUNTIME-INITIALIZE",
        )
        ...
        self.assertFalse((ordinary / ".caprmadio").exists())

    def test_retention_enforces_age_count_and_total_bytes(self) -> None:
        runs = self.root / ".caprmadio_runtime" / "runs"
        seed = start_run(
            self.root,
            public_entrypoint="caprmadio",
