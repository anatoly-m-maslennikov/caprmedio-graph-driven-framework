...
SCHEMA_PATH = ROOT / ".caprmadio/02_layer_gov/schemas/artifact-types.schema.toml"
...
class ArtifactTypeRegistryTests(unittest.TestCase):
    ...
    def test_analysis_report_templates_cover_every_direct_subtype(self) -> None:
        template_root = ROOT / ".caprmadio/02_layer_gov/templates/change"
        expected = {
    ...
    def test_type_only_names_are_default_and_subtype_names_are_opt_in(self) -> None:
        with temporary_directory() as raw:
            ...
            self.assertTrue(
                any(item.code == "CAPRMADIO-E157" for item in default_diagnostics)
            )
            self.assertEqual(
                validate_artifact_type_registry(
                    root,
    ...
    def test_project_enables_subtype_names_but_template_defaults_to_type_only(
        ...
        template = (
            ROOT / ".caprmadio/01_layer_meta/templates/caprmadio_settings.toml"
    ...
    def test_release_lifecycle_projection_is_compiled(self) -> None:
        rule = (
            ROOT / ".caprmadio/02_layer_gov/specification-artifact-classification.md"
        ).read_text(encoding="utf-8")
        for phrase in (
            "`version`",
            "`roadmap`",
    ...
    def assert_error(self, data: dict[str, Any], fragment: str) -> None:
        ...
        self.assertTrue(
            any(
                item.code == "CAPRMADIO-E156" and fragment in item.message
                for item in diagnostics
            ),
            diagnostics,
        )
