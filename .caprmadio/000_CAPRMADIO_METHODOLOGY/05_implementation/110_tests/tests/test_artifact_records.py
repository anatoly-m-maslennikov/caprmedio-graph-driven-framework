...
class ArtifactRecordTests(unittest.TestCase):
    ...
    def _project(self, raw: str) -> Path:
        root = Path(raw)
        control = root / ".caprmadio"
        control.mkdir()
        (control / "caprmadio_settings.toml").write_text(
            "\n".join(
                (
                    'schema_version = "1.8"',
                    "[artifacts]",
    ...
    def _atom(self, root: Path, extra: str = "") -> Path:
        path = root / ".caprmadio/atom.md"
        path.write_text(
            "\n".join(
                (
                    "+++",
                    'artifact_id = "CAPRMADIO-ATOMIC-001"',
                    'semantic_id = "CAPRMADIO-CLAIM-001"',
    ...
    def test_archive_moves_bytes_without_changing_route(self) -> None:
        with temporary_directory(prefix="dset-route-record-") as raw:
            ...
            destination = archive_atom(root, "CAPRMADIO-CLAIM-001")

            self.assertEqual(
                destination,
                root.resolve() / ".caprmadio/archive/atom.md",
            )
