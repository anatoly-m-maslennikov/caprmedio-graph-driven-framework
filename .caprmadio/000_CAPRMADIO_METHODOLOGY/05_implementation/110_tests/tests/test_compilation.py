...
class CompilationTests(unittest.TestCase):
    ...
    def test_loose_id_mentions_are_not_compiled_claim_fragments(self) -> None:
        with temporary_directory() as raw:
            ...
            path.write_text(
                "A loose mention of CAPRMADIO-REQUIREMENT-GOV-035 is not a claim.\n",
                encoding="utf-8",
            )
            self.assertEqual(
                _projection_fragments(path, "CAPRMADIO-REQUIREMENT-GOV-035"), []
            )
            path.write_text(
                "## CAPRMADIO-REQUIREMENT-GOV-035 — Explicit claim\n\n"
                "The governed consequence is stated here.\n",
                encoding="utf-8",
            )
            fragments = _projection_fragments(path, "CAPRMADIO-REQUIREMENT-GOV-035")
    ...
    def test_every_active_authority_has_a_digest_bound_projection(self) -> None:
        ...
        self.assertIn("CAPRMADIO-DECISION-GOV-008", records)
        self.assertNotIn("CAPRMADIO-DECISION-GOV-007", records)
    ...
    def test_projection_change_makes_explicit_compilation_stale(self) -> None:
        with temporary_directory() as raw:
            ...
            shutil.copytree(ROOT / ".caprmadio", root / ".caprmadio")
            write_compilation(root)
            self.assertTrue(compilation_is_fresh(root))
            spec = root / ".caprmadio/02_layer_gov/specification-methodology.md"
