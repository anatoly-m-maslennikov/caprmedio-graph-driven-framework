...
class ProjectSettingsTests(unittest.TestCase):
    ...
    def test_emitted_settings_are_documented_1_3_defaults(self) -> None:
        root_text = (ROOT / ".caprmadio" / SETTINGS_FILENAME).read_text(encoding="utf-8")
        template_text = (
            ROOT / f".caprmadio/01_layer_meta/templates/{SETTINGS_FILENAME}"
    ...
    def test_removed_priority_values_fail_closed(self) -> None:
        for removed in ("critical", "deferred"):
            with self.subTest(priority=removed):
                with temporary_directory() as raw:
                    root = Path(raw).resolve()
                    settings_path = root / ".caprmadio" / SETTINGS_FILENAME
    ...
    def test_competing_settings_filenames_fail_closed(self) -> None:
        with temporary_directory() as raw:
            root = Path(raw).resolve()
            (root / ".caprmadio").mkdir()
            (root / ".caprmadio" / SETTINGS_FILENAME).write_text(
                'schema_version = "1.3"\n', encoding="utf-8"
            )
            (root / LEGACY_SETTINGS_FILENAME).write_text(
                'schema_version = "1.2"\n', encoding="utf-8"
        ...
        self.assertEqual(
            ...
            (
                ...
                ".caprmadio/caprmadio_settings.toml, dset.toml",
            ),
        )
    ...
    def test_repository_validation_reports_competing_names_at_canonical_path(
        ...
        conflict = [item for item in diagnostics if item.code == "CAPRMADIO-E157"]
