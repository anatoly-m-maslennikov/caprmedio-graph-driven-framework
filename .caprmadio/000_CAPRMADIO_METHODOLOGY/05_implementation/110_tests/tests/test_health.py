...
class ProjectHealthTests(unittest.TestCase):
    ...
    def test_prose_mentions_do_not_create_coverage(self) -> None:
        ...
        requirements.append("CAPRMADIO-REQUIREMENT-999")
        tests.append("CAPRMADIO-TEST-CASE-999")
        manifest.write_text(dump(data, manifest), encoding="utf-8")

        spec = self.root / "dset/specs/packages/sample/spec.md"
        spec.write_text(
            spec.read_text(encoding="utf-8")
            + "\nLoose reference to CAPRMADIO-REQUIREMENT-999.\n",
            encoding="utf-8",
        )
        test_plan = self.root / "dset/specs/packages/sample/test-plan.md"
        test_plan.write_text(
            test_plan.read_text(encoding="utf-8")
            + "\nCAPRMADIO-TEST-CASE-999 mentions CAPRMADIO-REQUIREMENT-999.\n",
            encoding="utf-8",
        )
        proofs = self.root / "dset/changes/sample/proofs"
        proofs.mkdir(parents=True, exist_ok=True)
        (proofs / "mention.md").write_text(
            "CAPRMADIO-TEST-CASE-999 is mentioned without an evidence_for relation.\n",
            encoding="utf-8",
        )
        ...
        self.assertIn("CAPRMADIO-REQUIREMENT-999", compiled.gaps)
        self.assertIn("CAPRMADIO-REQUIREMENT-999", checked.gaps)
        self.assertIn("CAPRMADIO-TEST-CASE-999", evidenced.gaps)

    def test_inactive_qa_is_excluded_from_current_evidence_coverage(self) -> None:
        identifier = "CAPRMADIO-TEST-CASE-998"
        ...
        self.assertIn(identifier, _qa_ids(self.root, {identifier: atom}, set()))
        self.assertNotIn(
            identifier,
            _qa_ids(
                self.root,
    ...
    def test_version_roles_are_derived_from_direct_subtypes(self) -> None:
        ...
        for sequence, subtype in enumerate(subtypes, start=900):
            (folder / f"CAPRMADIO-VERSION-{sequence}.md").write_text(
                "+++\n"
                'artifact_type = "version"\n'
                f'artifact_subtype = "{subtype}"\n'
                f'artifact_id = "CAPRMADIO-VERSION-{sequence}"\n'
                'priority = "medium"\n'
                "llm_session_ids = []\n"
                "+++\n",
                encoding="utf-8",
