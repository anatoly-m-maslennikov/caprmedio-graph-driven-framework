...
class GovernanceTests(unittest.TestCase):
    ...
    def test_cli_resolve_reports_rule_identity(self) -> None:
        ...
        self.assertIn("CAPRMADIO-CONFLICT-RESOLUTION-UNAVAILABLE", output)
    ...
    def test_empty_conflicts_do_not_claim_resolution_coverage(self) -> None:
        ...
        self.assertEqual(
            resolved["conflict_resolution"],
            {
                "status": "unavailable",
                "coverage": [],
                "reason_code": "CAPRMADIO-CONFLICT-RESOLUTION-UNAVAILABLE",
            },
        )
    ...
    def test_registry_failure_codes_are_stable(self) -> None:
        cases = {
            "missing-registry": ("CAPRMADIO-E130", self._remove_registry),
            "missing-owner": ("CAPRMADIO-E131", self._missing_owner),
            "duplicate-owner": ("CAPRMADIO-E132", self._duplicate_owner),
            "missing-document": ("CAPRMADIO-E133", self._missing_document),
            "outside-root": ("CAPRMADIO-E134", self._outside_root),
            "dependency-cycle": ("CAPRMADIO-E135", self._dependency_cycle),
            "precedence-cycle": ("CAPRMADIO-E150", self._precedence_cycle),
            "missing-precedence": ("CAPRMADIO-E150", self._missing_precedence),
            "missing-precedence-owner": (
                "CAPRMADIO-E150",
                self._missing_precedence_owner,
            ),
            "backward-dependency": ("CAPRMADIO-E151", self._backward_dependency),
            "backward-precedence": ("CAPRMADIO-E151", self._backward_precedence),
            "incompatible-profile": ("CAPRMADIO-E137", self._incompatible_profile),
            "wrapper-mismatch": ("CAPRMADIO-E138", self._wrapper_mismatch),
        }
    ...
    def test_unknown_workflow_is_stable(self) -> None:
        ...
        self.assertEqual([item.code for item in diagnostics], ["CAPRMADIO-E136"])

    def test_irreducible_backward_authority_proposes_features(self) -> None:
        self._backward_dependency(self.root)
        diagnostic = next(
            item for item in validate_governance(self.root) if item.code == "CAPRMADIO-E151"
        )
        self.assertIn(
    ...
    def test_customization_changes_rules_not_wrapper(self) -> None:
        ...
        self.assertIn(
            "CAPRMADIO-E139", {item.code for item in validate_governance(self.root)}
        )
        ...
        with temporary_directory() as raw:
            ...
            for label in ("project-a", "project-b"):
                ...
                local_rule = next(
                    ...
                    if item["id"] == "CAPRMADIO-RULE-DOMAIN-SPEC"
                )
    ...
    def test_profile_assigns_every_rule_to_a_stable_layer(self) -> None:
        ...
        self.assertEqual(
            actual,
            {
                "CAPRMADIO-RULE-ARCHITECTURE": "GOV",
                "CAPRMADIO-RULE-ARTIFACT-CLASSIFICATION": "GOV",
                "CAPRMADIO-RULE-BUILD": "TOOL",
                "CAPRMADIO-RULE-DOMAIN-SPEC": "META",
                "CAPRMADIO-RULE-TEST-CASE": "META",
                "CAPRMADIO-RULE-EVALUATION-CASE": "META",
                "CAPRMADIO-RULE-SUPPORTABILITY": "OPS",
                "CAPRMADIO-RULE-ARTIFACT-MAINTENANCE": "GOV",
                "CAPRMADIO-RULE-WORK-ITEMS": "GOV",
                "CAPRMADIO-RULE-DELEGATION-BUDGET": "SKILL",
                "CAPRMADIO-RULE-SKILL-RUNS": "SKILL",
                "CAPRMADIO-RULE-RELEASE": "OPS",
                "CAPRMADIO-RULE-LIFECYCLE": "SKILL",
                "CAPRMADIO-RULE-DIAGNOSIS": "SKILL",
                "CAPRMADIO-RULE-PROTOTYPING": "SKILL",
            },
        )
    ...
    def test_bootstrap_release_target_is_explicit_not_framework_hard_coded(
        ...
        release = (ROOT / ".caprmadio/05_layer_ops/procedure-release.md").read_text(
            encoding="utf-8"
        )
    ...
    def test_temporary_adopter_materializes_current_project_inputs(self) -> None:
        ...
        self.assertEqual(manifest["contracts"], ["CAPRMADIO-CONTRACT-001"])
        ...
        self.assertTrue((self.root / "caprmadio_settings.toml").is_file())
        ...
        self.assertEqual(package["contracts"], ["CAPRMADIO-CONTRACT-001"])
        ...
        release = next(
            rule
            for rule in cast(list[dict[str, Any]], registry["rules"])
            if rule["id"] == "CAPRMADIO-RULE-RELEASE"
        )
        ...
        self.assertTrue(
            all(
                "CAPRMADIO-RULE-RELEASE" not in cast(list[str], item["rules"])
    ...
    def test_atomic_rationale_is_supported_but_optional(self) -> None:
        ...
        for schema_path in (
            ROOT / ".caprmadio/04_layer_skill/schemas/skill-run.schema.toml",
            ROOT / ".caprmadio/04_layer_skill/schemas/session-checkpoint.schema.toml",
        ):
        ...
        for name in ("decision.md", "adoption-decision.md"):
            decision_template = (
                ROOT / ".caprmadio/02_layer_gov/templates/change" / name
        ...
        rule = (
            ROOT / ".caprmadio/02_layer_gov/specification-artifact-maintenance.md"
        ).read_text(encoding="utf-8")
    ...
    def test_trace_id_schemas_are_type_first_and_layer_bounded(self) -> None:
        ...
        for valid in (
            "CAPRMADIO-SCENARIO-META-001",
            "CAPRMADIO-INVARIANT-001",
            "CAPRMADIO-TASK-TOOL-001",
            "CAPRMADIO-CONTRACT-OPS-001",
        ...
        for invalid in (
            "REQUIREMENT-CAPRMADIO-GOV-001",
            "CAPRMADIO-REQUIREMENT-GOV-001",
            "CAPRMADIO-REQUIREMENT-GLOBAL-001",
            "CAPRMADIO-ADR-GOV-001",
        ):
            self.assertIsNone(
    ...
    def test_validator_uses_the_manifest_project_key(self) -> None:
        ...
        for filename in ("spec.md", "test-plan.md", "contracts.md"):
            ...
            path.write_text(
                path.read_text(encoding="utf-8").replace("CAPRMADIO-", "ACME-"),
                encoding="utf-8",
            )
    ...
    def test_legacy_intake_enforces_type_and_registered_layer(self) -> None:
        ...
        self.assertNotIn(
            "CAPRMADIO-E142", {item.code for item in validate_repository(self.root)}
        )
        invalid_cases = (
            {**valid, "id": "CAPRMADIO-PROBLEM-GOV-001"},
            {**valid, "id": "CAPRMADIO-QUESTION-GLOBAL-001"},
            {**valid, "scope": "tool"},
            {**valid, "decision": "CAPRMADIO-DECISION-TOOL-001"},
        )
    ...
    def test_contract_template_requires_real_host_and_ci_proof(self) -> None:
        ...
        self.assertEqual(
            contract_ids,
            [
                "CAPRMADIO-CONTRACT-META-001",
                "CAPRMADIO-CONTRACT-OPS-001",
                "CAPRMADIO-CONTRACT-SKILL-001",
                "CAPRMADIO-CONTRACT-SKILL-002",
                "CAPRMADIO-CONTRACT-TOOL-001",
    ...
    def test_story_and_outcome_records_require_structured_fields(self) -> None:
        ...
        manifest = cast(dict[str, Any], load(manifest_path))
        manifest["stories"] = ["CAPRMADIO-STORY-001"]
        manifest["outcomes"] = ["CAPRMADIO-OUTCOME-001"]
        manifest_path.write_text(dump(manifest, manifest_path), encoding="utf-8")
        (package_root / "stories.md").write_text(
            "# User Stories\n\n## CAPRMADIO-STORY-001 — Incomplete\n\nActor only.\n",
            encoding="utf-8",
        )
        (package_root / "outcomes.md").write_text(
            "# Outcomes\n\n## CAPRMADIO-OUTCOME-001 — Incomplete\n\nTarget only.\n",
            encoding="utf-8",
        )
        self.assertIn(
            "CAPRMADIO-E117", {item.code for item in validate_repository(self.root)}
        )

        (package_root / "stories.md").write_text(
            """# User Stories

## CAPRMADIO-STORY-001 — Contributor can validate

- **Actor or stakeholder:** Contributor
- **Desired capability or outcome:** Validate locally
- **Value or purpose:** Catch errors early
- **Linked Requirements:** CAPRMADIO-REQUIREMENT-001
...
## CAPRMADIO-OUTCOME-001 — Earlier feedback
...
- **Linked User Stories:** CAPRMADIO-STORY-001.
...
""",
    ...
    def test_wrappers_are_thin_and_registered(self) -> None:
        workflows = {
            "lifecycle-orchestration": "caprmadio",
        ...
        for workflow, skill in workflows.items():
            with self.subTest(workflow=workflow):
                ...
                self.assertIn("CAPRMADIO-RULE-SKILL-RUNS", resolved_workflows[workflow])
                self.assertIn("CAPRMADIO-RULE-LIFECYCLE", resolved_workflows[workflow])
    ...
    def test_manifest_and_template_boundaries_are_stable(self) -> None:
        ...
        self.assertEqual(captured.exception.code, "CAPRMADIO-E140")
        discover_layout(self.root).manifest_path.unlink()
        self.assertEqual(validate_repository(self.root)[0].code, "CAPRMADIO-E001")
    ...
    def test_health_review_and_ranking_rules_are_compiled(self) -> None:
        ...
        template = (
            ROOT
            / ".caprmadio/02_layer_gov/templates/governance/core-v1/artifact-maintenance.md"
        ).read_text(encoding="utf-8")
        ...
        architecture = (
            ROOT / ".caprmadio/02_layer_gov/specification-architecture.md"
        ).read_text(encoding="utf-8")
        architecture_template = (
            ROOT / ".caprmadio/02_layer_gov/templates/governance/core-v1/architecture.md"
        ).read_text(encoding="utf-8")
        ...
        work_items = (
            ROOT / ".caprmadio/02_layer_gov/specification-work-items.md"
        ).read_text(encoding="utf-8")
        work_items_template = (
            ROOT / ".caprmadio/02_layer_gov/templates/governance/core-v1/work-items.md"
        ).read_text(encoding="utf-8")
        ...
        gov = (ROOT / ".caprmadio/02_layer_gov/specification-methodology.md").read_text(
            encoding="utf-8"
        )
        tool = (ROOT / ".caprmadio/03_layer_tool/specification-methodology.md").read_text(
            encoding="utf-8"
        )
        self.assertIn("CAPRMADIO-REQUIREMENT-GOV-024", gov)
        self.assertIn("CAPRMADIO-REQUIREMENT-GOV-025", gov)
        self.assertIn("CAPRMADIO-REQUIREMENT-GOV-026", gov)
        self.assertIn("CAPRMADIO-REQUIREMENT-TOOL-018", tool)
        self.assertIn("CAPRMADIO-REQUIREMENT-TOOL-019", tool)
        ...
        self.assertTrue(
            {
                ...
                "CAPRMADIO-TEST-CASE-TOOL-018",
                "CAPRMADIO-TEST-CASE-TOOL-019",
    ...
    def _missing_precedence_owner(cls, root: Path) -> None:
        _, data = cls._registry(root)
        data["rules"][0]["precedence_over"] = ["CAPRMADIO-RULE-MISSING"]
        cls._write_registry(root, data)
