...
### CAPRMADIO-DECISION-OPS-007 — Flat Version lifecycle roles
...
### CAPRMADIO-DECISION-OPS-013 — Operate without backward implementation authority
...
## CAPRMADIO-REQUIREMENT-OPS-001 — Production supportability is explicit and risk-scaled
...
**Scenario CAPRMADIO-SCENARIO-OPS-001:** A production-bound local tool uses bounded structured local diagnostic records and run/build identity appropriate to its risk without being forced to deploy a tracing backend; a non-production one-shot tool records a justified not-applicable disposition.

**Scenario CAPRMADIO-SCENARIO-OPS-002:** A distributed, stateful, retryable, or high-risk service propagates correlation and deploy/change identity across relevant effect boundaries, provides safe bounded diagnostics and incident runbooks, and adds tracing, audited access, rollback, or stronger redaction when its topology and profile require them.

## CAPRMADIO-REQUIREMENT-OPS-002 — Repository delivery is supportable
...
**Scenario CAPRMADIO-SCENARIO-OPS-003:** An operator can diagnose a blocked or incorrect `dev → main` delivery, contain it by preserving the draft/PR evidence, and recover without bypassing protected `main` or rewriting history.

## CAPRMADIO-REQUIREMENT-OPS-003 — DSET 0.3 release is framework-first
...
**Scenario CAPRMADIO-SCENARIO-OPS-004:** A TypeScript-only gate is not applied to Python sources, but its versioned adopter fixture passes before the gate is presented to Your Harness as released DSET behavior.

## CAPRMADIO-REQUIREMENT-OPS-004 — Every protected release PR has one deterministic transition

After policy activation, every configured integration-to-protected release-branch PR must contain exactly one committed release declaration in one release-owning Change; any other participating Changes contain only a reference to that owner. Omission, multiple declarations, and dangling/cyclic references fail preparation and cannot classify the PR as non-release. The declaration records protected base ref/SHA/version, candidate SHA, target, readiness owner, and exactly one class. `CAPRMADIO-RULE-RELEASE` owns the complete bootstrap/pre-1.0/RC/final/post-1.0 table, highest-impact aggregation, ambiguity stop, integer arithmetic, and immutable published-RC progression.

**Scenario CAPRMADIO-SCENARIO-OPS-005:** `0.2.4` becomes `0.3.0` for a normal PR or `0.2.5` for a small PR; `0.9.0` becomes `0.10.0`, never `1.0.0`.

## CAPRMADIO-REQUIREMENT-OPS-005 — Release authority is configured and idempotent

Each applicable project must configure an integration branch, protected release branch, publisher, and tag pattern containing one `{product_version}` placeholder. One committed declaration is authoritative for class, protected-base ref/SHA/version, candidate SHA, target, and readiness artifact; version files, package metadata, notes, PR text, rendered tag, and publisher release are mirrors. Preparation reads the protected base once, rejects drift, and is idempotent. Interactive `ca-release` prepares/verifies; authorized post-merge automation publishes at the exact merge SHA. Retry creates only missing objects, accepts exact matches, and stops on collision. Published tags are immutable; bad releases are preserved/withdrawn and corrected by a higher-version PR without a post-merge content commit.

**Scenario CAPRMADIO-SCENARIO-OPS-006:** A PR with mismatched product/package targets fails before merge; a passing merge is tagged at its exact merge commit without modifying `main` files afterward.

## CAPRMADIO-REQUIREMENT-OPS-006 — RC and final releases are fully working gates
...
**Scenario CAPRMADIO-SCENARIO-OPS-007:** A green candidate with one required pilot unfinished remains `0.y.z`; a fully working `1.0.0-rc.2` may promote to `1.0.0` only after final evidence passes and without adding features.

## CAPRMADIO-REQUIREMENT-OPS-007 — Product/package identity is coordinated
...
**Scenario CAPRMADIO-SCENARIO-OPS-008:** Product/package `0.3.0` may validly use schema `1.0` and governance profile `core-v1@0.2`; neither compatibility version implies DSET 1.0 readiness.

## CAPRMADIO-REQUIREMENT-OPS-013 — Release lifecycle artifacts are flat and typed
...
**Scenario CAPRMADIO-SCENARIO-OPS-014:** A `0.4.0` candidate references its `0.4`
