...
mutates. Sources: `CAPRMADIO-REQUIREMENT-META-047` and
...
2. Read `.caprmadio/caprmadio_settings.toml`, then the enabled type/subtype whitelist.
3. Read `.caprmadio/artifact_catalog.toml` for route, identity, carrier, and
   persistence mappings.
4. Resolve installed rules, artifacts, views, tools, and skill instructions
   only inside the selected `.caprmadio`.
...
Sources: `CAPRMADIO-REQUIREMENT-META-058` and
...
`CAPRMADIO-REQUIREMENT-GOV-102` and `CAPRMADIO-REQUIREMENT-GOV-112`.
...
view is stale until refreshed. Sources: `CAPRMADIO-REQUIREMENT-META-054` and
`CAPRMADIO-REQUIREMENT-GOV-115`.
...
release evidence stale. Source: `CAPRMADIO-REQUIREMENT-GOV-109`.
...
`CAPRMADIO-IMPL-GOV-006`, `CAPRMADIO-REQUIREMENT-GOV-065`, and
...
reference graph. Sources: `CAPRMADIO-IMPL-GOV-001`,
`CAPRMADIO-DECISION-GOV-027`, and `CAPRMADIO-REQUIREMENT-GOV-108`.
...
`CAPRMADIO-CONSTRAINT-GOV-002` and `CAPRMADIO-REQUIREMENT-GOV-116`.

## Storage and discovery boundaries

- `.caprmadio/` contains current governed state, installed methodology, settings,
  catalogs, tools, and skill instructions;
- `.caprmadio_journal/` contains durable append-only NDJSON running records; and
- `.caprmadio_runtime/` contains reconstructible or resumable runtime state; and
...
and inactive legacy documentation stay outside `.caprmadio` and are never a
fallback governance source. Repository legal files stay at root `LICENSE` and
`LICENSES`, outside all three DSET roots. Sources:
`CAPRMADIO-IMPL-GOV-008`, `CAPRMADIO-DECISION-GOV-029`, and
...
`CAPRMADIO-QUESTION-GOV-014`, and `CAPRMADIO-PROBLEM-GOV-008` blocks any claim that the
current catalog is total and one-to-one. Current role semantics come from
`CAPRMADIO-REQUIREMENT-GOV-078`, `CAPRMADIO-REQUIREMENT-GOV-080`, and
`CAPRMADIO-REQUIREMENT-GOV-103`.
