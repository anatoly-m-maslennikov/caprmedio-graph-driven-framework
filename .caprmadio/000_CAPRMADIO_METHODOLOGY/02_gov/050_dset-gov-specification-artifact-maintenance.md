...
**Rule ID:** `CAPRMADIO-RULE-ARTIFACT-MAINTENANCE`
...
relation meanings; a Test Case or Evaluation Case also fixes its conditions,
...
Source: `CAPRMADIO-DECISION-GOV-035` and `CAPRMADIO-REQUIREMENT-GOV-096`.
...
`.caprmadio/caprmadio_settings.toml` selects `artifacts.creation_strictness` independently from
...
`Implements: CAPRMADIO-REQUIREMENT-IMPL-004`. A linked Problem may explain why a
...
Source: `CAPRMADIO-REQUIREMENT-GOV-065`.
...
`CAPRMADIO-REQUIREMENT-GOV-107`. The former `critical` label is
...
`.caprmadio/caprmadio_settings.toml` records an explicit Boolean for every registered
