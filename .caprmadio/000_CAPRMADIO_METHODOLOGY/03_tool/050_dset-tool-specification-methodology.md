...
## CAPRMADIO-DECISION-TOOL-001 — Portable text-byte policy
...
This Decision replaces `CAPRMADIO-DECISION-OPS-008`. Repository byte normalization
...
## CAPRMADIO-REQUIREMENT-TOOL-001 — The canonical workflow is executable
...
**Scenario CAPRMADIO-SCENARIO-TOOL-001:** A contributor can run `python -m dset_toolchain check .` without installing OpenSpec or a second methodology and receives stable diagnostic codes for malformed artifacts.

## CAPRMADIO-REQUIREMENT-TOOL-002 — Traceability is generated from durable identities

`.caprmadio_runtime/generated/traceability.toml` must be generated in stable order from committed change manifests and repository-qualified PR references. It may cache evidence relationships but must not replace GitHub as owner of PR state, checks, diffs, or merge results.

**Scenario CAPRMADIO-SCENARIO-TOOL-002:** Regeneration without source changes produces no diff, and every archived change resolves to the PR that owns its implementation history.

## CAPRMADIO-REQUIREMENT-TOOL-003 — Archive writes are guarded
...
**Scenario CAPRMADIO-SCENARIO-TOOL-003:** A proposed, failed, incomplete, PR-less, or colliding change remains active and no archive path is overwritten.

## CAPRMADIO-REQUIREMENT-TOOL-004 — DSET 0.3 self-hosting is bounded
...
**Scenario CAPRMADIO-SCENARIO-TOOL-004:** One release run records released-to-candidate
...
## CAPRMADIO-REQUIREMENT-TOOL-018 — Project health has a portable deterministic renderer
...
**Scenario CAPRMADIO-SCENARIO-TOOL-018:** Two runs over unchanged inputs produce the
...
## CAPRMADIO-REQUIREMENT-TOOL-019 — Priority conflict resolution is deterministic
...
**Scenario CAPRMADIO-SCENARIO-TOOL-019:** A Requirement and project-owned Contract
...
## CAPRMADIO-REQUIREMENT-TOOL-021 — TypeScript enforcement starts as an evidence-derived candidate
...
**Scenario CAPRMADIO-SCENARIO-TOOL-021:** The pinned pilot has known lint and typing
...
## CAPRMADIO-REQUIREMENT-TOOL-022 — Profile references and applied instances are distinct
...
**Scenario CAPRMADIO-SCENARIO-TOOL-022:** A clean adopter receives the framework
...
## CAPRMADIO-REQUIREMENT-TOOL-023 — Configure governance surfaces

Atomic source: `CAPRMADIO-REQUIREMENT-TOOL-023`.
