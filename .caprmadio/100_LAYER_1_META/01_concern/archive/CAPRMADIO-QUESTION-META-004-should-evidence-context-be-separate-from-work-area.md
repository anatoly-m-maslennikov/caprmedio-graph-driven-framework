---
artifact_type: question
artifact_id: CAPRMADIO-QUESTION-META-004
scope_path: layer:meta
subject_scope: scope-topology
priority: medium
llm_session_ids:
  - codex:019f591f-04f6-70f2-8de7-828b7cccc69d
---

# Question — Should evidence context be separate from Work Area?

Should CAPRMADIO add a compact execution and evidence context for version, environment, assumptions, and time while keeping Work Area only as the repository ownership and target boundary?

## Migrated context

- Original intake status: `open`
- Original owner Change: `CAPRMADIO-CHANGE-SKILL-001`

This one-claim carrier replaces the former aggregate intake row. Current
status is derived from atomic lifecycle events.
