...
| Check | Result |
|---|---|
| Red test before registry/package rename | `domain-clarification` failed with `CAPRMADIO-E136` as expected |
| `dset rules resolve domain-clarification --format json` | Pass; resolves `dset-clarify` and the ordered local rule set |
| Deprecated `domain-grilling` workflow | Fails closed with `CAPRMADIO-E136` |
