...
artifact_id: CAPRMADIO-REQUIREMENT-GOV-178
...
Every structural scope that realizes Requirement, Method, Assurance, or Delivery Atoms owns one logical Implementation Journal under its `08_implementation/` role folder. The Journal uses append-only NDJSON and may rotate into ordered carrier segments without changing its logical identity or rewriting admitted records.
