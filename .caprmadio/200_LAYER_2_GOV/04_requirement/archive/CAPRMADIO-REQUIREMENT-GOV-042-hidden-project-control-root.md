...
artifact_id: CAPRMADIO-REQUIREMENT-GOV-042
...
- `.caprmadio/caprmadio_settings.toml` — the only writable settings and project-manifest
  carrier;
...
- `.caprmadio/<layer>/` — direct META, GOV, TOOL, SKILL, and OPS roots containing
  layer evergreen artifacts plus layer-owned atomic/context and proof records;
  and
- `.caprmadio/runtime/` — ignored run, session, cache, migration-backup, and other
...
`.caprmadio/gov/plan-tests.md`. They are never interpreted relative to `.caprmadio/`.
...
A current DSET project owns its control plane under .caprmadio, stores canonical paths relative to the repository root, and isolates ignored operational state under .caprmadio/runtime.
