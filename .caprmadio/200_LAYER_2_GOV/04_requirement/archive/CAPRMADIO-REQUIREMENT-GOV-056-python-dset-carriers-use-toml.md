...
artifact_id: CAPRMADIO-REQUIREMENT-GOV-056
...
relations:
  ...
  - type: replacement_of
    targets:
      - CAPRMADIO-CONSTRAINT-GOV-001
---

# Requirement — Python-owned DSET carriers use TOML

All CAPRMADIO-owned structured artifacts, settings, generated structured state,
...
The Python-owned DSET framework uses TOML for every CAPRMADIO-owned structured carrier and schema; YAML remains only where an external host contract mandates YAML, and JSON remains only in genuinely JavaScript or TypeScript-owned areas.
