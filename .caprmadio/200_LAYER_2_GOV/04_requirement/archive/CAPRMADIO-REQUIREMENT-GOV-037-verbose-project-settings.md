...
artifact_id: CAPRMADIO-REQUIREMENT-GOV-037
...
relations:
  - type: replacement_of
    targets:
      - CAPRMADIO-REQUIREMENT-GOV-036
---

# Requirement — Publish verbose project settings

The canonical root settings carrier is `caprmadio_settings.toml`. It must explain
...
New writers and bootstraps emit only `caprmadio_settings.toml`. A root `dset.toml`
...
CAPRMADIO-owned mutable structured artifacts and new DSET Markdown frontmatter use
...
DSET exposes operator-selectable behavior through one verbose caprmadio_settings.toml file while project identity, topology, contracts, and verification remain manifest truth.
