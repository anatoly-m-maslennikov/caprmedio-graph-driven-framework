...
artifact_id: CAPRMADIO-REQUIREMENT-GOV-043
...
- `.caprmadio/` — committed DSET settings, governance, evergreen artifacts, atomic
  artifacts, Version records, schemas, templates, and generated views;
- `.caprmadio_runtime/` — ignored, bounded, resumable project runtime state,
...
schema 1.3 `.caprmadio/runtime/` location remain read-only compatibility inputs;
current initialization and writes use `.caprmadio_runtime/`.
...
A DSET project separates committed control state under .caprmadio, ignored resumable runtime state under .caprmadio_runtime, and disposable scratch workspaces under the host temporary root.
