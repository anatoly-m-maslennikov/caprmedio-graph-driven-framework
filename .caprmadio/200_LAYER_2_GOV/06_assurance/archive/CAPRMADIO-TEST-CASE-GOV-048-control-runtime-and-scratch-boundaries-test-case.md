...
artifact_id: CAPRMADIO-TEST-CASE-GOV-048
...
1. current governed state resolves only below `.caprmadio/`;
2. resumable run, session, readiness, cache, and recovery state resolves below
   ignored `.caprmadio_runtime/` and never becomes project authority;
