...
artifact_id: CAPRMADIO-ANALYSIS-REPORT-004
...
relations:
  - type: analysis_of
    targets:
      - CAPRMADIO-REQUIREMENT-GOV-056
      - CAPRMADIO-REQUIREMENT-GOV-095
...
Immediately before this report was added, `.caprmadio` contained:
...
- `.caprmadio/caprmadio_settings.toml`;
...
and rationale, while `caprmadio_settings.toml` selects the profile by stable ID.
...
These normally belong under `.caprmadio_runtime` and are excluded from semantic
...
- Give every evergreen or maintained Markdown artifact valid YAML
  frontmatter.
- Convert identity-only `.caprmadio/**/references/*.toml` carriers to
...
`CAPRMADIO-REQUIREMENT-GOV-056` currently requires TOML for all Python-owned DSET
...
That Requirement should replace `CAPRMADIO-REQUIREMENT-GOV-056`, specialize
`CAPRMADIO-REQUIREMENT-GOV-095`, preserve
`CAPRMADIO-CONSTRAINT-GOV-002`, and authorize a separate migration plan.
