...
relations:
  ...
  - type: replacement_of
    targets:
      - CAPRMEDIO-GOV-REQU-459--distinguish-atomic-cases-from-maintained-plans
  - type: relates_to
    targets:
...
`qa_case` is the atomic, internal Evaluation type. It always has exactly one
direct subtype:
...
evaluation definitions, implementation carriers, and observations without
classifying containers by incidental text. Making Test Case and Eval Case
subtypes of QA Case also preserves one atomic Evaluation type for the route.
