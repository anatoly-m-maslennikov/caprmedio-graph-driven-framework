...
each Task Atom has one effective Author, one effective Assignee, one Claim assigned to that Assignee, one atomic or composite Task Scope, one Definition of Done, one Autonomous Confidence Threshold, optional bounded Task Details, explicit Task dependencies, and one recorded Task Scope Resolution for each execution.
