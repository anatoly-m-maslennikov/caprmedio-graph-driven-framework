...
**every** Epic **must** be a Relational Artifact with **`=1`** Directory Carrier, **`=1`** canonical Epic Identifier, **`=1`** canonical Directory Carrier name, **`=1`** Core Status **in** (Active, Done, Canceled), Status-qualified placement, **and** identity independent from member changes.

## Test case

create one Active Epic **in** its canonical current directory, one Done Epic **in** `done`, **and** one Canceled Epic **in** `canceled`; use canonical identifiers **and** Directory Carrier names; **and** add, remove, reorder, **and** change members **without** changing Epic identity. **then** create an Epic with **`!=1`** Directory Carriers, use malformed identifiers **or** names, place a non-Active Epic outside its Status subdirectory, place an Active Epic inside a Status subdirectory, **and** change Epic identity **after** a member change.
