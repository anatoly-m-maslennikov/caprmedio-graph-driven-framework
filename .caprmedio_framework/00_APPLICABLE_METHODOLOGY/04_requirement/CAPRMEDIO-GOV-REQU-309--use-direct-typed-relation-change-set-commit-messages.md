...
projection:
  source_carrier_path: ../000_APPLICABLE_MTHD_sources/001_CORE_META_MODEL/04_requirement/CAPRMEDIO-GOV-REQU-309--use-direct-typed-relation-change-set-commit-messages.md
---
...
Replacement currently uses no formal relation payload. First add the successor as an active Atom:

```text
child_of=parent.md@2 | ADD | replacement-file.md@1
```

Then archive the predecessor in a `MOVE` whose authoritative Work Journal event records explicit `predecessor_atom_id` and `successor_atom_ids`:

```text
0 | MOVE | active/old-file.md@4 -> archive/old-file.md@4
```

The successor MUST already be active before the predecessor change. The authoritative archival Journal event is the only persisted replacement record: neither the active successor nor the archived predecessor Atom frontmatter stores replacement history. Formal replacement relations and inverse navigation are deferred. The archival move changes no predecessor content, filename, relation frontmatter, or version. Adding each successor and archiving each predecessor remain separate one-file commits.
