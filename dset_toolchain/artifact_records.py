...
def collect_atomic_artifact_records(
    ...
    if settings_issues:
        return {}, [
            Diagnostic("CAPRMADIO-E169", root / ".caprmadio/caprmadio_settings.toml", issue)
...
def _parse_record(
    ...
    diagnostics = [
        Diagnostic("CAPRMADIO-E169", path, issue) for issue in route_issues(metadata)
    ]
    ...
    if not derived and metadata.get("revision_mode") != "atomic":
        diagnostics.append(
            Diagnostic("CAPRMADIO-E169", path, "atomic carrier requires revision_mode atomic")
        )
    if not isinstance(semantic_id, str) or ARTIFACT_ID.fullmatch(semantic_id) is None:
        diagnostics.append(
            Diagnostic("CAPRMADIO-E170", path, "atomic carrier requires semantic_id")
        )
    if not isinstance(carrier_id, str) or ARTIFACT_ID.fullmatch(carrier_id) is None:
        diagnostics.append(
            Diagnostic("CAPRMADIO-E170", path, "atomic carrier requires artifact_id")
        )
    if status not in {"proposed", "accepted"}:
        diagnostics.append(
            Diagnostic("CAPRMADIO-E170", path, "atomic status must be proposed or accepted")
        )
    if priority not in priorities:
        diagnostics.append(
            Diagnostic("CAPRMADIO-E170", path, "atomic priority is not in project scale")
        )
