...
def distribution_source(source_root: Path | None = None) -> Iterator[tuple[Path, str]]:
    ...
    with temporary_directory(prefix="dset-source-") as raw:
        ...
        for directory in APPLIED_LAYER_DIRECTORIES.values():
            (source / ".caprmadio" / directory).mkdir(parents=True)
...
def _stage_project(
    ...
    *,
    ...
    caprmadio_root = stage / ".caprmadio"
    caprmadio_root.mkdir()
    methodology_root = caprmadio_root / METHODOLOGY_ROOT
    methodology_root.mkdir()
    for directory in ("00_project", *METHODOLOGY_LAYER_DIRECTORIES.values()):
        (methodology_root / directory).mkdir()
    for directory in APPLIED_LAYER_DIRECTORIES.values():
        (caprmadio_root / directory).mkdir()
    if source_layout.separated:
        shutil.copytree(
            source_layout.caprmadio_root / METHODOLOGY_ROOT,
            methodology_root,
            dirs_exist_ok=True,
        )
    else:
    ...
    manifest_path = caprmadio_root / "caprmadio_settings.toml"
    _write_combined_settings(
        source_layout.find_template("dset-settings.toml"),
        manifest_path,
        manifest,
    )
    catalog_path = caprmadio_root / "artifact_catalog.toml"
    ...
    ops = caprmadio_root / APPLIED_LAYER_DIRECTORIES["ops"]
    supportability = ops / "08_OPS" / "supportability"
    supportability.mkdir(parents=True)
    ...
    (caprmadio_root / "CAPRMADIO-CONTROL-HUB.md").write_text(
    ...
    project_requirements = caprmadio_root / CONTENT_ROLE_DIRECTORIES[2]
    project_requirements.mkdir()
    (project_requirements / f"{project_key}-PROJECT-HUB.md").write_text(
        "# Applied project-wide DSET artifacts\n\n"
        "## Purpose\n\nOwn project-wide requirement navigation.\n\n"
        "## Boundaries\n\nLayer-specific truth belongs to its applied layer.\n\n"
        "## Start here\n\nUse the ordered Content-role folders.\n",
        encoding="utf-8",
    )
    delivery = caprmadio_root / PROJECT_DELIVERY_ROOT
    delivery.mkdir()
    (delivery / f"{project_key}-VERSIONS-HUB.md").write_text(
        "# Version lifecycle\n\n"
    ...
    for layer in LAYERS:
        layer_root = caprmadio_root / APPLIED_LAYER_DIRECTORIES[layer]
        hub_root = (
            layer_root
...
def _materialize_package(
    ...
    destination = stage / ".caprmadio" / APPLIED_LAYER_DIRECTORIES["meta"]
    replacements = {
        "{{project_key}}": project_key,
        "{{package_id}}": package_id,
        "{{title}}": project_name,
