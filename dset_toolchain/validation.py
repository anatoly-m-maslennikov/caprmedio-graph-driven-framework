...
def validate_repository(root: Path) -> list[Diagnostic]:
    ...
    diagnostics.extend(
        _diag("CAPRMADIO-E157", selected_settings_path(root), issue)
        for issue in settings_issues
    )
    try:
        layout = discover_layout(root)
    except ValueError as error:
        return [_diag("CAPRMADIO-E001", root / ".caprmadio", str(error))]
    manifest_path = layout.manifest_path
    if not manifest_path.is_file():
        return [_diag("CAPRMADIO-E001", manifest_path, "project manifest is missing")]
    ...
    try:
        ...
        diagnostics.append(_diag("CAPRMADIO-E118", layout.caprmadio_root, str(error)))
        schema_paths = ()
    ...
    if not layout.separated:
        ...
        diagnostics.extend(
            _diag("CAPRMADIO-E168", transition_ledger, issue)
    ...
    if health_path(root).is_file() and not health_is_fresh(root):
        diagnostics.append(
            _diag("CAPRMADIO-E162", health_path(root), "project health is stale")
        )
...
def validate_change(
    ...
    *,
    ...
    if not manifest_path.is_file():
        return [_diag("CAPRMADIO-E102", manifest_path, "change manifest is missing")]
    ...
    if not CHANGE_PATTERN.fullmatch(change_slug) or change_slug != folder_id:
        diagnostics.append(
            _diag(
                "CAPRMADIO-E151" if layered else "CAPRMADIO-E101",
                manifest_path,
    ...
    if profile not in VALID_PROFILES:
        diagnostics.append(
            _diag("CAPRMADIO-E103", manifest_path, f"unknown change profile: {profile}")
        )
        return diagnostics
    if not layout.slim:
        try:
            ...
            diagnostics.append(_diag("CAPRMADIO-E103", manifest_path, str(error)))
            return diagnostics
        for relative in sorted(files):
            ...
            if not path.is_file():
                diagnostics.append(
                    _diag("CAPRMADIO-E104", path, "required artifact is missing")
                )
        for relative in sorted(directories):
            path = change_dir / relative
            if not path.is_dir():
                diagnostics.append(
                    _diag("CAPRMADIO-E104", path, "required artifact directory is missing")
                )
    ...
    if archived and status != "archived":
        diagnostics.append(
            _diag("CAPRMADIO-E108", manifest_path, "archive status must be archived")
        )
    if not archived and status == "archived":
        diagnostics.append(
            _diag("CAPRMADIO-E108", manifest_path, "archived change is in active root")
        )
    if archived:
        archive = data.get("archive")
        if not isinstance(archive, dict):
            diagnostics.append(
                _diag("CAPRMADIO-E108", manifest_path, "archive metadata is missing")
            )
        else:
            ...
            if expected and not (
                ...
                diagnostics.append(
                    _diag(
                        "CAPRMADIO-E150" if layered else "CAPRMADIO-E108",
                        manifest_path,
...
def _validate_project_manifest(
    ...
    if (
        ...
        diagnostics.append(
            _diag("CAPRMADIO-E115", path, "repository identity is inconsistent")
        )
    packages = data.get("packages", [])
    ids = [item.get("id") for item in packages if isinstance(item, dict)]
    if not ids or len(ids) != len(set(ids)):
        diagnostics.append(
            _diag("CAPRMADIO-E115", path, "package IDs must be non-empty and unique")
        )
    ...
    if "pending" in command.lower() or not command:
        diagnostics.append(
            _diag("CAPRMADIO-E116", path, "canonical command is not executable")
        )
    ...
    if data.get("schema_version") == 1.1 and registry not in {
        ...
        diagnostics.append(
            _diag("CAPRMADIO-E115", path, "schema 1.1 requires the central intake path")
        )
    elif isinstance(registry, str) and not (root / registry).is_file():
        diagnostics.append(
            _diag("CAPRMADIO-E115", root / registry, "work-item registry is missing")
        )
    ...
    if not isinstance(contracts, list) or any(
        ...
        diagnostics.append(
            _diag("CAPRMADIO-E115", path, "project contract IDs are inconsistent")
        )
    for group, trace_type in (("stories", "STORY"), ("outcomes", "OUTCOME")):
        ...
        if not isinstance(identifiers, list) or any(
            ...
            diagnostics.append(
                _diag("CAPRMADIO-E115", path, f"project {group} IDs are inconsistent")
            )
    return diagnostics
...
def _validate_intake_registry(root: Path, manifest: dict[str, Any]) -> list[Diagnostic]:
    ...
    if layered:
        ...
        if data.get("scope_mode") != "multi-scope":
            diagnostics.append(
                _diag("CAPRMADIO-E142", path, "intake must use registered layer scopes")
            )
        raw_scopes = data.get("scopes")
        if not isinstance(raw_scopes, list):
            diagnostics.append(_diag("CAPRMADIO-E142", path, "scopes must be a list"))
            return diagnostics
        scopes = {}
        ...
        for raw_scope in raw_scopes:
            if not isinstance(raw_scope, dict):
                diagnostics.append(
                    _diag("CAPRMADIO-E142", path, "every scope must be a mapping")
                )
                continue
            ...
            if (
                ...
                diagnostics.append(
                    _diag("CAPRMADIO-E142", path, f"invalid layer scope: {scope_id}")
                )
                continue
    ...
    if not isinstance(raw_items, list):
        diagnostics.append(_diag("CAPRMADIO-E142", path, "items must be a list"))
        return diagnostics
    ...
    for raw_item in raw_items:
        if not isinstance(raw_item, dict):
            diagnostics.append(
                _diag("CAPRMADIO-E142", path, "every intake item must be a mapping")
            )
            continue
        identifier = raw_item.get("id")
        match = (
        ...
        if (
            ...
            diagnostics.append(
                _diag("CAPRMADIO-E142", path, f"invalid intake identity: {identifier}")
            )
        ...
        if (
            ...
            diagnostics.append(
                _diag("CAPRMADIO-E142", path, f"invalid Decision identity: {decision}")
            )
    return diagnostics
...
def _validate_version(
    ...
    if layout.recursive or layout.separated:
        try:
            ...
            diagnostics.append(_diag("CAPRMADIO-E124", path, str(error)))
            data = None
    else:
        data = _safe_load(path, diagnostics)
    if not data:
        return diagnostics or [_diag("CAPRMADIO-E124", path, "version contract is missing")]
    ...
    if (
        ...
        diagnostics.append(
            _diag("CAPRMADIO-E124", path, "framework version contract is inconsistent")
        )
    if (
        ...
        diagnostics.append(
            _diag("CAPRMADIO-E124", path, "Python package version contract is inconsistent")
        )
    if (
        ...
        diagnostics.append(
            _diag("CAPRMADIO-E124", path, "schema version contract is inconsistent")
        )
    commit = released.get("commit") if isinstance(released, dict) else None
    if not isinstance(commit, str) or not re.fullmatch(r"[0-9a-f]{40}", commit):
        diagnostics.append(
            _diag("CAPRMADIO-E124", path, "released validator commit must be a full SHA")
        )
    if data.get("schema_version") != "1.2" or released.get("assurance") not in {
        ...
        diagnostics.append(
            _diag("CAPRMADIO-E124", path, "validator assurance contract is inconsistent")
        )
    return diagnostics
...
def _validate_packages(root: Path, manifest: dict[str, Any]) -> list[Diagnostic]:
    ...
    for package in manifest.get("packages", []):
        ...
        if not package_manifest.is_file():
            diagnostics.append(
                _diag("CAPRMADIO-E117", package_manifest, "package manifest is missing")
            )
            continue
        ...
        if data.get("id") != package.get("id"):
            diagnostics.append(
                _diag("CAPRMADIO-E117", package_manifest, "package ID mismatch")
            )
        artifacts = data.get("artifacts", {})
        if not isinstance(artifacts, dict):
            diagnostics.append(
                _diag("CAPRMADIO-E117", package_manifest, "artifacts must be a mapping")
            )
            continue
        for relative in artifacts.values():
            ...
            if not path.is_file():
                diagnostics.append(
                    _diag("CAPRMADIO-E117", path, "package artifact is missing")
                )
        owner_paths = {
        ...
        for group, trace_type in group_types.items():
            ...
            if not isinstance(identifiers, list):
                diagnostics.append(
                    _diag("CAPRMADIO-E117", package_manifest, f"{group} must be a list")
                )
                continue
            pattern = _trace_id_pattern(project_key, (trace_type,))
            content = "\n".join(
...
def _validate_layered_packages(
    ...
    for path in layout.package_fragments():
        ...
        try:
            ...
            diagnostics.append(
                _diag("CAPRMADIO-E145", path, "package fragment is outside layer roots")
            )
            continue
        ...
        if not isinstance(artifacts, dict) or artifacts != artifact_names:
            diagnostics.append(
                _diag("CAPRMADIO-E144", path, "package fragment artifacts are malformed")
            )
            artifacts = {}
        for name, filename in artifact_names.items():
            ...
            if artifacts.get(name) == filename and not artifact.is_file():
                diagnostics.append(
                    _diag("CAPRMADIO-E144", artifact, "package fragment artifact is missing")
                )
        for group, trace_type in group_types.items():
            identifiers = data.get(group)
            if not isinstance(identifiers, list) or len(identifiers) != len(
                ...
                diagnostics.append(
                    _diag("CAPRMADIO-E144", path, f"{group} must be a unique list")
                )
                continue
            pattern = _trace_id_pattern(project_key, (trace_type,))
            ...
            for identifier in identifiers:
                ...
                if match is None:
                    diagnostics.append(
                        _diag("CAPRMADIO-E144", path, f"invalid {group} ID: {identifier}")
                    )
                    continue
    ...
    for package_id, layer in sorted(expected - found):
        ...
        diagnostics.append(
            _diag("CAPRMADIO-E144", missing_path, "declared package fragment is missing")
        )
    return diagnostics
...
def _validate_artifacts(
    ...
    *,
    ...
    if not isinstance(profiles, dict):
        return [_diag("CAPRMADIO-E120", manifest_path, "profiles must be a mapping")]
    ...
    try:
        ...
        diagnostics.append(_diag("CAPRMADIO-E120", registry_path, str(error)))
        registry = {}
    if not registry:
        return diagnostics
...
def validate_artifact_type_registry(
    ...
    *,
    ...
    if str(data.get("schema_version")) != "1.0":
        diagnostics.append(
            _diag("CAPRMADIO-E156", registry_path, "schema_version must be 1.0")
        )
    if data.get("profile") != "documentation-v1":
        diagnostics.append(
            _diag("CAPRMADIO-E156", registry_path, "profile must be documentation-v1")
        )
...
def _artifact_type_catalog(
    ...
    for entry in raw_types:
        if not isinstance(entry, dict):
            diagnostics.append(
                _diag(
                    "CAPRMADIO-E156", registry_path, "every artifact type must be a mapping"
                )
            )
            continue
...
def _artifact_exclusions(
    ...
    for item in raw_exclusions:
        ...
        if not isinstance(pattern, str) or not pattern.strip():
            diagnostics.append(
                _diag("CAPRMADIO-E156", registry_path, "exclusion pattern is missing")
            )
            continue
        if pattern in seen:
            diagnostics.append(
                _diag("CAPRMADIO-E156", registry_path, f"duplicate exclusion: {pattern}")
            )
        seen.add(pattern)
    ...
    *,
    ...
    for identity in raw_identities:
        if not isinstance(identity, dict):
            diagnostics.append(
                _diag("CAPRMADIO-E156", registry_path, "retained_for must be a mapping")
            )
            continue
        ...
        if not isinstance(value, str) or not value:
            diagnostics.append(
                _diag("CAPRMADIO-E156", registry_path, f"retained_for {key} is invalid")
            )
            continue
        marker = (key, value)
        if marker in seen:
            diagnostics.append(
                _diag("CAPRMADIO-E156", registry_path, f"duplicate retained_for: {value}")
            )
        seen.add(marker)
        if key == "semantic_id":
    ...
    for carrier, _text in markdown_texts.items():
        ...
        for target in targets:
            ...
            if (
                ...
                and (
                    _within_root(root / ".caprmadio", target)
...
def _validate_artifact_path_rules(
    ...
    *,
    ...
    if not isinstance(raw_rules, list) or not raw_rules:
        return [
            _diag("CAPRMADIO-E156", registry_path, "path_rules must be a non-empty list")
        ]
    ...
    for rule in raw_rules:
        if not isinstance(rule, dict):
            diagnostics.append(
                _diag("CAPRMADIO-E156", registry_path, "every path rule must be a mapping")
            )
            continue
        ...
        if not isinstance(pattern, str) or not pattern.strip():
            diagnostics.append(
                _diag("CAPRMADIO-E156", registry_path, "path rule pattern is missing")
            )
            continue
        if pattern in seen_patterns:
            diagnostics.append(
                _diag("CAPRMADIO-E156", registry_path, f"duplicate path rule: {pattern}")
            )
...
def validate_artifact_registry(
    ...
    if not isinstance(root_entry, dict):
        return [_diag("CAPRMADIO-E121", registry_path, "root must be a mapping")]
    if not isinstance(raw_areas, list) or not raw_areas:
        return [_diag("CAPRMADIO-E121", registry_path, "areas must be a non-empty list")]
    areas = [item for item in raw_areas if isinstance(item, dict)]
    if len(areas) != len(raw_areas):
        diagnostics.append(
            _diag("CAPRMADIO-E121", registry_path, "every area must be a mapping")
        )
...
def _validate_artifact_entries(
    ...
    for entry in entries:
        ...
        if not isinstance(identifier, str) or not CHANGE_PATTERN.fullmatch(identifier):
            diagnostics.append(
                _diag("CAPRMADIO-E121", registry_path, f"invalid artifact ID: {identifier}")
            )
        elif identifier in seen_ids:
            diagnostics.append(
                _diag(
                    "CAPRMADIO-E121", registry_path, f"duplicate artifact ID: {identifier}"
                )
            )
        else:
            seen_ids.add(identifier)
        ...
        if area_root is None:
            ...
            if not area_root.is_dir():
                diagnostics.append(
                    _diag("CAPRMADIO-E121", area_root, "artifact root does not exist")
                )
        if hub is None:
            diagnostics.append(
                _diag(
            ...
            seen_hubs.add(hub)
            if not hub.is_file():
                diagnostics.append(_diag("CAPRMADIO-E121", hub, "artifact hub is missing"))
        if area_root is not None and hub is not None:
            try:
                ...
                diagnostics.append(
                    _diag(
                        "CAPRMADIO-E121", registry_path, "artifact hub is outside its root"
                    )
                )
    return diagnostics
...
def _validate_artifact_hubs(
    ...
    for entry in [root_entry, *areas]:
        ...
        if not required.issubset(headings):
            diagnostics.append(
                _diag("CAPRMADIO-E123", hub, "hub requires Purpose and Boundaries sections")
            )
        ...
        if headings.isdisjoint(navigation):
            diagnostics.append(
                _diag("CAPRMADIO-E123", hub, "hub requires a navigation section")
            )
        named_atoms = sorted(
            semantic_id
        ...
        if runtime_descendants:
            diagnostics.append(
                _diag(
                    "CAPRMADIO-E123",
                    hub,
                    "hub must not include .caprmadio_runtime descendants: "
...
def _validate_change_ids(
    ...
    if project_key is None:
        diagnostics.append(
            _diag("CAPRMADIO-E106", manifest_path, "project.key is unavailable")
        )
        return diagnostics
    ...
    if legacy:
        ...
        if "contracts" not in data:
            diagnostics.append(
                _diag("CAPRMADIO-E106", manifest_path, "schema 1.1 requires contracts")
            )
        groups["contracts"] = [
            *list((change_dir / "specs").glob("*.md")),
    ...
    for group, paths in groups.items():
        ...
        if not isinstance(ids, list):
            diagnostics.append(
                _diag("CAPRMADIO-E106", manifest_path, f"{group} must be a list")
            )
            continue
        pattern = (
            ID_PATTERN
    ...
    if not isinstance(intake_ids, list):
        diagnostics.append(_diag("CAPRMADIO-E106", manifest_path, "intake must be a list"))
    else:
...
def _validate_slim_change_ids(
    ...
    if project_key is None:
        return [_diag("CAPRMADIO-E106", manifest_path, "project.key is unavailable")]
    atoms, _ = collect_semantic_atoms(root)
    ...
    for group, kinds in groups.items():
        ...
        for identifier in values:
            if not isinstance(identifier, str) or pattern.fullmatch(identifier) is None:
                diagnostics.append(
                    _diag("CAPRMADIO-E106", manifest_path, f"invalid ID: {identifier}")
                )
            elif identifier not in known:
                diagnostics.append(
                    _diag(
                        "CAPRMADIO-E106", manifest_path, f"unknown governed ID: {identifier}"
                    )
                )
    if not data.get("requirements") or not data.get("tests"):
        diagnostics.append(
            _diag("CAPRMADIO-E106", manifest_path, "requirements and tests cannot be empty")
        )
...
def _validate_layered_change(
    ...
    for group in ("contracts", "stories", "outcomes"):
        if not isinstance(data.get(group), list):
            diagnostics.append(
                _diag("CAPRMADIO-E148", path, f"schema 1.2 requires {group} as a list")
            )
    project_key = _manifest_project_key(project)
    if project_key is None:
        return diagnostics
    ...
    if isinstance(release, dict):
        ...
        if policy is not None and policy != expected_policy:
            diagnostics.append(
                _diag("CAPRMADIO-E153", path, "schema 1.2 release policy path is invalid")
            )
        if owner is not None:
            ...
            if owner_match is None or owner_match.group("layer") is None:
                diagnostics.append(
                    _diag("CAPRMADIO-E153", path, "release owner_change must be stable")
                )
...
def _validate_workspace(
    ...
    if not isinstance(workspace, dict):
        return [_diag("CAPRMADIO-E152", path, "workspace metadata is required")]
    ...
    if not valid:
        return [_diag("CAPRMADIO-E152", path, "workspace metadata is inconsistent")]
    release = data.get("release")
...
def _validate_change_dependencies(
    ...
    if not isinstance(dependencies, list):
        return [_diag("CAPRMADIO-E153", path, "dependencies must be a list")]
    diagnostics: list[Diagnostic] = []
    seen: set[str] = set()
    ...
    for dependency in dependencies:
        if not isinstance(dependency, dict) or set(dependency) != required:
            diagnostics.append(
                _diag("CAPRMADIO-E153", path, "dependency record shape is inconsistent")
            )
            continue
        ...
        if not valid:
            diagnostics.append(
                _diag("CAPRMADIO-E153", path, f"invalid dependency: {dependency_id}")
            )
...
def _validate_schemas(schema_paths: tuple[Path, ...]) -> list[Diagnostic]:
    ...
    for path in schema_paths:
        try:
            ...
            diagnostics.append(_diag("CAPRMADIO-E118", path, str(error)))
    return diagnostics
...
def _validate_provenance(root: Path) -> list[Diagnostic]:
    ...
    try:
        ...
        diagnostics.append(_diag("CAPRMADIO-E112", path, str(error)))
        data = {}
    if not data:
        return diagnostics or [_diag("CAPRMADIO-E112", path, "provenance is missing")]
    for source in data.get("sources", []):
        ...
        if not re.fullmatch(r"[0-9a-f]{40}", revision):
            diagnostics.append(
                _diag("CAPRMADIO-E112", path, "source revision must be a full commit SHA")
            )
        if not license_path.is_file():
            diagnostics.append(
                _diag("CAPRMADIO-E112", license_path, "retained license is missing")
            )
    return diagnostics
...
def _validate_markdown(root: Path, layout: RepositoryLayout) -> list[Diagnostic]:
    ...
    for path in _markdown_paths(root):
        ...
        if "[[" in rendered or "![[" in rendered:
            diagnostics.append(
                _diag("CAPRMADIO-E114", path, "Obsidian wiki links are not portable")
            )
        ...
        if details_open != rendered.count("</details>"):
            diagnostics.append(_diag("CAPRMADIO-E114", path, "unbalanced details element"))
...
def _project_visible_files(root: Path) -> list[Path]:
    ...
    return sorted(
        ...
        if path.is_file()
        and path.relative_to(root).parts[:1] != (".caprmadio_runtime",)
        and path.relative_to(root).parts[:2] != (".caprmadio", "runtime")
        and not any(
...
def _active_applied_files(root: Path) -> list[Path]:
    ...
    if layout.separated:
        owners = (
            *(layout.caprmadio_root / role for role in CONTENT_ROLE_DIRECTORIES),
...
def _safe_load(path: Path, diagnostics: list[Diagnostic]) -> dict[str, Any] | None:
    ...
    try:
        ...
        diagnostics.append(_diag("CAPRMADIO-E119", path, str(error)))
        return None
    if not isinstance(data, dict):
        diagnostics.append(_diag("CAPRMADIO-E119", path, "root must be a mapping"))
        return None
    return data
