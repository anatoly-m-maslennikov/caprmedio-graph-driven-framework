...
Commands inspect or preview one repository-local ``caprmadio_settings.toml``.
...
def _matching_carriers(root: Path, surface_id: str) -> list[str]:
    ...
    matches: list[str] = []
    for path in sorted((root / ".caprmadio").rglob("*")):
        if (
            not path.is_file()
...
def _settings_error(path: Path, message: str) -> DsetCommandError:
    ...
    return DsetCommandError("CAPRMADIO-E173", path, message)
