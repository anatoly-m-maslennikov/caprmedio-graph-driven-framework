...
|---|---|
| `00_Tool Development Playbook.md` | Karpathy + Anthropic: natural-language programming raises the value of a stage pipeline; fast agentic implementation must be wrapped in spec → deterministic test case plus applicable evaluation case → implementation plan → implementation → gates. |
...
| `02_Test and Evaluation Case Patterns — Proof Artifact Conventions.md` | Anthropic + Shankar + ABTest: give agents runnable checks, assertions, behavior tests, evaluator calibration, and pinning tests before trusting generated code. superpowers sharpens this into red/green evidence and verification-before-completion. |
...
- **Separate test and evaluation cases for LLM-facing tools** — folded into `00_Tool Development Playbook.md` and `02_Test and Evaluation Case Patterns — Proof Artifact Conventions.md`: deterministic checks stay in the test case; LLM assertions, evaluator calibration, adversarial/drift cases, and budgets stay in the evaluation case.
- **Agent can verify its own work** — folded into `01_Spec Authoring Patterns — Service Spec Conventions.md`, `02_Test and Evaluation Case Patterns — Proof Artifact Conventions.md`, `03_Implementation Plan Patterns — Service Build Conventions.md`, `04_General Build Rules — Tool Code Conventions.md`, and `05_Layered Build Standard — DDD, TDD, Small Functions, Typed Gates.md` as runnable checks, milestone gates, and canonical verification commands.
