...
| `02_Test and Evaluation Case Patterns — Proof Artifact Conventions.md` | How to design proof before code in two linked artifacts: a deterministic test case, including supportability-contract tests, and a probabilistic/qualitative evaluation case, including operator-usefulness evals | You need to answer “how will we know this works?” before implementation | The business/domain spec itself, rollout sequencing, or code architecture rules |
...
| 2 | **Test case + evaluation case** | The *proof design*: deterministic unit→integration→E2E and supportability-contract checks in `test-plan.md`; probabilistic, qualitative, LLM, or operator-usefulness proof in `eval-plan.md`; both authored WITH the spec, before code | `02_Test and Evaluation Case Patterns — Proof Artifact Conventions.md` |
...
- **Ordering is load-bearing**: the test case and applicable evaluation case are written *with* the spec, before code — writing either after the fact produces proof that follows the implementation instead of pinning the intent.
