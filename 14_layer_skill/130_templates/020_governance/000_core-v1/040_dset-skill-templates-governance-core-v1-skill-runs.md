...
ignored `.carmadio_runtime/runs/`. The filename is
...
ignored `.carmadio_runtime/sessions/<session-id>.json`. A checkpoint contains the same
...
`.carmadio_runtime/runs/`; the checkpoint is only a recovery index over those runs and the
