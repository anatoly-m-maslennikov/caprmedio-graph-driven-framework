...
ignored `.caprmadio_runtime/runs/`. The filename is
...
ignored `.caprmadio_runtime/sessions/<session-id>.json`. A checkpoint contains the same
...
`.caprmadio_runtime/runs/`; the checkpoint is only a recovery index over those runs and the
