...
`((all active and draft Atoms where Current Scope in (META_METHODOLOGY, METAMODEL, SEMANTICS, GOVERNANCE)) union (the Project-owned META_METHODOLOGY Goal) union (all active Plans that govern BSEED reconciliation or migration) union (all current authority, configuration, Journals, Projections, installed components, and runtime classifications under .caprmedio, .caprmedio_install, and .caprmedio_runtime))`
