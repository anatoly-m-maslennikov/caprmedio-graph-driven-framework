...
the AI Agent **must** deliver a tested programmatic Tool that checks source Atom Carriers selected by Scope Unit, optional descendants, Global Tiers, Local Tiers, **or** an explicit Atom list against the current applicable Property **and** Carrier authority, **without** reconstructing Atom Properties from filename **or** directory placement.

## Definition of Done

the Plan is **not** Done **if** an applicable machine-checkable rule lacks a check **or** an explicit unsupported-rule error, **or** a required Property is accepted through filename **or** placement fallback, **or** invalid fixture cases pass, **or** valid fixture cases fail, **or** invocation **and** diagnostic results cannot be reproduced, **or** the mocked end-to-end suite is missing, unexecuted, skipped, **or** fails, **or** its rule/error coverage matrix has unexplained gaps.

## Details

- implement **=1** Action, Check Atoms under CA-O-087. CA-O-080 invokes it through Step CA-O-088. O remains the operational source of truth; realize the Tool through CA-R-1622–1623, CA-M-316, CA-E-515–518, **and** CA-D-491–493. methodology Evaluation CA-E-513 checks operational definitions, **not** the Tool implementation.
...
- accept an explicit selector, bounded source inventory, applicable methodology, **and** declared Project Structure context **when** needed. use internal Scope Unit/tier values **and** declared parentage, **not** physical paths; report unresolved selector membership. return deterministic machine-readable findings with source locator, declared identity **when** present, Property/section, rule reference, severity, **and** failure reason; do **not** silently skip malformed files.
- build the end-to-end suite under the delivered `VALIDATE_ATOMS/tests/` layout in CA-D-491. invoke the real Tool against mock Atoms **and** context; cover applicable roles, Types, Statuses, Drafts, Plan file/folder bundles, unknown Properties, broken YAML, body/frontmatter duplication, **and** one-way Relations. include isolated defects, multiple independent defects, mixed good/bad batches, all selector modes, source bindings, **and** safe failure cases.
- maintain the rule/error-to-fixture coverage matrix **and** reviewed expected results required by CA-E-515. good Atoms **must** pass with no spurious findings; bad Atoms **must** report **all** expected detectable errors correctly. reject false negatives, false positives, incorrect targets/reasons, fabricated downstream errors, **and** incorrect coverage **or** exit codes. mock inputs **and** external boundaries, **not** validation logic; missing implementation **must not** become a skipped green suite.
