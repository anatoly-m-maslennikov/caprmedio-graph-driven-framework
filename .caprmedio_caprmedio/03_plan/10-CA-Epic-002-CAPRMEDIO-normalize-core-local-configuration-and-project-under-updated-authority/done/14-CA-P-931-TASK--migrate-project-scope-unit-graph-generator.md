...
subjects:
  ...
  depends_on:
    occurrent:
      - CA-P-930
version: 3
updated_at: 2026-08-30 20:24:09 +0400
...
The generator now discovers 13 active typed Scope Unit Directory Carriers under `.caprmedio_caprmedio`, derives each parent edge from its nearest typed Scope Unit ancestor, and excludes Epic and inactive lifecycle carriers. It derives actual Structural Level from typed Scope Unit ancestor depth, allocates the numeric-prefix Structural Level width dynamically from the greatest current level, and emits the remaining digits as Navigational Order Number; Layer Local Order remains separate. It follows the current one-hyphen Epic Directory Carrier grammar, binds the nested Local Configuration Project Configuration carrier at revision 4, emits lowercase `caprmedio` Project identity, and does not write registered Projections. Six focused tests pass; full execution evidence is stored in `execution_evidence/CA-P-931-project-scope-unit-graph-generator-migration.projection.json`.
