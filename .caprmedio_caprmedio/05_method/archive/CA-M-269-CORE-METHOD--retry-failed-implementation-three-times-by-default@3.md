...
**to** recover from a failed implementation Evaluation, use the Operator-configured retry policy **or**, **when** no override applies, allow **`<=3`** additional fix-and-evaluate retries **after** the initial failed evaluation. the initial failure does **not** consume a retry. stop on successful completion under CA-O-016; **if** failure remains **after** the third retry under the default policy, **then** pause **and** report the remaining failures **to** the Operator. retry permission **must not** override the Task confidence threshold, governing-Atom approval rules, **or** other Operator constraints; those rules **may** require earlier escalation. a changed failure set **or** automatic next loop **must not** silently reset the retry budget.
