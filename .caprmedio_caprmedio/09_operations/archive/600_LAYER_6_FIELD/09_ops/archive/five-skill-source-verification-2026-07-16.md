...
- **Intended use:** Support the source-wrapper portion of `CAPRMEDIO-SPEC-SKILLS-METH-055--select-the-dset-0-3-operating-shape`; it does not prove the runtime adapter, release publisher, declared agent hosts, or native operating-system matrix.
- **LLM session IDs:**
...
The repository-native source skill surface is implemented. `CAPRMEDIO-TASK-TOOL-010` remains open for the runtime adapter, run/checkpoint persistence, executable release/version mechanics, and publication automation. `CAPRMEDIO-TASK-SKILL-020` remains open for real installation, discovery, load, invocation, handoff, and stop proof on every declared agent host. Cross-platform execution proof remains separately open.
