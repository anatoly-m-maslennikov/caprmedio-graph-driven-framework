...
- The active proposal now uses `CAPRMEDIO-CHANGE-SKILL-001` and does not claim current
  hosted proof.
...
| Generated temporary adopter | Partial pass | Candidate still proves legacy schema 1.1 compatibility and local rule mutation only. `CAPRMEDIO-SPEC-TOOLS-CONC-070--generated-adopter-does-not-exercise-current-schema-1-2` requires a separate current-schema 1.2 adopter. |
| `obsidian-your-harness` | Not adopted | Candidate returns `CAPRMEDIO-E900`; no DSET root exists. Existing `CLAUDE.md`/spec surfaces retain authority, Test/Eval ownership is not migrated, hosted workflows are disabled, and no DSET delivery/supportability lifecycle exists. This is `CAPRMEDIO-GOV-CONC-032--the-declared-your-harness-pilot-has-not-adopted-dset`, not a failed rollout claim. |
| Local upstream Claudian checkout | Not started as designed | Candidate returns `CAPRMEDIO-E900`; the checkout is a local feature branch rather than the future clean pinned upstream fixture. Roadmap §7 correctly defers it until after the owned pilot and prohibits upstream writes. |

## Intake disposition

Open blocking Problems are `CAPRMEDIO-SPEC-TOOLS-CONC-069--released-validator-cannot-parse-schema-1-2..002`,
`CAPRMEDIO-SPEC-SKILLS-CONC-076--core-skill-runtime-is-incomplete`, `CAPRMEDIO-FIELD-CONC-089--current-exact-head-hosted-proof-is-missing`, and
`CAPRMEDIO-GOV-CONC-032--the-declared-your-harness-pilot-has-not-adopted-dset`. The re-audit also registered
`CAPRMEDIO-GOV-CONC-043--should-proof-dependency-closures-become-machine-readable` for machine-readable proof closures and optional
Opportunities `CAPRMEDIO-GOV-CONC-028--generate-a-proof-currentness-review-view`, `CAPRMEDIO-SPEC-TOOLS-CONC-068--expand-schema-template-validation-into-a-compatibility-matrix`, and
`CAPRMEDIO-FIELD-CONC-088--generate-a-cross-repository-adoption-status-view` for generated currentness, compatibility, and
