...
- **Intended use:** Support `CAPRMEDIO-GOV-METH-018--fpf-aligned-boundaries-for-the-flat-type-model` and the compiled
  `CAPRMEDIO-REQUIREMENT-GOV-027`; not claim FPF conformance or completed runtime
  enforcement.
...
- **Unsupported uses:** this proof does not import FPF ontology, identifiers,
  ...
  prove semantic equivalence; or close `CAPRMEDIO-GOV-CONC-035--canonical-flat-type-model-is-not-schema-enforced` and
  `CAPRMEDIO-TASK-GOV-049`.
...
|---|---|---|
| Does each Type change an action-facing governance rule? | Yes. Decision owns authority, Question unresolved knowledge/choice, Problem current insufficiency, and QA evaluation definitions. | Keep exactly four Types; do not promote Work, Evidence, Carrier, Verification, Change, or Release into new Types. |
...
The qualitative `CAPRMEDIO-EVALUATION-CASE-GOV-017` reviewer-agreement run remains a separate
