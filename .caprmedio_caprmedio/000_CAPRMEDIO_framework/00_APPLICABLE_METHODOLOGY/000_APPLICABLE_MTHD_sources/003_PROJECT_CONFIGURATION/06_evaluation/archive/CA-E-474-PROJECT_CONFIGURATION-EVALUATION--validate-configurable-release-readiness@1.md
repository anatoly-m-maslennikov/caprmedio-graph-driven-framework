...
cce_form: evaluation
subjects:
  governs: "Release Readiness/validation"
  depends_on:
    - "Release Readiness"
...
1. use a Project with no selected release-readiness Process; execute its admitted implementation mode under CA-O-016.
...
7. use a non-Git candidate binding satisfying the selected gates. inspect the selected Process's Action references **and** its dependency on the existing Core implementation flow.
...
- Process nodes reference Actions, the implementation loop is **not** duplicated, **and** non-Git Projects require no Git mechanics.
