...
3. the nearest enclosing Epic with an explicit Implementation Retry Limit **in** its active Objective Atom under CA-D-447. resolve the Objective by its target Epic, **not** its containing folder. **if** the Epic has no active Objective **or** the retry field is absent, inspect enclosing Epics from nearest **to** farthest; another explicit override field does **not** block this inheritance.
...
- **when** an Epic is reached **in** the precedence, multiple active Objectives for that Epic **or** invalid override metadata require Operator disposition **without** silently falling through.
