...
relations:
  - type: replacement_of
    targets:
      - CAPRMEDIO-GOV-METH-017--flat-canonical-type-and-subtype-model
---
...
record a Question when the ambiguity affects implementation, evaluation, or
...
conditions. An Evaluation atom defines an evaluation conclusion that depends on
...
## Authority, evaluation, and lifecycle
...
`CAPRMEDIO-GOV-CONC-035--canonical-flat-type-model-is-not-schema-enforced` and `CAPRMEDIO-TASK-GOV-049` remain open. Their schema and
runtime migration must now enforce both the flat model and these recognition
boundaries without editing legacy atoms. Existing `CAPRMEDIO-GOV-METH-017--flat-canonical-type-and-subtype-model`
...
Problem owns current insufficiency, and QA owns evaluation definitions. Adding
...
- **If reopened, retain:** four-way authority/uncertainty/insufficiency/
  evaluation separation, one direct subtype maximum, User Story as a direct
