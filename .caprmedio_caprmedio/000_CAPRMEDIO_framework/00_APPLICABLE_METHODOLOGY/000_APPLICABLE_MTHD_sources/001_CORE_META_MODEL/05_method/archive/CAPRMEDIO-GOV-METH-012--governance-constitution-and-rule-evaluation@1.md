...
# Decision — Governance constitution and rule evaluation

- **Resolves Question:** direct operator requirement following bounded FPF review
- **Selected option:** strengthen `CAPRMEDIO-RULE-ARCHITECTURE` as the sole governance constitution and separate rule authority, dependency, precedence, and evaluation
...
candidate review considered a new `CAPRMEDIO-RULE-RULE-AUTHORITY`, a new
`CAPRMEDIO-RULE-GOVERNANCE-OF-GOVERNANCE`, and an extension of the existing
...
- authority, applicability/currentness, provenance, transactional discharge,
  and evaluation are different questions;
- dependency and precedence are separate relations and both must remain
  acyclic;
- proof depth should be risk-proportionate;
- a new sibling root would duplicate `CAPRMEDIO-RULE-ARCHITECTURE` and complicate
...
- `CAPRMEDIO-REQUIREMENT-GOV-023` and `CAPRMEDIO-INVARIANT-GOV-012`;
- `CAPRMEDIO-TEST-CASE-GOV-023` and `CAPRMEDIO-EVALUATION-CASE-GOV-013`;
...
acyclic. Registry order never creates precedence. Missing or stale evaluation
...
- **If reopened, retain:** one repository-local owner, fail-closed resolution,
  authority/evaluation separation, and bounded proof currentness.
