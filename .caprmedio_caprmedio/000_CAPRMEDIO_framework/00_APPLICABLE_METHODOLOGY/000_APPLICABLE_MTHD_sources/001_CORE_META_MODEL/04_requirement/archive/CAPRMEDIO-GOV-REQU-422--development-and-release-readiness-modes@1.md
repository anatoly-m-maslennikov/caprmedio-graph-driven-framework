...
relations:
  - type: child_of
    targets:
      - CAPRMEDIO-GOV-REQU-406--on-demand-semantic-compilation
---
...
implementation, or applicable configuration change after an evaluation run
makes the affected development evidence stale for release. Release-readiness
evaluation runs against the exact candidate head.

## Settings and invocation

`caprmedio_settings.toml` records `development` as the default workflow mode and
