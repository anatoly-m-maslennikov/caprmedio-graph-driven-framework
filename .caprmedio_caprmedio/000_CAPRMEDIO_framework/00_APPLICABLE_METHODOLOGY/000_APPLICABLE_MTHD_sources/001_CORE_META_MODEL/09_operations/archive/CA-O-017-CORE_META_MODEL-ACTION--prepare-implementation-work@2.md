...
- return `methods_ready` **only** after required current-candidate checks pass **and** recorded Draft Method lessons remain for acceptance; return `complete` **only** after **all** selected Plan work, required child Plans, Definitions of Done, Method dispositions, **and** applicable Evaluations are satisfied, with no failed, blocked, stale, **or** unevaluated obligations.
