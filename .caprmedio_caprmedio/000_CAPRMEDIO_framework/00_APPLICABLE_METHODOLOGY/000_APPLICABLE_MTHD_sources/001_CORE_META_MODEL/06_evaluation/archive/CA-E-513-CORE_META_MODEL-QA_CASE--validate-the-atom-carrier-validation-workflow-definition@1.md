...
- require **=1** entry, resolvable Step references, **=1** Action per Step, explicit compatible input bindings, **and** admitted Workflow-scoped transitions.
- require reachable terminal outcomes for prepared, failed, incomplete, invalid, valid, **and** execution-error cases as defined by the referenced Actions; no outcome **may** be silently mapped **to** valid.
