...
- a node is **not** an Entity under its governing authority, including an Action **or** Workflow identity copied as an Entity.
...
the checks **must** also cover exact selected-source node **and** edge provenance, eligible omissions **and** outside-selection additions, repeated appearances of one identity, distinct qualified occurrences sharing a reusable Term, multiple governing Atoms for one Entity, Action **and** Workflow exclusion, a legitimate shared value admitted independently **in** two Property contexts, **and** explicit hierarchy filtering **without** a universal-tree assumption.
