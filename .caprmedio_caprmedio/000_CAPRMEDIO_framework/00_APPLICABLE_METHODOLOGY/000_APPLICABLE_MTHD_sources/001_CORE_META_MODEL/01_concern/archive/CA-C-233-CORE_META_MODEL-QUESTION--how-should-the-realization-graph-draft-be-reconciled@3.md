...
- Carrier: `.caprmedio_caprmedio/000_CAPRMEDIO_framework/00_APPLICABLE_METHODOLOGY/000_APPLICABLE_MTHD_sources/001_CORE_META_MODEL/04_requirement/drafts/CA-R--MMODEL-REQUIREMENT--define-realization-graph.md`.
...
the Operator confirmed the job: reverse engineering legacy Implementation into proposed RMED, followed by accepted RMED governing refactoring **and** Evaluation. the definition now permits legacy inputs **without** pre-existing RMED **or** Journals; exact selected inputs **and** available evidence remain traceable. the purpose decision is resolved; the specialized graph contract **and** promotion are **not** completed.
