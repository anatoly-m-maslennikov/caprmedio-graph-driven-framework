...
child_of = ["CAPRMEDIO-SPEC-SKILLS-REQU-567--entry-criteria-skill-chains"]
version = 1
...
- **Intended use:** Replace the earlier static wrapper/rule evidence for
  `CAPRMEDIO-SPEC-SKILLS-REQU-567--entry-criteria-skill-chains`, `CAPRMEDIO-SPEC-SKILLS-EVAL-051--lazy-implementation-preparation`, and
  `CAPRMEDIO-TASK-SKILL-024` with executable runtime evidence.
...
- **Unsupported uses:** this proof does not claim native-host usefulness,
  qualitative `CAPRMEDIO-SPEC-SKILLS-EVAL-049--implementation-preparation-modes`, automatic judgment of plan completeness,
...
| Observable progress | Each successful workflow owns one criterion transition; an unchanged observation stops with `CAPRMEDIO-RUNTIME-NO-PROGRESS` | Pass |
...
The older `CAPRMEDIO-FIELD-OPER-026--entry-criteria-skill-chains` remains immutable evidence of the static
