...
priority = "high"
child_of = ["CAPRMEDIO-SPEC-TOOLS-EVAL-044--typescript-candidate-evaluation-case"]
version = 1
...
`CAPRMEDIO-SPEC-TOOLS-EVAL-044--typescript-candidate-evaluation-case`; it does not promote the candidate.
...
2. Ran `dset init` as a no-write preview, then executed it only in a disposable
   copy. The first executable attempt exposed `CAPRMEDIO-SPEC-TOOLS-CONC-074--initializer-supportability`; after
...
fields. See
[`CAPRMEDIO-SPEC-TOOLS-ANRP-023--clean-upstream-typescript-boundary`](../CAPRMEDIO-SPEC-TOOLS-ANRP-023--clean-upstream-typescript-boundary.md).
