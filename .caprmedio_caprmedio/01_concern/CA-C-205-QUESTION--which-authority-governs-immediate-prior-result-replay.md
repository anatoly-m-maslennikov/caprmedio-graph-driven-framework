...
- `CA-E-198@13`: `.caprmedio_caprmedio/102_LAYER_2_FRAMEWORK_ENGINE/201_FEATURE_PROGRAMMATIC/201_FEATURE_TOOLS/06_evaluation/CA-E-198-TOOLS-QA_CASE--replay-previous-state-without-before-fields.md`; SHA-256 `4f51d7a4e6eb1f302ddbe87da0d8f18fc82cc0c9e97687289f69548e0e15e797`.
...
## First-baseline recovery consumer

E-200 still asks **to** run a first schema-v2 change **and** append `governed_file_state` with `previous_result_event`. D-009 instead selects v3 writes for file/folder events **and** read compatibility for accepted v2 records. GOV-REQU-340 establishes evidence-only recovery **and** explicit uncertainty, **not** the exact v3 predecessor encoding. preserve matching versus contradictory/insufficient evidence, a separate baseline, no invented facts **and** the required prior-result linkage while locating the precise admitted schema **and** replay owner.

do **not** mechanically rename event types, silently authorize new v2 writes, remove the predecessor safeguard **or** treat a read-compatibility test as proof of current write behavior. E-200's ordinary prose formatting can be corrected independently of this unresolved schema mapping.
