...
- test **every** admitted action Type, including state-dependent event fields. require structured `llm_session.app` **and** `llm_session.uuid`; derive the combined session display rather than storing it again. reject duplicate provenance, projected messages, **and** prior-state fields such as `action_message`, `before_path`, **and** `before_sha256` under the selected schema. keep `sources` ordered **and** `result` singular.
