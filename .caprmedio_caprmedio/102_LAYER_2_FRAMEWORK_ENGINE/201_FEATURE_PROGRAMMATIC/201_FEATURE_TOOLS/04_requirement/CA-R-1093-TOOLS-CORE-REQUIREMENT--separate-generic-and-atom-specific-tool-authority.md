...
Finders, including `ATOM_SEARCH` **and** `ATOM_READ`, are strictly read-only. Doers default **to** dry run **and** require explicit `--apply`. **every** Atom Doer accepts **=1** resolved target for an atomic action **or** a frozen set of two **or** more resolved targets for a bulk action; a bulk action preflights **every** target **and** either applies **all** targets **or** **none**.
