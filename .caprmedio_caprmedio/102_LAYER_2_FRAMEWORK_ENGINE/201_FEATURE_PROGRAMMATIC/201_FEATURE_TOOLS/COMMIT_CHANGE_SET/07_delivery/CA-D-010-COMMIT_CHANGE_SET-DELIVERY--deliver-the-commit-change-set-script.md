...
Realize `COMMIT_CHANGE_SET` through the canonical source script `102_FRAMEWORK_ENGINE/201_PROGRAMMATIC/201_TOOLS/COMMIT_CHANGE_SET/commit_change_set.py` **and** its content-identical carrier **in** the selected `.caprmedio_runtime/tools` release. It exposes the common Doer CLI **in** dry-run **and** authorized-MCP apply modes. It consumes **only** sealed peer results **and** **must not** import, invoke, schedule, **or** **otherwise** orchestrate `COMMIT_TRIGGER`, `COMMIT_CONTEXT`, **or** `APPEND_CHANGE_RECORDS`.
