...
subjects:
  ...
  depends_on:
    ...
    - "Artifact/Revision"
version: 15
updated_at: "2026-09-17 20:52:58 +0000"
llm_session_ids:
...
4. require exactly the `TYPE + ID`, `Summary`, **and** `Child of` columns. require **every** `REQU-NNN` identity **and** **every** direct `child_of` target **to** link **to** its canonical Atom, **and** require multiple parents **to** be numerically ordered **without** ancestor substitution.
...
reject the Catalog, retain the previous Projection bytes for recovery **without** treating retention as proof of currentness; classify those bytes against their own source frontier under CA-R-1062, report the first missing, extra, stale, misordered, malformed, **or** misclassified row, **and** record a Concern **before** publication.
