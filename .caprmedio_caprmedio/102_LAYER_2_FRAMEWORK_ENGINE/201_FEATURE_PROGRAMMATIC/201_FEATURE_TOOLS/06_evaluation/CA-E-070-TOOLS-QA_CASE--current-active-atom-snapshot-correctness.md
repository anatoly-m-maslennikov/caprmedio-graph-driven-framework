...
2. generate the snapshot **without** the malformed **and** duplicate carriers **and** require exactly `<project-control-root>/biz_atoms_current_snapshot.md`, the correct grand total, **every** registered Type, structural level, **and** structural unit, explicit zero counts, **and** **every** rollup sum equal **to** the grand total.
3. move an Atom between active **and** inactive registered lifecycle placements **and** require the total **and** **every** affected rollup **to** change by **=1** **without** reading lifecycle state from frontmatter.
