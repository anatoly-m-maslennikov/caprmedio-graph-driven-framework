...
START_BACKGROUND_SERVICES **must** be one Doer Tool owned immediately by TOOLS as an unordered_unit at Structural level 4, addressed by 102_FRAMEWORK_ENGINE/201_PROGRAMMATIC/201_TOOLS/START_BACKGROUND_SERVICES, **and** realized under that path. It **must** read one machine-readable registry from the selected installed release **and** expose status, pause, resume, stop, start, **and** reload for **every** enabled service.
