...
the generator uses the appropriate Settings revision for each exposed value **without** a Project-specific effective-value catalog **or** an Atom-owned current settings selection. output bindings identify the changed Settings Artifacts; the generator **and** the Projection **must not** become their authority.
