...
---
# Serialize admitted local commits through one logical gate

`COMMIT_CHANGE_SET` MUST own the only logical commit gate for one resolved repository. Trigger producers, schedulers, context gatherers, append workers, and retry workers MAY operate concurrently, but all staging and commit creation for admitted CAPRMEDIO commit actions pass through this one gate.
...
A Journal-only batch commit contains only canonical Journal carrier changes selected by the batcher, may batch completed records for multiple actions, and shares this same Git gate. The gate MUST never combine unrelated real-change actions, mix a real-change item with a Journal-only batch, or treat Journal append itself as a Git-gate operation. Interrupted work remains durable, idempotent, and recoverable through its outbox state and reconciliation without allowing a second commit-creating worker to bypass the gate.

The Tool admits exactly two Git effect classes: one local real-change commit and one local Journal-only commit. It MUST reject requests to create, delete, rename, switch, or otherwise mutate branches; select or configure an upstream or remote; fetch, pull, merge, or rebase; push or force-push; create tags or releases; or perform any other Git effect. Rejection occurs before staging, ref mutation, network access, or modification of repository configuration.
