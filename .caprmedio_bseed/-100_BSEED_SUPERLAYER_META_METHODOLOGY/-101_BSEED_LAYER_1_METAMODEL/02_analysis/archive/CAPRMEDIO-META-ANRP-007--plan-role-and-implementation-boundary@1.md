...
The real Implementation is the project realization outside `.caprmedio/`,
including its native code, configuration, executable evaluation mechanisms,
...
The specification segment remains Requirement, Method, Evaluation, and
Delivery. The model can therefore be read as `CAP · RMED · IO`:

- `CAP`: Concern, Analysis, Plan;
- `RMED`: Requirement, Method, Evaluation, Delivery; and
- `IO`: Implementation, Ops.

`CAPSIO` was considered as a shorter name by collapsing `RMED` into
...
work have different identities. CAPRMEDIO adapts that distinction by assigning
Change Plan to Plan, native project realization to Implementation, and factual
execution results to Ops.

## Conclusion

The framework should become CAPRMEDIO, add Plan as a first-class Content role,
