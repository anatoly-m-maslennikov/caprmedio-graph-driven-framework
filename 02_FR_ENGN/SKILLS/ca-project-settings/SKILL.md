...
3. Run `python3 02_FRAMEWORK_ENGINE/TOOLS/native_atom_revision.py framework-settings` from the repository root.
...
5. Run `python3 02_FRAMEWORK_ENGINE/TOOLS/generate_project_settings.py`.
...
When an effective setting or its ownership changes, update the owning RMED Atom's `project_settings` contribution through the applicable governed-change flow, then rebuild both Projections. When authority changes composition, TOML grammar, Map grammar, precedence, or validation mechanics, update the registered Tool implementation under `02_FRAMEWORK_ENGINE/TOOLS/`, register the changed Framework Settings Atom revision when applicable, and rebuild both Projections through the workflow above.
