...
## ADDED — CARMADIO-REQUIREMENT-GOV-002 One explicit package
...
**Scenario CARMADIO-SCENARIO-GOV-002:** The manifest resolves the `methodology` package to one current-truth root and reports `global_truth_root: null`.

## ADDED — CARMADIO-REQUIREMENT-META-001 Standard change artifact set
...
**Scenario CARMADIO-SCENARIO-META-001:** A structural check distinguishes the eight documents from the `specs/` and `proofs/` evidence directories.

## ADDED — CARMADIO-REQUIREMENT-OPS-001 PR-safe archival
...
**Scenario CARMADIO-SCENARIO-OPS-001:** With `Implementing PR: pending`, the bootstrap change remains under active `changes/` and cannot appear under `archive/`.

## ADDED — CARMADIO-REQUIREMENT-GOV-003 Honest enforcement status
...
**Scenario CARMADIO-SCENARIO-GOV-003:** Public metadata cannot imply that a documentation gate or canonical validator is active when no such executable asset exists.
