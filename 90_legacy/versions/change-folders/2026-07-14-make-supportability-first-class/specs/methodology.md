...
**Scenario CAPRMADIO-SCENARIO-META-002:** A contributor can distinguish the DSET expansion from the spec-loop artifact without dropping specifications or treating supportability as an optional post-release add-on.

## ADDED — CAPRMADIO-REQUIREMENT-META-002 Public identity is exact
...
**Scenario CAPRMADIO-SCENARIO-META-003:** Active public metadata contains the new identity and no active, non-historical document claims that DSET expands through “Spec.”

## ADDED — CAPRMADIO-REQUIREMENT-OPS-002 Production changes declare a supportability contract
...
**Scenario CAPRMADIO-SCENARIO-OPS-002:** A service incident can be investigated from its public failure or correlation ID without first adding ad hoc logging in production.

## ADDED — CAPRMADIO-REQUIREMENT-OPS-003 Runtime evidence is traceable to corrective intent
...
**Scenario CAPRMADIO-SCENARIO-OPS-003:** Given one reported production incident ID, an operator can identify the owning version and change and locate the proof that should reproduce or prevent recurrence.

## ADDED — CAPRMADIO-REQUIREMENT-OPS-004 Telemetry is safe and bounded
...
**Scenario CAPRMADIO-SCENARIO-OPS-004:** A support bundle is useful for diagnosis but contains no credentials, raw tokens, or unapproved personal data and cannot grow without a declared bound.

## ADDED — CAPRMADIO-REQUIREMENT-OPS-005 Supportability is profile- and topology-scaled
...
**Scenario CAPRMADIO-SCENARIO-OPS-005:** A one-shot local transform does not inherit distributed tracing, while an asynchronous multi-service workflow preserves correlation across retry and effect boundaries.

## ADDED — CAPRMADIO-REQUIREMENT-OPS-006 Support proof keeps tests and evals separate
...
**Scenario CAPRMADIO-SCENARIO-OPS-006:** An automated redaction assertion remains a test, while a timed incident-investigation exercise with a rubric remains an eval.
