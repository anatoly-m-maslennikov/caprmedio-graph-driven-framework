...
|---|---|---|---|---|
| Artifact-profile selection | `dset/dset.toml` | Reviewed repository change | Every validation run | DSET-TEST-CASE-GOV-007 |
| Governed area graph | `dset/artifacts.toml` | Reviewed repository change | Every validation run | `DSET-TEST-CASE-TOOL-006`, `DSET-TEST-CASE-GOV-008` |
| Artifact-type rules | `documentation/` normative docs | Reviewed methodology change | When rules change | `DSET-TEST-CASE-META-003`, `DSET-EVALUATION-CASE-META-002`, `DSET-EVALUATION-CASE-META-003` |
| Rationale | `documentation/rationale.md` | Reviewed methodology change | When reasoning changes | Link and navigation checks |
| Runtime evidence | GitHub checks and archived DSET proof | GitHub plus evidence-only commits | Per PR head | DSET-TEST-CASE-TOOL-008 |
