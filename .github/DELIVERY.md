...
5. The owner explicitly merges the pull request with the merge-commit method after the required checks pass.
6. GitHub updates `main` through that explicit merge.
...
Repository validation was explicitly suspended on 2026-08-10 so the current CAPRMADIO development line can be delivered while the self-hosting migration backlog is red. The DSET workflow and its required status check are removed for this interval. `PR policy` remains the sole required check; no workflow schedules or performs a merge.
