...
`CARMADIO-REQUIREMENT-003` assigns contextual governance defaults for this
...
| Local skill-run evidence | Ignored `.carmadio_runtime/runs/` records | Invoked skill/runtime | Every run; bounded retention | Schema/redaction tests and authoritative-state comparison |
...
- Use the exact schema 1.2 migration commit recorded in `10_versions/version.toml` as a temporary fixed-point baseline. It is explicitly not a released-validator substitute: `CARMADIO-PROBLEM-TOOL-001` remains open until a published compatible validator checks a later candidate or an accepted compatibility proof closes the gap.
...
- Store bounded skill-run evidence under ignored `.carmadio_runtime/runs/`; never let heuristics override accepted artifacts, Git history, or hosted state.
...
- **CARMADIO-QUESTION-GOV-001 — First-class Action entity** in `intake.toml`:
