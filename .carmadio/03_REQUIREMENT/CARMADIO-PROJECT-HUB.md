# Applied project-wide CARMADIO artifacts

## Purpose

Project-level carriers live directly in the ordered Content-role folders under
`.carmadio`. Structural scopes repeat the same role layout beneath their own
directories.

## Boundaries

Layer-specific truth belongs to the corresponding applied layer. The external
installed methodology remains isolated under `000_CARMADIO_METHODOLOGY`.
Native implementation remains outside `.carmadio`; only governed
Implementation carriers and journals belong under `07_IMPLEMENTATION`.

```mermaid
flowchart TB
    ROOT[".carmadio"]
    METHODOLOGY["000_CARMADIO_METHODOLOGY<br/>external installed methodology"]
...
- Installed methodology: `000_CARMADIO_METHODOLOGY`.
- Settings: `carmadio_settings.toml`.
