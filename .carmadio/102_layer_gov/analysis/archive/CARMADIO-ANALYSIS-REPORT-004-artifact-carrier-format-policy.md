...
artifact_id: CARMADIO-ANALYSIS-REPORT-004
...
relations:
  - type: analysis_of
    targets:
      - CARMADIO-REQUIREMENT-GOV-056
      - CARMADIO-REQUIREMENT-GOV-095
      - CARMADIO-CONSTRAINT-GOV-002
---
...
Immediately before this report was added, `.carmadio` contained:
...
These normally belong under `.carmadio_runtime` and are excluded from semantic
...
- Give every evergreen or maintained Markdown artifact valid YAML
  frontmatter.
- Convert identity-only `.carmadio/**/references/*.toml` carriers to
...
`CARMADIO-REQUIREMENT-GOV-056` currently requires TOML for all Python-owned DSET
...
That Requirement should replace `CARMADIO-REQUIREMENT-GOV-056`, specialize
`CARMADIO-REQUIREMENT-GOV-095`, preserve
`CARMADIO-CONSTRAINT-GOV-002`, and authorize a separate migration plan.
