...
artifact_id: CARMADIO-TEST-CASE-GOV-048
...
1. current governed state resolves only below `.carmadio/`;
2. resumable run, session, readiness, cache, and recovery state resolves below
   ignored `.carmadio_runtime/` and never becomes project authority;
