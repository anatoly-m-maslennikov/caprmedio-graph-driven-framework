...
artifact_id: CARMADIO-REQUIREMENT-GOV-043
...
- `.carmadio/` — committed DSET settings, governance, evergreen artifacts, atomic
  artifacts, Version records, schemas, templates, and generated views;
- `.carmadio_runtime/` — ignored, bounded, resumable project runtime state,
...
schema 1.3 `.carmadio/runtime/` location remain read-only compatibility inputs;
current initialization and writes use `.carmadio_runtime/`.
...
A DSET project separates committed control state under .carmadio, ignored resumable runtime state under .carmadio_runtime, and disposable scratch workspaces under the host temporary root.
