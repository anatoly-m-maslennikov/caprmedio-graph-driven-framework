...
artifact_id: CARMADIO-IMPL-GOV-008
...
Every CARMADIO skill begins at the target repository's `.carmadio` control root,
...
outside `.carmadio` may be located only after accepted authority identifies the
implementation subject.

## Primary claim

Skills resolve unique active identities only inside the current project's
`.carmadio`, exclude archives and legacy fallback, and never persist resolved
