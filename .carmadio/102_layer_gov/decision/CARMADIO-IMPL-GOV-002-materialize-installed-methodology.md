...
artifact_id: CARMADIO-IMPL-GOV-002
...
unidirectionally into `.carmadio/000_dset_methodology/` only by an explicit
...
selected project's `.carmadio/` control root.
...
`.carmadio/`, produced only by explicit one-way synchronization.
