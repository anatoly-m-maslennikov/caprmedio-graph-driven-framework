...
artifact_id: CARMADIO-REQUIREMENT-GOV-111
...
Every CARMADIO project uses four distinct storage boundaries:

| Boundary | Owned content | Revision behavior |
|---|---|---|
| `.carmadio/` | Governed Atoms and Projections, installed methodology, settings, tools, skill instructions, schemas, and templates | Atom revisions are Git-bound; Projections and configuration update through Git |
| `.carmadio_journal/` | Canonical running logs, event streams, audit trails, and ordered record sequences | Append-only NDJSON |
...
Nothing under `.carmadio_runtime` or the host temporary root may be the only copy
of governed truth, durable history, or canonical evidence. CARMADIO scratch uses
...
CARMADIO separates governed control state, canonical append-only journals,
