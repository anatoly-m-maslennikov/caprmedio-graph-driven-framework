...
artifact_id: CARMADIO-DECISION-GOV-028
...
`.carmadio` owns only current project-local DSET truth:
...
belong to the inert repository archive outside `.carmadio`.

Skills and current semantic compilation search only `.carmadio`. They do not
...
The project .carmadio tree contains only current settings, installed methodology, applied artifacts, and Version truth; historical aggregates, completed migration records, and compatibility snapshots live in the repository's inert legacy archive outside .carmadio and are never skill-discovery or current-compilation inputs.
