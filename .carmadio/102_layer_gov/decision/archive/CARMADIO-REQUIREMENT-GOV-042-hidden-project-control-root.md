...
artifact_id: CARMADIO-REQUIREMENT-GOV-042
...
- `.carmadio/<layer>/` — direct META, GOV, TOOL, SKILL, and OPS roots containing
  layer evergreen artifacts plus layer-owned atomic/context and proof records;
  and
- `.carmadio/runtime/` — ignored run, session, cache, migration-backup, and other
...
`.carmadio/gov/plan-tests.md`. They are never interpreted relative to `.carmadio/`.
...
A current DSET project owns its control plane under .carmadio, stores canonical paths relative to the repository root, and isolates ignored operational state under .carmadio/runtime.
