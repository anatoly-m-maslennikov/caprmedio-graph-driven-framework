...
artifact_id: CARMADIO-REQUIREMENT-GOV-046
...
`.carmadio/` contains only current, distributable DSET framework material plus the
...
A DSET framework repository separates the distributable framework surface under .carmadio from the repository's recursive self-hosting project artifacts at the repository root.
