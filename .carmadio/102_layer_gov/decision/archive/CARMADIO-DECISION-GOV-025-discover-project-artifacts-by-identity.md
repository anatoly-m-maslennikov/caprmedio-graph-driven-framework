...
artifact_id: CARMADIO-DECISION-GOV-025
...
relations:
  - type: replacement_of
    targets:
      - CARMADIO-DECISION-GOV-023
---

# Decision — Discover project artifacts by identity

Every DSET skill begins at the target repository's `.carmadio` control root. It
...
Implementation files outside `.carmadio` remain project content and may be located
...
that the project-local `.carmadio` edition—not a global installation, remote copy,
or root framework source—governs every run.

## Primary claim

CARMADIO-to-DSET references use unique identities only, and every skill discovers settings, methodology documents, evergreen artifacts, atomic artifacts, and lifecycle events by identity within the target repository's .carmadio control root without storing their physical carrier paths.
