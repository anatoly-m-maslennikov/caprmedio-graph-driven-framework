...
artifact_id: CARMADIO-DECISION-GOV-018
...
relations:
  - type: resolution_of
    targets:
      - CARMADIO-CONFLICT-GOV-001
  - type: replacement_of
    targets:
      - CARMADIO-DECISION-GOV-003
      - CARMADIO-DECISION-GOV-016
      - CARMADIO-DECISION-GOV-017
---
...
retain externally prescribed formats because they are not CARMADIO-owned artifact
encodings.

This Decision fully replaces `CARMADIO-DECISION-GOV-003`,
`CARMADIO-DECISION-GOV-016`, and `CARMADIO-DECISION-GOV-017`. It carries forward the
...
independent owners—especially `CARMADIO-DECISION-GOV-010` and
`CARMADIO-REQUIREMENT-GOV-020..026`—remain active. Only permanent byte identity and
...
Atomic semantics and identity are immutable, but an explicitly authorized lossless carrier migration may convert every CARMADIO-owned YAML artifact carrier to TOML under one transactional old-to-new digest ledger.
