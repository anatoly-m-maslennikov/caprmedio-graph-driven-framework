...
at `.carmadio/dset_settings.toml`; `.carmadio/artifact_catalog.toml` separately owns
...
1. Walk upward from the working location until exactly one project containing the unique settings carrier `dset_settings.toml` inside `.carmadio` is found. Nested competing project roots fail closed.
...
`.carmadio/000_dset_methodology/` as ordinary project-local files. Skills resolve
