...
- Define `CARMADIO-CONTRACT-SKILL-001` for real installable host-native skill artifacts and clean install/discovery/load/invocation proof on every declared agent host.
- Define `CARMADIO-CONTRACT-TOOL-001` for macOS, native Windows, WSL, and Linux behavior or explicitly narrower applicability proven before execution.
- Define `CARMADIO-CONTRACT-TOOL-002` for dependency allowlist/denylist, exact registry/version/license/provenance and lockfile authority, plus bounded authorized exceptions with expiry.
- Define `CARMADIO-CONTRACT-OPS-001` for real GitHub Actions workflow/run/check evidence bound to the actual implementing PR SHA and protected integration.
...
- Apply the flat semantic model under `CARMADIO-DECISION-GOV-007`; keep Conflict as
...
- Enforce current Change/intake/Decision/proof records with stable
  `CARMADIO-E155`, version and test run/checkpoint schemas, and backfill this
...
- Keep `CARMADIO-RULE-ARCHITECTURE` as the sole dependency-free governance root and
...
- Promote the project-local artifact classes, transactional discharge,
  ...
  profile version 0.3.
- Compile `CARMADIO-DECISION-GOV-002` into GOV requirements, tests, evals, schema,
...
- Migrate CARMADIO-owned structured artifacts and Markdown frontmatter to TOML.
...
## Batch 12 — Complete CARMADIO-owned TOML cutover — complete

- Replace byte-level immutability with semantic immutability under
  `CARMADIO-DECISION-GOV-018`; permit carrier changes only through a lossless,
...
- Convert every CARMADIO-owned standalone YAML artifact to an adjacent historical
  TOML envelope, and convert every CARMADIO-owned Markdown YAML frontmatter block
