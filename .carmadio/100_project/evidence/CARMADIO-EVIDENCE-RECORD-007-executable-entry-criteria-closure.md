...
artifact_id = "CARMADIO-EVIDENCE-RECORD-007"
child_of = ["CARMADIO-REQUIREMENT-SKILL-010"]
...
- **Intended use:** Replace the earlier static wrapper/rule evidence for
  `CARMADIO-REQUIREMENT-SKILL-010`, `CARMADIO-TEST-CASE-SKILL-012`, and
  `CARMADIO-TASK-SKILL-024` with executable runtime evidence.
...
- **Unsupported uses:** this proof does not claim native-host usefulness,
  qualitative `CARMADIO-EVALUATION-CASE-SKILL-010`, automatic judgment of plan completeness,
...
| Observable progress | Each successful workflow owns one criterion transition; an unchanged observation stops with `CARMADIO-RUNTIME-NO-PROGRESS` | Pass |
...
The older `CARMADIO-EVIDENCE-RECORD-004` remains immutable evidence of the static
