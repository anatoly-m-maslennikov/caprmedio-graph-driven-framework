...
artifact_id = "CARMADIO-EVIDENCE-RECORD-019"
priority = "high"
child_of = ["CARMADIO-REQUIREMENT-SKILL-011"]
...
|---|---|---|
| `CARMADIO-DEFECT-GOV-001` | Validate lifecycle transitions, terminal states, and one absorption successor | `df42c5a` |
| `CARMADIO-DEFECT-GOV-002` | Accept only digest-bound ID-owned claim fragments; keep semantic equivalence in Evaluation | `c0ee76a` |
| `CARMADIO-GAP-GOV-001` | Seal legacy Decision carriers and shared-registry selector fragments | `5500d87` |
| `CARMADIO-DEFECT-GOV-003` | Make atom sealing repeat repository-backed admission | `14c8331` |
| `CARMADIO-DEFECT-TOOL-001` | Preserve nested active runs and require wrapper terminalization | `540af58`, `8f85e3e` |
| `CARMADIO-GAP-SKILL-001` | Render an exact installed interpreter/runtime launcher | `f71a42f` |
| `CARMADIO-DEFECT-SKILL-001` | Carry and validate host session provenance through every wrapper | `8f85e3e` |
| `CARMADIO-GAP-TOOL-001` | Validate Decision-backed commit provenance from a manifest anchor | `f445592` |
| `CARMADIO-DEFECT-OPS-001` | Align the active Change to one `0.3.1` to `0.4.0` transaction | `507a4e4` |
| `CARMADIO-GAP-TOOL-002` | Roll back exact-digest transaction-created host destinations | `9680094` |
| `CARMADIO-DEFECT-GOV-004` | Rebuild current Verification from this fresh closure | this evidence-only reconciliation |
