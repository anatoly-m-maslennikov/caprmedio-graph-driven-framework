...
class LayeredValidationTests(unittest.TestCase):
    ...
    def test_schema_1_2_shapes_are_explicit(self) -> None:
        project = json.loads(
            (ROOT / ".carmadio/01_layer_meta/schemas/project.schema.json").read_text(
                encoding="utf-8"
            )
        )
        fragment = json.loads(
            (
                ROOT / ".carmadio/01_layer_meta/schemas/package-fragment.schema.json"
        ...
        change = json.loads(
            (ROOT / ".carmadio/02_layer_gov/schemas/change.schema.json").read_text(
                encoding="utf-8"
            )
        )
        self.assertEqual(
    ...
    def test_schema_1_2_work_area_contract_has_two_target_modes(self) -> None:
        ...
        traceability = json.loads(
            (ROOT / ".carmadio/03_layer_tool/schemas/traceability.schema.json").read_text(
                encoding="utf-8"
            )
        )
