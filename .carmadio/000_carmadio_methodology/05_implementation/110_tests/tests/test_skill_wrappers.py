...
class SkillWrapperTests(unittest.TestCase):
    ...
    def test_implementation_uses_governed_entry_criteria_closure(self) -> None:
        lifecycle = (
            ROOT / ".carmadio/04_layer_skill/procedure-lifecycle-orchestration.md"
        ...
        skill_runs = (ROOT / ".carmadio/04_layer_skill/procedure-skill-runs.md").read_text(
            encoding="utf-8"
        )
        skill_runs_template = (
            ROOT / ".carmadio/04_layer_skill/templates/governance/core-v1/skill-runs.md"
        ).read_text(encoding="utf-8")
