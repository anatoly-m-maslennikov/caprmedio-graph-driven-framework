...
- Author reusable methodology only in the repository-root source. Refresh the
  installed `.carmadio/000_carmadio_methodology/` snapshot only through an explicit
...
`.carmadio/carmadio_settings.toml` selects `artifacts.creation_strictness` independently from
...
`.carmadio/carmadio_settings.toml` records an explicit Boolean for every registered
