...
**Thesis:** When you **implement a deployed service from its spec**, the implementation plan must carry forward the spec's runtime risk profile, concern-scoped durability ownership, and production supportability contract. A stateless process persists through backing services; a local tool may use files at modest write rates or a local database when volume/concurrency demands it. Event sourcing, reconciliation, and durable workflow engines are conditional implementation patterns, not the definition of a service. This is **stage 3** of `00_Tool Development Playbook.md`; deterministic test proof and probabilistic/qualitative eval proof remain separate per `02_Test and Evaluation Case Patterns — Proof Artifact Conventions.md`, and generic code-build patterns live in `04_General Build Rules — Tool Code Conventions.md`. The catalog is §1; the load-bearing four §2; the apply-it checklist §3.
...
| **Build separate test and eval harnesses with the code** — exact checks and fakes implement the test case; datasets, rubrics, thresholds, and budgeted model runs implement the evaluation case | **test doubles / Humble Object** (Meszaros) + **characterization / golden-master tests** (Feathers, 2004) + calibrated evaluation |
...
- [ ] **Test and eval harnesses with the code** — deterministic fakes and pinning tests implement `test-plan.md`; resolved-case replay, rubrics, and budgeted live-model runs implement `eval-plan.md`. Keep their evidence and pass criteria separate per `02_Test and Evaluation Case Patterns — Proof Artifact Conventions.md`.
