...
artifact_id: CARMADIO-REQUIREMENT-META-131
...
3. `plan` accepts and coordinates intended changes to governed artifacts and
   their realization without itself making those changes.
...
Requirement, Method, Assurance, and Delivery collectively form the distributed
specification. Plan governs a change to that specification and its realization
but is not itself part of the specification. Implementation is the realized
...
Plan closes the semantic gap between understanding a concern and changing the
normative specification: it commits and coordinates intended work without
misclassifying that work as Analysis, Requirement, Method, or Implementation.
