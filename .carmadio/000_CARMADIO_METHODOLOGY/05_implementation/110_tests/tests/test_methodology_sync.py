...
METHODOLOGY = ROOT / ".carmadio" / "000_CARMADIO_METHODOLOGY"
...
class MethodologySyncTests(unittest.TestCase):
    ...
    def test_installed_executables_are_real_files_without_symlinks(self) -> None:
        carriers = (
            _unique_carrier("010_dset-implementation-python-run.py"),
            _unique_carrier("010_dset-implementation-tests-run.py"),
            _unique_carrier(
                "010_dset-implementation-evaluations-independent-review.md"
            ),
            _unique_carrier(
                "020_dset-implementation-evaluations-result-reconciliation.md"
            ),
        )
    ...
    def test_installed_python_launcher_uses_the_local_package(self) -> None:
        launcher = _unique_carrier("010_dset-implementation-python-run.py")
    ...
    def test_evaluation_implementations_preserve_qa_boundaries(self) -> None:
        ...
        self.assertRegex(reconciliation, r"convert evidence into\s+authority")
