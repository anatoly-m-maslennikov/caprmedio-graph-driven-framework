...
CURRENT_DSET_ROOT: Final[str] = ".carmadio"
...
METHODOLOGY_ROOT: Final[str] = "000_CARMADIO_METHODOLOGY"
...
class RepositoryLayout:
    ...
    layer-owned roots below ``dset/``. Schema 1.3 uses the hidden ``.carmadio/``
    control root. Schema 1.4 separates the portable framework below ``.carmadio``
    ...
    inside ``.carmadio`` while leaving the governed product outside it.
    """
    ...
    def settings_path(self) -> Path:
        ...
        if self.slim:
            return self.carmadio_root / "carmadio_settings.toml"
        return self.root / "carmadio_settings.toml"
    ...
    def project_root(self) -> Path:
        """Handle root using the declared repository contract."""
        if self.separated:
            return self.carmadio_root
        return (
            self.root / "00_project" if self.recursive else self.carmadio_root / "project"
        )
    ...
    def versions_root(self) -> Path:
        """Handle root using the declared repository contract."""
        if self.separated:
            return self.carmadio_root / PROJECT_DELIVERY_ROOT
        return (
            self.root / "10_versions" if self.recursive else self.carmadio_root / "versions"
        )
    ...
    def scopes_root(self) -> Path:
        ...
        if self.recursive:
            return self.root
        return self.carmadio_root if self.slim else self.carmadio_root / "scopes"
    ...
    def framework_layer_root(self, layer: str) -> Path:
        ...
        if self.recursive:
            return self.carmadio_root / RECURSIVE_LAYER_DIRECTORIES[normalized]
    ...
    def framework_project_root(self) -> Path:
        ...
        if self.separated:
            return self.carmadio_root / METHODOLOGY_ROOT / "00_project"
        if self.recursive:
            return self.carmadio_root / "00_project"
    ...
    def manifest_path(self) -> Path:
        ...
        return self.structured_file(self.carmadio_root, "dset.toml")
    ...
    def artifact_type_registry_path(self) -> Path:
        ...
        if self.schema_version == SEPARATED_SCHEMA_VERSION:
            return self.carmadio_root / "artifact_catalog.toml"
        if self.recursive or self.separated:
            return self.settings_path
        if self.slim:
    ...
    def traceability_path(self) -> Path:
        """Handle path using the declared repository contract."""
        if self.recursive or self.separated:
            return self.root / ".carmadio_runtime/generated/traceability.toml"
        if self.slim:
        ...
        return self.structured_file(
            self.carmadio_root, self._preferred_name("traceability")
        )
    ...
    def migrations_root(self) -> Path:
        """Handle root using the declared repository contract."""
        if self.separated:
            return self.root / ".carmadio_runtime" / "migrations"
        if self.recursive:
    ...
    def schema_roots(self) -> tuple[Path, ...]:
        ...
        return (self.carmadio_root / "schemas",)
    ...
    def package_roots(self) -> tuple[Path, ...]:
        ...
        return (self.carmadio_root / "specs" / "packages",)
    ...
    def resolve_dset_path(self, relative: str | Path) -> Path:
        ...
        return self.carmadio_root / _canonical_relative(relative)

    def _owned_file(self, layer: str, name: str) -> Path:
        base = self.layer_root(layer) if self.layered else self.carmadio_root
...
def discover_layout(root: Path) -> RepositoryLayout:
    ...
    slim = current_root / "carmadio_settings.toml"
    ...
    if is_slim:
        ...
        if legacy_root.exists():
            raise ValueError(
                "layout conflict: schema 1.3 .carmadio root and legacy dset root "
...
def has_manifest(root: Path) -> bool:
    ...
    root = root.resolve()
    return (
        (root / CURRENT_DSET_ROOT / "carmadio_settings.toml").is_file()
...
def _registered_snapshot_after_cutover(snapshot: Path) -> bool:
    ...
    for root in (resolved.parent, *resolved.parents):
        ...
        if registry is None:
            continue
        cutover = (
            (root / ".carmadio/carmadio_settings.toml").is_file()
            or (root / "dset/carmadio_settings.toml").is_file()
