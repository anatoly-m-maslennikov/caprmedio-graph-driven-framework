...
def distribution_source(source_root: Path | None = None) -> Iterator[tuple[Path, str]]:
    ...
    with temporary_directory(prefix="dset-source-") as raw:
        ...
        for directory in APPLIED_LAYER_DIRECTORIES.values():
            (source / ".carmadio" / directory).mkdir(parents=True)
...
def _stage_project(
    ...
    *,
    ...
    carmadio_root = stage / ".carmadio"
    carmadio_root.mkdir()
    methodology_root = carmadio_root / METHODOLOGY_ROOT
    methodology_root.mkdir()
    for directory in ("00_project", *METHODOLOGY_LAYER_DIRECTORIES.values()):
        (methodology_root / directory).mkdir()
    for directory in APPLIED_LAYER_DIRECTORIES.values():
        (carmadio_root / directory).mkdir()
    if source_layout.separated:
        shutil.copytree(
            source_layout.carmadio_root / METHODOLOGY_ROOT,
            methodology_root,
            dirs_exist_ok=True,
        )
    else:
    ...
    manifest_path = carmadio_root / "carmadio_settings.toml"
    _write_combined_settings(
        source_layout.find_template("dset-settings.toml"),
        manifest_path,
        manifest,
    )
    catalog_path = carmadio_root / "artifact_catalog.toml"
    ...
    ops = carmadio_root / APPLIED_LAYER_DIRECTORIES["ops"]
    ...
    (carmadio_root / "CARMADIO-CONTROL-HUB.md").write_text(
        ...
        "- `000_CARMADIO_METHODOLOGY`\n"
        "- `03_REQUIREMENT`\n"
        "- `06_DELIVERY`\n",
        encoding="utf-8",
    )
    project_requirements = carmadio_root / CONTENT_ROLE_DIRECTORIES[2]
    ...
    delivery = carmadio_root / PROJECT_DELIVERY_ROOT
    delivery.mkdir()
...
def _materialize_package(
    ...
    destination = stage / ".carmadio" / APPLIED_LAYER_DIRECTORIES["meta"]
    replacements = {
        "{{project_key}}": project_key,
        "{{package_id}}": package_id,
        "{{title}}": project_name,
