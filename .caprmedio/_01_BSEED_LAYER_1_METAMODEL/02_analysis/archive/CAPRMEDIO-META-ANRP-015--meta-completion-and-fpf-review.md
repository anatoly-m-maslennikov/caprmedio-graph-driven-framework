...
coherent with the FPF principles that CAPRMEDIO intentionally adapts?
...
- active `CAPRMEDIO-REQUIREMENT-META-*` atomic records
...
- Authority, evaluation, implementation, observation, and evidence are
...
| ID | Priority | Finding | CAPRMEDIO evidence | FPF consequence |
|---|---|---|---|---|
| META-REVIEW-001 | High | Atomic provenance is incomplete. `CAPRMEDIO-META-REQU-182--governed-feedback-cycle` through `017` appear as evergreen requirements without active atomic carriers, while active `CAPRMEDIO-META-REQU-196--three-axis-artifact-routing` is not directly cited by the root evergreen META documents. | `040_dset-meta-specification-methodology.md`; `CAPRMEDIO-META-REQU-196--three-axis-artifact-routing.md` | A view cannot provide an auditable return to its governing source. |
| META-REVIEW-002 | High | The META entity table does not obey its own topological-definition rule. `Project truth` depends on undefined artifact concepts, and `Design` references Requirements, Scenarios, and Contracts before all are defined. | `030_dset-meta-specification-domain.md:7-35`; `CAPRMEDIO-INVARIANT-META-019` | Definition order hides semantic imports and weakens kind recovery. |
| META-REVIEW-003 | High | Stateful coverage is incomplete. Governance-surface activation exists, but Governance Surface is not a domain entity and has no explicit inactive, active, stale, current, deactivated, and reconciliation semantics. | `CAPRMEDIO-META-REQU-209--progressive-governance-surface-activation`; `040_dset-meta-specification-methodology.md:430-446`; `CAPRMEDIO-INVARIANT-META-020` | A state-bearing relation is described without all decision-relevant slots. |
| META-REVIEW-004 | High | Layer handoffs promise entry criteria, exit criteria, and blocker behavior, but the handoff table records only inputs and outputs. | `040_dset-meta-specification-methodology.md:329-343`; `CAPRMEDIO-META-REQU-202--layer-handoff-contracts` | The relation is not replayable or conformance-checkable. |
| META-REVIEW-005 | High | The META public contract imports volatile downstream realization facts: exact directories, CLI invocation, wrapper count, fixtures, and release implementation. | `020_dset-meta-specification-contracts.md:10-33`; `CAPRMEDIO-META-REQU-200--meta-eligibility-rule` | Volatile implementation pulls the stable constitutional layer into downstream change cycles. |
| META-REVIEW-006 | High | Governance-of-governance lacks a complete amendment protocol for META itself. Eligibility and forward propagation exist, but there is no explicit sequence from accepted META atom and rationale through impact mapping, constitutional refresh, downstream invalidation, and fixed-point verification. | `CAPRMEDIO-META-REQU-200--meta-eligibility-rule`; `CAPRMEDIO-META-REQU-204--forward-change-propagation`; `CAPRMEDIO-META-REQU-207--bounded-recursive-self-hosting` | Normative Core change can lose rationale, distribution boundaries, or amendment replayability. |
| META-REVIEW-007 | Medium | Empty Outcomes and User Stories evergreen files remain as placeholders despite atomic-first progressive activation. | `050_dset-meta-specification-outcomes.md`; `060_dset-meta-specification-user-stories.md`; `CAPRMEDIO-META-REQU-209--progressive-governance-surface-activation` | Optional structure is materialized without current coordination value. |
...
- The methodology comparison reported extensive changed, missing, and
  ...
  `.caprmedio/000_caprmedio_methodology` snapshot.
