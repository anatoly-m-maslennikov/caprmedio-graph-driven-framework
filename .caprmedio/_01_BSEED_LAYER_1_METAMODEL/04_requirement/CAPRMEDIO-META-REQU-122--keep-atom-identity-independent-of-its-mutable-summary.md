...
stable identity. A draft may change its Summary while it omits the `atom_id`
property. An accepted role-classified Atom may change its Summary through a governed Revision
while preserving the exact assigned `atom_id` property value.
...
A change to the primary atomic meaning, rather than a filename or Summary
change by itself, requires a successor Atom and a new `atom_id`.
