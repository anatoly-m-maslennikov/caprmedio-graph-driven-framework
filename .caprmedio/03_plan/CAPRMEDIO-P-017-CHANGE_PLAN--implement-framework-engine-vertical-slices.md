...
subject_scopes:
  - development-flow
version: 3
updated_at: 2026-08-19 07:28:00
---
# Implement the Framework Engine in vertical slices

1. Reconcile the selected FRAMEWORK_ENGINE Requirements, Methods, Evaluations, and Deliveries with the final SKILLS, TOOLS, and APP Feature structure before implementing them.
...
6. Implement the APP Feature as a source indexer, rebuildable derived database, local read-only server, and interconnected web interface. Atoms and Journals remain the source; the database and views remain disposable and rebuildable. Select the database engine through a separate accepted Method before binding the implementation.
...
9. Finish with one natural-language request that routes through a Skill, finds governed context, dry-runs and applies a source change, validates it, refreshes projections and the APP read model, and records the result through the Work Journal.
