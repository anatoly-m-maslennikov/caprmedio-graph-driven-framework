...
One governed subject change plus its complete related Journal sidecar set gives Hook and retry logic one deterministic recovery boundary, preserves the exact typed upstream graph consumed by the change, and prevents unrelated work from sharing that boundary.
