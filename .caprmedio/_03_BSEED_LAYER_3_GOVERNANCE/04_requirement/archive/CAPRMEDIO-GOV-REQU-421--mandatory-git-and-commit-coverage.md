...
Git is mandatory for every CAPRMEDIO-managed project.
...
Every CAPRMEDIO-managed project uses Git, dset-init requires an existing enclosing repository or initializes one, and every implemented Decision or resolved Problem has at least one committed change that names the governed artifact.
