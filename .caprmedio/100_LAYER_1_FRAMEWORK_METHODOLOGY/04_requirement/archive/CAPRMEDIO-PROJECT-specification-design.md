...
`CAPRMEDIO-GOV-REQU-386--self-hosted-governance-loci` assigns contextual governance defaults for this
self-hosting repository:

- root framework and methodology sources outside `.caprmedio` are internal;
- `.caprmedio/000_caprmedio_methodology` is the external installed-framework boundary;
- all other applied project artifacts under `.caprmedio` are internal.
...
|---|---|---|---|---|
| Selected profile | `.caprmedio/caprmedio_settings.toml` | Project change | Explicit reviewed edit | Schema and repository check |
...
| Local skill-run evidence | Ignored `.caprmedio_runtime/runs/` records | Invoked skill/runtime | Every run; bounded retention | Schema/redaction tests and authoritative-state comparison |
...
- Use the exact schema 1.2 migration commit recorded in `10_versions/version.toml` as a temporary fixed-point baseline. It is explicitly not a released-validator substitute: `CAPRMEDIO-SPEC-TOOLS-CONC-069--released-validator-cannot-parse-schema-1-2` remains open until a published compatible validator checks a later candidate or an accepted compatibility proof closes the gap.
...
- Store bounded skill-run evidence under ignored `.caprmedio_runtime/runs/`; never let heuristics override accepted artifacts, Git history, or hosted state.
...
- **CAPRMEDIO-GOV-CONC-038--should-action-become-a-first-class-entity — First-class Action entity** in `intake.toml`:
