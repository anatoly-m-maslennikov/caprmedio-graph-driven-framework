...
- Extract the pinned pre-transition validator from Git history and record its
  schema 1.2 rejection as degraded bootstrap evaluation rather than a pass.
...
- Define `CAPRMEDIO-CONTRACT-SKILL-001` for real installable host-native skill artifacts and clean install/discovery/load/invocation proof on every declared agent host.
- Define `CAPRMEDIO-CONTRACT-TOOL-001` for macOS, native Windows, WSL, and Linux behavior or explicitly narrower applicability proven before execution.
- Define `CAPRMEDIO-CONTRACT-TOOL-002` for dependency allowlist/denylist, exact registry/version/license/provenance and lockfile authority, plus bounded authorized exceptions with expiry.
- Define `CAPRMEDIO-CONTRACT-OPS-001` for real GitHub Actions workflow/run/check evidence bound to the actual implementing PR SHA and protected integration.
...
- Apply the flat semantic model under `CAPRMEDIO-GOV-METH-017--flat-canonical-type-and-subtype-model`; keep Conflict as
...
- Enforce current Change/intake/Decision/proof records with stable
  `CAPRMEDIO-E155`, version and test run/checkpoint schemas, and backfill this
...
- Keep `CAPRMEDIO-RULE-ARCHITECTURE` as the sole dependency-free governance root and
  separate rule authority from evaluation.
...
- Promote the project-local artifact classes, transactional discharge,
  ...
  profile version 0.3.
- Compile `CAPRMEDIO-GOV-METH-012--governance-constitution-and-rule-evaluation` into GOV requirements, tests, evals, schema,
...
- Migrate CAPRMEDIO-owned structured artifacts and Markdown frontmatter to TOML.
...
## Batch 12 — Complete CAPRMEDIO-owned TOML cutover — complete

- Replace byte-level immutability with semantic immutability under
  `CAPRMEDIO-GOV-METH-028--semantic-immutability-and-toml-carriers`; permit carrier changes only through a lossless,
...
- Convert every CAPRMEDIO-owned standalone YAML artifact to an adjacent historical
  TOML envelope, and convert every CAPRMEDIO-owned Markdown YAML frontmatter block
