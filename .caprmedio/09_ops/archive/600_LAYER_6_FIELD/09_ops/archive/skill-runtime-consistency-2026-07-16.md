...
- **Intended use:** Support `CAPRMEDIO-SPEC-SKILLS-METH-055--select-the-dset-0-3-operating-shape` and
  `CAPRMEDIO-GOV-METH-015--semantic-artifact-types-and-open-conflicts` for the bounded deterministic claims above.
- **LLM session IDs:**
...
release-transition library cores are implemented. `CAPRMEDIO-TASK-TOOL-010` remains
open for initialization, the portable CLI/host bridge, synchronized version
writes, and post-merge publication. `CAPRMEDIO-TASK-SKILL-020` remains open for real
...
independent eval remain open under `CAPRMEDIO-TASK-GOV-045` and
`CAPRMEDIO-TASK-TOOL-046`.
