...
- **Intended use:** Support `CAPRMEDIO-REQUIREMENT-GOV-022`,
  `CAPRMEDIO-TEST-CASE-GOV-022`, and `CAPRMEDIO-TASK-GOV-041`; not certify hosted delivery,
...
- **Method/setup:** Resolve the current repository-owned lifecycle rules; audit
  ...
  schemas, templates, and `CAPRMEDIO-E155`; run canonical verification; then run the
...
- **Reopen when:** Change/intake/Decision/proof/run/checkpoint provenance shape,
  the YAML subset, `CAPRMEDIO-E155`, the canonical verification commands, or bounded
...
| Failure contract | Missing or malformed Change/intake provenance and missing Decision/proof fields produce stable `CAPRMEDIO-E155`; Markdown `none` is accepted only as an explicit human-only disposition | Pass |
...
| Commit provenance | The evaluated commit body contains `Implements: CAPRMEDIO-GOV-METH-011--artifact-classes-compilation-and-provenance` and the contributing Codex session | Pass |
...
`CAPRMEDIO-SPEC-TOOLS-CONC-069--released-validator-cannot-parse-schema-1-2`, not a failure of the candidate or adopter checks.
