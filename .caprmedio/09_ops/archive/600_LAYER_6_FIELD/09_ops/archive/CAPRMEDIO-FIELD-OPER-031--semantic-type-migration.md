...
- **Claim:** DSET executes one flat four-Type semantic model for native atoms
  ...
  carriers.
- **Intended use:** Support `CAPRMEDIO-GOV-METH-018--fpf-aligned-boundaries-for-the-flat-type-model`,
  `CAPRMEDIO-REQUIREMENT-GOV-027`, `CAPRMEDIO-INVARIANT-GOV-004`,
  `CAPRMEDIO-INVARIANT-GOV-017`, `CAPRMEDIO-TEST-CASE-GOV-027`, and
...
| Canonical vocabulary | QA definitions use Test Case/Evaluation Case identities; standalone Opportunity maps to Question/Opportunity; the governed historical identity graph uses the same canonical tokens as current artifacts | Pass |
| Fail closed | `CAPRMEDIO-E166` rejects ID-kind/carrier mismatch; nested, invalid, and QA-without-subtype native atoms remain rejected | Pass |
