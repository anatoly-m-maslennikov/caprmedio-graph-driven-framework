...
Realize `START_BACKGROUND_SERVICES` through `002_FRAMEWORK_ENGINE/TOOLS/START_BACKGROUND_SERVICES/start_background_services.py`. Install the Tool and `002_FRAMEWORK_ENGINE/TOOLS/background_services.toml` in the same selected release. The Tool exposes machine-readable `describe`, read-only `status`, dry-run `run`, and explicit `run --apply` interfaces.
