...
`GRAPH_CHECK` must be one Checker Tool owned immediately by `TOOLS` as an `unordered_unit` at Structural level `3`, addressed by `002_FRAMEWORK_ENGINE/TOOLS/GRAPH_CHECK`, and realized under `002_FRAMEWORK_ENGINE/TOOLS/GRAPH_CHECK/`; it applies selected registered Evaluation criteria to one resolved target set and returns stable issues, evidence, and verdicts without changing governed Atoms, Journals, native Implementation, or derived outputs.
