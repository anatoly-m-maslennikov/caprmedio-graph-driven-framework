...
6. Confirm each direct relation is persisted only in its registered
   `declaration_carrier`, and that every inverse remains derived.
7. Confirm `replaced_by` occurs only in the archival Work Journal event, never
   in either predecessor or successor Atom frontmatter.

## Acceptance criteria

All valid examples pass and every invalid fixture fails with the exact relation,
endpoint, or declaration carrier identified.
