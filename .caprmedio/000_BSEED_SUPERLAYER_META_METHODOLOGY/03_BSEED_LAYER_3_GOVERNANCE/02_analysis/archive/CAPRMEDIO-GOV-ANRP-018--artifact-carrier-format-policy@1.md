...
relations:
  - type: analysis_of
    targets:
      - CAPRMEDIO-GOV-REQU-413--python-dset-carriers-use-toml
...
Immediately before this report was added, `.caprmedio` contained:
...
- `.caprmedio/caprmedio_settings.toml`;
...
and rationale, while `caprmedio_settings.toml` selects the profile by stable ID.
...
These normally belong under `.caprmedio_runtime` and are excluded from semantic
...
- Give every evergreen or maintained Markdown artifact valid YAML
  frontmatter.
- Convert identity-only `.caprmedio/**/references/*.toml` carriers to
...
`CAPRMEDIO-GOV-REQU-413--python-dset-carriers-use-toml` currently requires TOML for all Python-owned DSET
...
That Requirement should replace `CAPRMEDIO-GOV-REQU-413--python-dset-carriers-use-toml`, specialize
`CAPRMEDIO-GOV-REQU-451--all-markdown-artifacts-have-frontmatter`, preserve
`CAPRMEDIO-GOV-CNST-001--github-preview-compatibility`, and authorize a separate migration plan.
