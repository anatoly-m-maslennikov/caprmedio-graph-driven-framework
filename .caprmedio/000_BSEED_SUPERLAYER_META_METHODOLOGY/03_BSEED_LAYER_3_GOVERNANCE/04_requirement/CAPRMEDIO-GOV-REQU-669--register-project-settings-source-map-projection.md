...
GOVERNANCE must register the versionless internal Project Graph State Source Map Projection with Content role `implementation` and Type `map`. It maps every emitted Graph State value to the exact Project Configuration Atom revision and any Atom revisions or Journal records that directly contribute that value, records `updated_at`, omits a blanket source frontier, and is never read as authority for values or source selection.
